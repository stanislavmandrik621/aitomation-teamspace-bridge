...
**Public share (P1-C):** Guest links are `https://<this-bridge-origin>/share/<token>` (same host as Team Space). Mint and revoke from the Pro desktop app when connected. Form submissions queue here until the host app drains them into local SQLite. Do **not** put public share rows or Form bodies in Directus.

## Quick start (Docker - preferred)

Use the compose file that ships with the bridge. It names the data volume for
you, so the one decision that loses a whole team if you forget it is already
made:

```bash
cd packages/bridge
# Set TEAMSPACE_AT_REST_KEY in a .env file beside docker-compose.yml first.
docker compose up -d
...
Hand-rolled `docker run` works too, but every flag below is load-bearing - the
named volume most of all, since without `-v name:/data` Docker gives you an
anonymous volume that a later `docker rm` silently orphans:
...
> Developers working in the private AItomation monorepo still edit this package under `packages/bridge`. That path is **not** a customer install path and must not be created inside a desktop project folder.
...
| `TEAMSPACE_OP_RETENTION_DAYS` | `21` | Catch-up window before prune |
| `TEAMSPACE_CHAT_RETENTION_DAYS` | `90` | Seed for team chat prune window (Admin can override live via Settings; stored in `chat/_meta.json`) |
...
| `TEAMSPACE_MAX_OPS_PER_FRAME` | `500` | Ops accepted in one WS frame (remainder stays on the device outbox) |
...
| `TEAMSPACE_OPS_FRAME_TOKENS` | `120` | Frames one device may send per refill window |
| `TEAMSPACE_OPS_FRAME_WINDOW_MS` | `10000` | Token-bucket refill window (ms) |
| `TEAMSPACE_RECENT_OPS_LIMIT` | `5000` | Op-log tail replayed on a reconnect that already has acks |
| `TEAMSPACE_FULL_OPS_CATCHUP_LIMIT` | `200000` | Progress-log interval while streaming the durable log (not a send stop) |
| `TEAMSPACE_ACK_IDS_PER_CALL` | `2000` | Ack ids stamped per `ack_ops` frame (desktop chunks at the same size) |
...
| `TEAMSPACE_MAX_INFLIGHT_HTTP_BODY_BYTES` | `268435456` (256 MiB) | Process-wide ceiling on total bytes buffered across every in-flight HTTP request body (chat/compose blob uploads, JSON bodies) at once. Bounds worst-case resident memory when many uploads land in parallel |
...
| `TEAMSPACE_YJS_ROOM_MAX_PEERS` | `40` | Max sockets allowed to co-edit ONE live Doc/Whiteboard/Compose room at once |
...
| `TEAMSPACE_FANOUT_BACKPRESSURE_BYTES` | `2097152` | Skip fanout to a peer whose send buffer exceeds this |
| `TEAMSPACE_TLS_CERT_FILE` | (unset) | PEM certificate path - with KEY enables native `wss://` / HTTPS |
| `TEAMSPACE_TLS_KEY_FILE` | (unset) | PEM private key path (required with CERT) |
| `TEAMSPACE_AT_REST_KEY` | (unset) | 64-hex key or passphrase (>=16 chars) - AES-GCM for members/invites/acks/ops/blobs |
| `TEAMSPACE_ADMIN_RECOVERY_KEY` | (auto-generated) | Recovery key that lets a locked-out Admin get back in. Must be at least 24 characters - a shorter value stops the server with one clear line instead of being accepted as a weak password. Leave unset and the server generates a strong key on first boot and saves it to `$TEAMSPACE_DATA_DIR/admin-recovery.key`. See [Admin recovery](#admin-recovery-locked-out-of-your-own-server) |
...
- Set `TEAMSPACE_AT_REST_KEY` so members/invites/acks JSON, the op log, and blobs are AES-256-GCM encrypted on disk. Without it, CRM field values in `ops.jsonl` and media blobs are plaintext files - encrypt the volume (FileVault / BitLocker / LUKS) or set the key.
...
1. `TEAMSPACE_ADMIN_RECOVERY_KEY` if you set it (at least 24 characters).
...
Set `TEAMSPACE_ADMIN_RECOVERY_KEY` to a new value (at least 24 characters) and restart - the environment always wins over the file. To go back to a generated key, unset the variable, delete `admin-recovery.key`, and restart. Old keys stop working immediately; sessions already issued are unaffected.
...
The official image is `ghcr.io/stanislavmandrik621/aitomation-teamspace-bridge` (tags: `latest` and `vX.Y.Z`). It binds `0.0.0.0` (`TEAMSPACE_BRIDGE_HOST`) and runs as a non-root `bridge` user with `/data` mode `700`.
...
To build locally from this source tree:
