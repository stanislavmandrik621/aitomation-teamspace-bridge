...
export function parseBackupOwnerFields(raw: unknown): TeamBackupOwnerMeta | undefined {
  ...
  for (const k of BACKUP_META_KEYS) {
    ...
    const cleaned = v.replace(/[\u0000-\u001f\u007f]/g, '').trim().slice(0, BACKUP_META_FIELD_MAX)
...
export class TeamBackupStore {
  ...
  private memberDir(memberId: string): string {
    ...
    return join(this.membersRoot, memberId)
  }
  ...
  private readIndex(memberId: string): TeamBackupSnapshotRow[] {
    ...
    for (const raw of rows.snapshots) {
      ...
      out.push({
        ...
        bytes,
        sha256,
        label: typeof r.label === 'string' ? r.label.replace(/\0/g, '').trim().slice(0, 120) : '',
   ...
   * ceiling under a dedicated global lock key (`__team_quota__`, never a
  ...
  > {
    return this.withMemberLock('__team_quota__', () => {
    ...
    return this.withMemberLock(memberId, async () => {
      ...
      try {
        ...
        try {
          ...
          const msg = err instanceof Error ? err.message : 'Backup write failed'
        ...
        try {
          ...
          return {
            ok: false,
            error: err instanceof Error ? err.message : 'Could not finalize backup',
            status: 500,
          }
        }
        ...
        const snapshot: TeamBackupSnapshotRow = {
          ...
          bytes,
          sha256,
          label: typeof args.label === 'string'
            ? args.label.replace(/\0/g, '').trim().slice(0, 120)
            : '',
  ...
  > {
    ...
    return this.withMemberLock(memberId, () => {
      ...
      try {
        ...
        return {
          ok: false,
          error: err instanceof Error ? err.message.slice(0, 200) : 'Could not delete member backups',
        }
      }
