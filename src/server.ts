import { openIndependentAuthority } from './independent-authority.js'
import { verifyAtRestStartupKey } from './at-rest-startup.js'
import { createRecordTeamworkHttpHandler, type RecordTeamworkHttpDeps } from './record-teamwork-http.js'
import { createRecordTreeHttpHandler } from './record-tree-http.js'
import { recordTeamworkAuthority, resolveRecordTeamworkIdentity, recordTeamworkStatusKeys } from './record-teamwork-authority.js'
import { OfficeObjectStore } from './office-objects.js'
import { createOfficeObjectsHttpHandler } from './office-objects-http.js'
import { parseModuleEditTarget, liveModuleEditLeases, updateModuleEditLeases, type ModuleEditLease } from './module-edit-presence.js'
import { chatFrameIdentityError } from './chat-frame-identity.js'
import { contentClockAdmissionError } from './content-clock-admission.js'
/**
...
import { mkdtemp, rm as removeExportSnapshot } from 'node:fs/promises'
import { tmpdir } from 'node:os'
import { snapshotChatBackupMetadata } from './chat-backup-snapshot.js'
import { pipeline as pipelineBackupDownload } from 'node:stream/promises'
import { Transform as BackupDownloadGuard } from 'node:stream'
...
import {
  BridgeStore,
  MAX_BLOB_BYTES_EXPORT,
  TEAMSPACE_KICK_MEMBERS_MAX,
  TEAMSPACE_KICK_MEMBER_DEVICE_IDS_REPLY_MAX,
} from './store.js'
import { createConfiguredMailService } from './mail-config.js'
import { createMailHttpHandler } from './mail-http.js'
import { configureLocalAttachmentScanner } from './attachment-clamav.js'
import { runChatAttachmentScan } from './chat-dangerous-type.js'
import { WorkspaceWorkStore } from './workspace-work.js'
import { createWorkspaceWorkHttpHandler } from './workspace-work-http.js'
import { WorkspaceCoordinatorMonitor } from './workspace-coordinator-monitor.js'
import { TeamContentLockStore, teamContentLockBlocks, teamContentLockRevisionRefusal } from './team-content-lock-store.js'
import {
...
import { GUEST_LIVE_JS } from './guest-page-live.js'
import { TeamMemberProfileStore } from './team-member-profile-store.js'
import { createTeamMemberProfileHttpHandler } from './team-member-profile-http.js'

/** Admin register body: pack_b64 + meta (~28 MiB ceiling for 20 MiB pack). */
const COMPOSE_SHARE_JSON_BODY_MAX = 28_000_000
/** Member-aware Compose ACL snapshot, capped well below the process body budget. */
...
import {
  ...
  AUTH_HTTP_TOKENS_PER_WINDOW,
  RECORD_TEAMWORK_HTTP_TOKENS_PER_WINDOW,
  MEMBER_PROFILE_HTTP_TOKENS_PER_WINDOW,
  createMemberHttpWriteBudget,
...
import {
  ...
  bootstrapComposeAclSharedDocIds,
...
import { syncComposeContentAccess } from './compose-content-access.js'
import { canonicalContentOp } from './content-access.js'
import { FieldAccessProjection } from './field-access-projection.js'
import { backupReadScope, parseBackupReadTargets } from './backup-read-scope.js'
import {
...
import {
  CHAT_BLOB_UPLOAD_AUTH_REVOKED,
  ChatBlobRegistry,
  chatBlobRoomIds,
} from './chat-blob-registry.js'
...
const CHAT_RETENTION_DAYS = Number(process.env.TEAMSPACE_CHAT_RETENTION_DAYS ?? 0)
...
try {
  ...
  verifyAtRestStartupKey(DATA_DIR, atRestKey)
} catch (err) {
...
const AUTHORITY_DIR = process.env.TEAMSPACE_AUTHORITY_DIR?.trim() || `${DATA_DIR}.authority`
const authorityDirLock = acquireBridgeDataDirLock(AUTHORITY_DIR)
if (!authorityDirLock.ok) { console.error(`[bridge] ${authorityDirLock.reason}`); process.exit(1) }
try {
  // Hold both locks before initializing bindings, so two fresh instances
  // cannot race to assign a different identity to the same authority store.
  openIndependentAuthority(DATA_DIR, AUTHORITY_DIR)
  verifyAtRestStartupKey(AUTHORITY_DIR, atRestKey)
} catch (error) {
  console.error(`[bridge] ${error instanceof Error ? error.message : 'Independent authorization is unavailable'}`)
  process.exit(1)
}
...
try {
  adminRecoveryKey = resolveAdminRecoveryKey(AUTHORITY_DIR)
} catch (err) {
  console.error(`[bridge] ${err instanceof Error ? err.message : `Invalid ${ADMIN_RECOVERY_ENV_VAR}`}`)
  process.exit(1)
}

let fieldAccessCache: { hash: string; policy: FieldAccessProjection } | undefined
const unavailableFieldAccess = new FieldAccessProjection(null, false)
const store = new BridgeStore(DATA_DIR, RETENTION_DAYS, atRestKey, adminRecoveryKey, undefined, AUTHORITY_DIR)
const workspaceWorkStore = new WorkspaceWorkStore(AUTHORITY_DIR, atRestKey, memberId => !!store.findMember(memberId), memberId => { const member = store.findMember(memberId); return !!member && (member.role === 'admin' || member.role === 'member') }, (workspaceId, memberIds, action) => {
  if (action === 'setWorkspaceMembers') {
    try { workspaceCoordinatorMonitor.invalidateAccess(workspaceId) } catch { console.error('[bridge] Coordinator consent invalidation failed; monitoring is unavailable until storage is repaired') }
  }
  const audience = new Set(memberIds)
  for (const [ws, session] of live) {
    if (audience.has(session.memberId) && sessionStillAuthorized(ws, session)) reply(ws, { type: 'workspace_work_changed', workspaceId })
  }
})
const workspaceCoordinatorMonitor = new WorkspaceCoordinatorMonitor(AUTHORITY_DIR, atRestKey, workspaceWorkStore, memberId => {
  const member = store.findMember(memberId)
  return member ? { memberId: member.memberId, role: member.role } : null
})
// TCC-FIX-CMP-002: unlike the old process-local replace map, this survives a
// bridge restart and ordinary Admin changes are per-document mutations. That
// prevents a second Admin's stale reconnect snapshot from re-granting access.
configureComposeAclStore(AUTHORITY_DIR, atRestKey)
store.contentAccess.setComposeAccessCheck(isComposeDocSharedWithMember)
store.contentAccess.setFieldAccessCheck((entityId, fieldSlug, role, action) => currentFieldAccess().allows(entityId, role, fieldSlug, action),
  (entityId, role) => currentFieldAccess().hasPrivateFields(entityId, role, store.contentAccess))
try { syncComposeContentAccess(store) } catch {
  store.contentAccess.failClosed()
  console.error('[bridge] Compose/content authority recovery failed; content access is closed')
}
// Mail tokens are only held by this self-hosted process; no Directus integration.
// Invalid optional mail configuration fails closed without stopping other team services.
let mailService: ReturnType<typeof createConfiguredMailService> = null
configureLocalAttachmentScanner()
try {
  mailService = createConfiguredMailService(DATA_DIR, atRestKey)
} catch {
  console.error('[bridge] Self-hosted mail disabled: check mail OAuth configuration and TEAMSPACE_AT_REST_KEY')
}
const teamContentLockStore = new TeamContentLockStore(DATA_DIR, atRestKey)
...
store.onTeamCreated = (team) => {
  console.warn(
    `[bridge] created a NEW team ${team.teamId} - no team.json existed in ${AUTHORITY_DIR}.` +
...
const chatRooms = new ChatRoomsStore(AUTHORITY_DIR, atRestKey)
...
const publicShares = new PublicShareBridgeStore(AUTHORITY_DIR, atRestKey)
publicShares.setFieldAclReadGuard(hash => store.fieldAcl.isCurrentHash(hash))
const composeShares = new ComposeShareBridgeStore(AUTHORITY_DIR, atRestKey)
const portals = new PortalBridgeStore(AUTHORITY_DIR, atRestKey)
portals.setFieldAclReadGuard(hash => store.fieldAcl.isCurrentHash(hash))
...
const COMPOSE_YJS_NOT_SHARED_REASON = 'This board is not shared with the team'

/**
 * A missing session cannot inherit room access. Admin owns the sharing state;
 * every other peer must be named in the current document audience.
 */
function peerYjsRoomStillAuthorized(
  session: LiveSession | undefined,
  recordId: string,
  contentField: string,
): boolean {
  if (!session) return false
  if (!store.contentAccess.healthy()) return false
  if (!isYjsComposeRecordId(recordId)) return store.contentAccess.mayAccessRecordField(recordId, contentField, session.memberId, session.role)
  if (session.role === 'admin') return true
  return isComposeDocSharedWithMember(contentField, session.memberId)
    && store.contentAccess.mayReadCompose(contentField, session.memberId, session.role)
}

/**
 * Re-check Compose authorization for every relay operation, not only join.
 * The Admin host can replace the shared-document allowlist while sockets are
 * already in a room, so membership in yjsSocketRooms is not durable proof of
 * continued access.
 */
function composeYjsAccessRefusal(
  session: LiveSession,
  recordId: string,
  contentField: string,
  write = false,
): string | null {
  if (write && !isYjsComposeRecordId(recordId)
    && !store.contentAccess.mayAccessRecordField(recordId, contentField, session.memberId, session.role, 'write')) return 'This document field is not writable for your role'
  return peerYjsRoomStillAuthorized(session, recordId, contentField)
    ? null
    : COMPOSE_YJS_NOT_SHARED_REASON
}

/**
 * Replace semantics on the Compose ACL must take effect for live sockets as
 * well as future joins. Remove only peers that are no longer authorized by
 * the replacement audience before the HTTP update is acknowledged. Admin
 * remains connected, including when the document is no longer team-shared.
 */
function evictUnsharedComposeYjsPeers(): number {
  let evicted = 0
  for (const [room, peers] of [...yjsRooms.entries()]) {
    const parsed = parseYjsRoomId(room)
    if (!parsed.ok) continue
    const teamId = boundTeamId()
    const refusal = serializeBridgeFrame({
      type: 'yjs_refuse',
      frameId: 'compose-acl-revoked',
      room,
      reason: COMPOSE_YJS_NOT_SHARED_REASON,
      ...(teamId ? { teamId } : {}),
    })
    for (const peer of [...peers]) {
      const peerSession = live.get(peer)
      if (peerYjsRoomStillAuthorized(peerSession, parsed.recordId, parsed.contentField)) continue
      leaveYjsRoom(peer, room)
      evicted += 1
      if (peer.readyState === 1) {
        sendSerializedFrame(peer, refusal, FANOUT_BACKPRESSURE_BYTES)
      }
    }
  }
  return evicted
...
/** memberId (invitee) -> formations they joined, until formation terminates (including accepted invitees). */
...
/** Invitation/member selectors must not alias after NUL removal or truncation. */
function exactChatInviteIdentity(raw: unknown): string {
  return typeof raw === 'string' && raw.length > 0 && raw.length <= 128 && raw.trim() === raw
    && !raw.includes('\0') && raw.isWellFormed() ? raw : ''
}
...
function cancelEphemeralGroupFormation(
  ...
  for (const memberId of f.allMemberIds) {
...
function fanoutYjsUpdate(
  ...
  const peers = yjsRooms.get(room)
  if (!peers) return
  const parsedRoom = parseYjsRoomId(room)
  if (!parsedRoom.ok) return
  const teamId = boundTeamId()
  ...
  for (const peer of peers) {
    ...
    if (peer.readyState !== 1 /* OPEN */) continue
    if (!peerYjsRoomStillAuthorized(live.get(peer), parsedRoom.recordId, parsedRoom.contentField)) { leaveYjsRoom(peer, room); continue }
...
function presencePeerFromSession(s: LiveSession): PresencePeer {
  return {
    ...
    deviceId: s.deviceId,
    lastSeen: s.lastSeen,
    editing: s.role === 'viewer' ? [] : liveModuleEditLeases(s.editing),
    avatarRef: store.findMember(s.memberId)?.avatarRef ?? undefined,
  }
}

function presenceSnapshotBase(): PresencePeer[] {
  return buildPresenceSnapshot([...live.values()].map(session => {
    const peer = presencePeerFromSession(session)
    peer.editing = peer.editing?.filter(target => store.contentAccess.mayReadEditTarget(target, session.memberId, session.role)) ?? []
    return peer
  }))
}
function presenceSnapshotNow(recipient?: LiveSession, base = presenceSnapshotBase()): PresencePeer[] {
  if (!base.some(peer => peer.editing?.length)) return base
  return base.map(peer => ({ ...peer, editing: peer.editing?.filter(target => !!recipient
    && store.contentAccess.mayReadEditTarget(target, recipient.memberId, recipient.role)) ?? [] }))
}
...
function schedulePresenceFanoutBroadcast(): void {
  ...
  presenceFanoutTimer = setTimeout(() => {
    ...
    const teamId = boundTeamId()
    const snapshot = presenceSnapshotBase()
    const buckets = new Map<string, { frame: BridgeFrame; sockets: WebSocket[] }>()
    for (const [sock, recipient] of live) {
      if (sock.readyState !== 1) continue
      const peers = presenceSnapshotNow(recipient, snapshot)
      const key = peers.map(peer => (peer.editing ?? []).map(lease => lease.leaseId).join('\0')).join('\n')
      const bucket = buckets.get(key)
      if (bucket) bucket.sockets.push(sock)
      else buckets.set(key, { frame: { type: 'presence_snapshot', peers, ...(teamId ? { teamId } : {}) }, sockets: [sock] })
    }
    for (const bucket of buckets.values()) {
      const text = serializeBridgeFrame(bucket.frame)
      for (const sock of bucket.sockets) {
      if (!sendSerializedFrame(sock, text, FANOUT_BACKPRESSURE_BYTES)) {
        // Presence is a full-replace authority frame. A peer that misses it
        // must reconnect for a fresh snapshot instead of staying stale.
        forceCloseBackpressured(sock, 'presence fanout backpressure')
      }
      }
    }
  }, PRESENCE_JOIN_COALESCE_MS)
...
function pushPresenceSnapshot(ws: WebSocket, frameId?: string): void {
  ...
  const frame: BridgeFrame = frameId
    ? { type: 'presence_snapshot', frameId, peers: presenceSnapshotNow(live.get(ws)), ...(teamId ? { teamId } : {}) }
    : { type: 'presence_snapshot', peers: presenceSnapshotNow(live.get(ws)), ...(teamId ? { teamId } : {}) }
  if (!reply(ws, frame)) {
    forceCloseBackpressured(ws, 'presence snapshot backpressure')
  }
}
...
const socketCatchUpAckWindow = new WeakSet<WebSocket>()
const socketRestoreReplay = new WeakSet<WebSocket>()
const socketCatchUpFrame = new WeakMap<WebSocket, { id: string; processed: boolean }>()
...
function sessionStillAuthorized(ws: WebSocket, session: LiveSession): boolean {
  if (live.get(ws) !== session) return false
  const auth = store.findBySession(session.sessionToken)
  return !!auth && auth.member.memberId === session.memberId
    && auth.deviceId === session.deviceId && auth.member.role === session.role
}

/** Capture authority at admission, then revalidate inside the room write lock. */
function chatCommitGuard(
  ws: WebSocket,
  session: LiveSession,
  room: string | undefined,
  allowAdminOutsideRoom = false,
  requirePinPermission = false,
): () => boolean {
  const admittedRole = session.role
  return () => {
    const member = store.findMember(session.memberId)
    if (!sessionStillAuthorized(ws, session) || !member || member.role !== admittedRole || admittedRole === 'viewer') return false
    if (room && !(allowAdminOutsideRoom && admittedRole === 'admin') && !chatRooms.memberCanAccess(room, session.memberId)) return false
    if (requirePinPermission) {
      const row = room ? chatRooms.get(room) : undefined
      if (!row || !canPinRoomMessages(row, session.memberId, admittedRole)) return false
    }
    return true
  }
}
...
function fanoutChatPeer(
  ...
  for (const [peer, sess] of live) {
    ...
    if (!sendSerializedFrame(peer, text, FANOUT_BACKPRESSURE_BYTES)) {
      forceCloseBackpressured(peer, 'chat fanout backpressure')
    }
  }
}
...
function handleMemberLeaveChat(memberId: string): void {
  ...
  // Offboarding is not a UI page: walk every open room, not the capped
  // listForMember page, or a high-volume member remains in the tail rooms.
  for (const row of chatRooms.listAllOpenRooms()) {
    if (row.kind !== 'team' && !row.memberIds.includes(memberId)) continue
  ...
  if (cleanupFailed) throw new Error('one or more chat cleanup writes failed')
}

/**
 * Complete live offboarding for one or many roster removals. Session authority
 * is dropped before close/chat work, so a failing cleanup cannot leave a kicked
 * socket able to submit another frame. Chat failures are returned honestly to
 * the Admin while the successful roster/session removal remains committed.
 */
function finalizeKickedMembers(memberIds: readonly string[]): string[] {
  for (const memberId of memberIds) {
    try { workspaceCoordinatorMonitor.invalidateAccess(undefined, memberId) } catch { console.error('[bridge] Coordinator consent invalidation failed after member removal') }
  }
  const targets = new Set(
    memberIds
      .map((memberId) => capTrim(memberId, LIST_MEMBER_DEVICE_ID_CAP))
      .filter(Boolean),
  )
  if (targets.size === 0) return []
  for (const [peerWs, peer] of [...live]) {
    if (!targets.has(peer.memberId)) continue
  ...
  const chatCleanupFailedMemberIds: string[] = []
  for (const memberId of targets) {
    try {
      handleMemberLeaveChat(memberId)
    } catch (err) {
      chatCleanupFailedMemberIds.push(memberId)
      console.error(
        `[bridge] kicked member ${logId(memberId)} chat cleanup failed: ` +
          `${err instanceof Error ? capStr(err.message, 160) : 'error'}`,
      )
    }
  }
  if (chatCleanupFailedMemberIds.length > 0) {
    // Transient disk/registry failures get one delayed repair attempt. Access
    // is already gone synchronously; this only heals retained room metadata.
    const retryIds = [...chatCleanupFailedMemberIds]
    const timer = setTimeout(() => {
      for (const memberId of retryIds) {
        try { handleMemberLeaveChat(memberId) } catch { /* surfaced in original ack/log */ }
      }
    }, 1_000)
    timer.unref()
  }
  return chatCleanupFailedMemberIds
}
...
// Mail remains pre-auth IP bounded, but a shared office NAT must not share
// the much smaller general HTTP allowance with every mailbox page request.
const mailHttpLimiter = new TokenBucketLimiter()
const MAIL_HTTP_TOKENS_PER_WINDOW = 600
...
const recordTeamworkHttpBudget = createMemberHttpWriteBudget(RECORD_TEAMWORK_HTTP_TOKENS_PER_WINDOW)
const memberProfileHttpBudget = createMemberHttpWriteBudget(MEMBER_PROFILE_HTTP_TOKENS_PER_WINDOW)
...
function takeHttpToken(key: string, tokens: number): boolean {
  return httpLimiter.take(key, tokens, OPS_FRAME_WINDOW_MS)
}
...
/**
 * Parse an audience without ever turning one member id into another. In
 * particular, truncating an over-cap id can alias a real roster member and
 * disclose a scoped op to that person. Persisted legacy/malformed audiences
 * therefore collapse to the closed empty audience, while new inbound writes
 * are refused by handleOps below.
 */
function parseOpVisibleToMemberIds(op: ModulesSyncOp): ParsedOpAudience {
  const hasTop = Object.prototype.hasOwnProperty.call(op, 'visibleToMemberIds')
  const hasPatch =
    !!op.patch
    && typeof op.patch === 'object'
    && !Array.isArray(op.patch)
    && Object.prototype.hasOwnProperty.call(op.patch, 'visibleToMemberIds')
  if (!hasTop && !hasPatch) return { present: false, valid: true, ids: [] }
  const raw = hasTop
    ? op.visibleToMemberIds
    : (op.patch as { visibleToMemberIds: unknown }).visibleToMemberIds
  // A present malformed audience is an explicit but unreadable restriction,
  // never permission to fan the op to the whole team.
  ...
  for (const rawId of raw) {
    const id = exactMemberDeviceId(rawId)
    if (!id) return { present: true, valid: false, ids: [] }
    if (seen.has(id)) continue
    seen.add(id)
    ids.push(id)
  }
  return { present: true, valid: true, ids }
}

function opVisibleToMemberIds(op: ModulesSyncOp): string[] | undefined {
  const parsed = parseOpVisibleToMemberIds(op)
  return parsed.present ? parsed.ids : undefined
}
...
function peerMayReceiveScopedOp(
  ...
  return projectScopedOp(peerSession, op) !== null
}

function currentFieldAccess(pending?: ModulesSyncOp[]): FieldAccessProjection {
  const hash = pending ? store.fieldAcl.previewHash(pending) : store.fieldAcl.currentHash()
  if (!hash) return unavailableFieldAccess
  if (fieldAccessCache?.hash !== hash) {
    const authority = pending ? store.fieldAcl.previewSnapshot(pending) : store.fieldAcl.snapshot()
    if (!authority) return unavailableFieldAccess
    fieldAccessCache = { hash, policy: new FieldAccessProjection(authority.bag) }
  }
  return fieldAccessCache.policy
}

function projectScopedOp(peerSession: LiveSession, op: ModulesSyncOp): ModulesSyncOp | null {
  const scoped = store.contentAccess.projectForRead(op, peerSession.memberId, peerSession.role)
  const projected = scoped ? currentFieldAccess().project(scoped, peerSession.role, store.contentAccess) : null
  if (!projected) return null
  const visible = opVisibleToMemberIds(projected)
  if (visible === undefined) return projected
  if (peerSession.memberId === op.originMemberId) return projected
  ...
  if (peerSession.role === 'admin') return projected
  return visible.includes(peerSession.memberId) ? projected : null
}
...
function sendColleagueScopedOps(
  ...
  // Authority deltas create their own durable cleanup obligations. Deliver
  // them live as well as on reconnect, even if the sender queued no notice.
  const roots = new Set(scoped.filter(op => /^(module|playbook|compose)\./.test(op.kind)).map(op => store.contentAccess.itemKey(op)))
  if (roots.size) {
    const ids = new Set(scoped.map(op => op.opId))
    scoped = [...scoped, ...store.contentAccess.cleanupNotices().filter(op => !ids.has(op.opId) && roots.has(store.contentAccess.itemKey(op)))]
  }
  ...
  for (const [peer, s] of live) {
    ...
    const forPeer = scoped.flatMap(op => {
      const projected = projectScopedOp(s, op)
      if (projected) projectionKeys.push(projected === op ? `id:${op.opId}` : `projection:${JSON.stringify(projected)}`)
      return projected ? [withContentAuthority(projected)] : []
    })
    if (forPeer.length === 0) continue
    // The same opId can now have different safe subsets for different peers.
    // Group by the delivered body, never reuse another recipient's projection.
    const key = projectionKeys.join('\0')
...
function removeMembersFromComposeAuthority(memberIds: string[]): ReturnType<typeof removeComposeAclMembers> {
  const result = removeComposeAclMembers(memberIds)
  if (!result.ok) return result
  try {
    const changed = syncComposeContentAccess(store)
    if (changed.length) sendColleagueScopedOps(null, 'compose-access', changed)
    return result
  } catch { return { ok: false, reason: 'Could not persist Compose content authority' } }
}
...
function handleOps(ws: WebSocket, session: LiveSession, frameId: string, ops: unknown): void {
  ...
  const previousFieldAclHash = store.fieldAcl.snapshot()?.hash
  const toPersist: ModulesSyncOp[] = []

  // The safety lock is enforced at the durable server boundary. This remains
  // effective for older protocol-v2 clients that do not know the lock frames.
  const contentLock = teamContentLockStore.get(boundTeamId())
  if (teamContentLockBlocks(contentLock, 'modules_ops')) {
    for (const raw of batch) {
      const opId = raw && typeof raw === 'object' && typeof raw.opId === 'string'
        ? capStr(raw.opId, 200) || 'unknown'
        : 'unknown'
      results.push({
        opId,
        status: 'refused',
        reason: contentLock.unavailable
          ? 'Team content lock is unavailable on the server'
          : 'Team content is locked by an Admin',
        // An attempt made while the authoritative lock is active is never a
        // delayed write. Legacy clients must quarantine it so an unlock does
        // not replay the edit later.
        permanent: true,
      })
    }
    reply(ws, { type: 'ops_result', frameId, results, fieldAclBaseline: currentFieldAclBaseline(),
      ...(session.role === 'admin' ? { fieldAclAuthority: store.fieldAcl.snapshot() } : {}) })
    return
  }
  ...
  for (const raw of batch) {
    ...
    try { op = canonicalContentOp(op) } catch {
      results.push({ opId, status: 'refused', reason: 'Invalid content patch', permanent: true })
      continue
    }
    ...
    const lockRevisionRefusal = teamContentLockRevisionRefusal(contentLock, op.lockRevision)
    // Revision 0 preserves compatibility with servers/clients that predate
    // the lock. Once this team has ever transitioned the lock, every author
    // must prove the exact unlocked revision captured at edit time. A queued
    // pre-lock or legacy write is permanently refused after unlock.
    if (lockRevisionRefusal) {
      results.push({
        opId,
        status: 'refused',
        reason: lockRevisionRefusal === 'missing'
          ? 'Update the app before changing shared content after a team lock'
          : 'Team content lock revision changed; discard this queued change',
        permanent: true,
      })
      continue
    }
    const parsedAudience = parseOpVisibleToMemberIds(op)
    if (!parsedAudience.valid) {
      results.push({
        opId,
        status: 'refused',
        reason: 'Invalid member audience',
        permanent: true,
      })
      continue
    }
    const scopedVisible = parsedAudience.present ? parsedAudience.ids : undefined
    let stamped: ModulesSyncOp = {
      ...op,
      opId,
      ...
      // Team identity is authority, just like member identity. Never relay a
      // sender's claimed team into another team's desktop ownership gates.
      teamId: boundTeamId(),
      team_id: boundTeamId(),
      // BRG-109: device attribution is part of the collaboration audit trail.
      // It must come from the authenticated socket binding just like member
      // id/name/role; accepting a client-supplied string let a modified client
      // impersonate another device and also broke self-origin catch-up logic.
    ...
    // Duplicate commands cannot grant a pending baseline to later frame rows.
    // Only an exact committed retry bypasses current-epoch authorization.
    if (store.hasSeenOpId(opId)) {
      results.push(store.isExactOpRetry(stamped)
        ? { opId, status: 'applied' }
        : { opId, status: 'refused', reason: 'opId was already used for a different change', permanent: true })
      continue
    }
    if (toPersist.some(previous => previous.opId === opId)) {
      results.push({ opId, status: 'refused', reason: 'Duplicate opId in one frame', permanent: true })
      continue
    }
    if(stamped.kind==='record.teamwork'){results.push({opId,status:'refused',reason:'Use the authenticated record teamwork command endpoint',permanent:true});continue}
    const clockRefusal = contentClockAdmissionError(stamped)
    if (clockRefusal) {
      results.push({ opId, status: 'refused', reason: clockRefusal, permanent: true })
      continue
    }
    const contentRefusal = store.contentAccess.authorize(stamped, session.memberId, session.role, toPersist)
    if (contentRefusal) {
      results.push({ opId, status: 'refused', reason: contentRefusal, permanent: store.contentAccess.healthy() })
      continue
    }
    try { stamped = store.fieldAcl.stamp(stamped, session.role, toPersist) } catch (error) {
      results.push({ opId, status: 'refused', reason: store.fieldAcl.healthy() && error instanceof Error
        ? error.message : 'Field access authority unavailable', permanent: store.fieldAcl.healthy() })
      continue
    }
    const fieldRefusal = currentFieldAccess(toPersist).writeRefusal(stamped, session.role, store.contentAccess)
    if (fieldRefusal) {
      results.push({ opId, status: 'refused', reason: fieldRefusal, permanent: store.fieldAcl.healthy() })
      continue
    }
    try {
      const identity = stamped.kind === 'record.teamwork' || stamped.kind.startsWith('record.') && !!store.recordTeamwork.read(stamped.targetId).config
        ? resolveRecordTeamworkIdentity(store, boundTeamId(), stamped.targetId, store.findMember(session.memberId)!) : null
      if (identity) stamped = store.recordTeamwork.stamp(stamped,
        recordTeamworkAuthority(store, identity, store.findMember(session.memberId)!, (teamId,id) => officeObjectStore.hasDepartment(teamId,id)),
        state => recordTeamworkStatusKeys(store, identity.entityId, state), toPersist)
      else delete stamped.teamwork
    } catch (error) {
      results.push({ opId, status: 'refused', reason: error instanceof Error ? error.message : 'Record teamwork is unavailable', permanent: true })
      continue
    }
  ...
  try {
    const persisted = store.appendOps(toPersist)
    for (const outcome of persisted.outcomes) {
      if (outcome.status === 'accepted') {
        results.push({ opId: outcome.op.opId, status: 'applied' })
        applied.push(outcome.op)
      } else if (outcome.status === 'idempotent') {
        // A lost ops_result may cause an exact retry. Confirm it without a
        // second durable line or a second live fanout.
        results.push({ opId: outcome.opId, status: 'applied' })
      } else {
        results.push({
          opId: outcome.opId || 'unknown',
          status: 'refused',
          reason: outcome.status === 'conflict'
            ? 'opId was already used for a different change'
            : 'persist failed',
          permanent: outcome.status === 'conflict',
        })
      }
    }
  } catch (err) {
  ...
  const authoritativeResults = results.map(result => {
    const source = batch.find(op => op && op.opId === result.opId)
    const item = source ? store.contentAccess.itemKey(source) : ''
    return item && source ? { ...result, contentAclItem: item, contentAclRevision: store.contentAccess.revision(source) } : result
  })
  const resultOk = reply(ws, { type: 'ops_result', frameId, results: authoritativeResults, fieldAclBaseline: currentFieldAclBaseline(),
    ...(session.role === 'admin' ? { fieldAclAuthority: store.fieldAcl.snapshot() } : {}) })
  if (!resultOk) {
    forceCloseBackpressured(ws, 'ops_result backpressure')
    // Persistence already succeeded. The sender's failed acknowledgement
    // must not suppress delivery to healthy peers that remain connected.
  }

  if (applied.length > 0) {
    if (store.fieldAcl.snapshot()?.hash !== previousFieldAclHash) {
      const baseline = currentFieldAclBaseline()
      const baselineText = serializeBridgeFrame({ type: 'field_acl_baseline', teamId: boundTeamId(), fieldAclBaseline: baseline })
      // Members need the current authorship version after a privacy change,
      // including when they cannot receive the module that carries the ACL.
      // Never send the admin grant bag or hidden item identifiers here.
      for (const [peer, recipient] of live) if (peer !== ws && sessionStillAuthorized(peer, recipient)) {
        if (!sendSerializedFrame(peer, baselineText, FANOUT_BACKPRESSURE_BYTES)) forceCloseBackpressured(peer, 'field access baseline undelivered')
      }
    }
    evictUnsharedComposeYjsPeers()
    const accessChanged = applied.some(op => op.kind.endsWith('.delete') || op.kind === 'record.purge' || op.kind.endsWith('.share_revoked')
      || /^(module|playbook|compose)\./.test(op.kind))
    if (accessChanged) {
      for (const session of live.values()) session.editing = liveModuleEditLeases(session.editing)
        .filter(target => store.contentAccess.mayReadEditTarget(target, session.memberId, session.role))
      schedulePresenceFanoutBroadcast()
    }
    // Absence of a sender-supplied audience does not mean the item is public.
    // The same bucketed, serialize-once fanout enforces current grants for all.
    sendColleagueScopedOps(ws, frameId, applied)
  }
}

function currentFieldAclBaseline(): { revision: number; hash: string } {
  const authority = store.fieldAcl.snapshot()
  if (!authority) throw new Error('Field access authority unavailable')
  return { revision: authority.revision, hash: authority.hash }
}
...
/** Identity selectors are refused when malformed; they are never truncated. */
function exactMemberDeviceId(raw: unknown): string {
  if (typeof raw !== 'string') return ''
  const value = raw.trim()
  if (
    !value
    || value.length > LIST_MEMBER_DEVICE_ID_CAP
    || value.includes('\0')
    || !value.isWellFormed()
  ) return ''
  return value
...
function withContentAuthority(op: ModulesSyncOp): ModulesSyncOp {
  op = store.fieldAcl.currentForRead(canonicalContentOp(op))
  if(op.kind.startsWith('record.')&&!op.kind.endsWith('.delete')&&op.kind!=='record.purge'){
    const state=store.recordTeamwork.read(op.targetId)
    if(state.revision>0)op={...op,teamwork:{...op.teamwork,state}}
  }
  const { contentAclItem: _claimedItem, contentAclRevision: _authoredRevision, ...rest } = op
  const item = store.contentAccess.itemKey(op)
  // Lifecycle notices reach former recipients after the content grant ends.
  // Rebuild their envelope so a snapshot/extra client field cannot hitchhike.
  const lifecycle = op.kind.endsWith('.share_revoked') || op.kind.endsWith('.delete') || op.kind === 'record.purge'
  const safe = lifecycle ? {
    opId: op.opId, kind: op.kind, targetKind: op.targetKind, targetId: op.targetId,
    moduleId: op.moduleId, entityId: op.entityId, teamId: op.teamId, team_id: op.team_id,
    originMemberId: op.originMemberId, originDevice: op.originDevice, originRole: op.originRole,
    hlc: op.hlc, protocolVersion: op.protocolVersion, hopCount: op.hopCount,
    ...(opVisibleToMemberIds(op) !== undefined ? { visibleToMemberIds: opVisibleToMemberIds(op) } : {}),
    patch: { id: op.targetId, ...(op.kind.startsWith('module.') ? { moduleId: op.targetId } : {}),
      ...(op.patch?.authoritativeDelete === true ? { authoritativeDelete: true } : {}) },
  } : rest
  return item ? { ...safe, teamId: boundTeamId(), team_id: boundTeamId(), contentAclItem: item, contentAclRevision: store.contentAccess.revision(op) } : safe
}
...
function catchUpOpNeeded(
  ...
  if (!restoreReplay && store.hasAcked(deviceId, opId, peerSession.memberId)) return false
  if (!restoreReplay && op.originDevice === deviceId && op.originMemberId === peerSession.memberId) return false
...
function sendCatchUpChunk(
  ...
  const currentSession = live.get(ws)
  if (!currentSession) return false
  // A role/ACL may change while a prior chunk yielded to the event loop.
  // Recheck this chunk at the actual send boundary, not only scan time.
  const visible = chunk.flatMap(op => {
    if (!catchUpOpNeeded(op, deviceId, currentSession, socketRestoreReplay.has(ws))) return []
    const projected = projectScopedOp(currentSession, op)
    return projected ? [withContentAuthority(projected)] : []
  })
  if (visible.length === 0) {
    const receipt = socketCatchUpFrame.get(ws)
    if (receipt) receipt.processed = true
    return true
  }
  const ok = reply(ws, {
    type: 'ops',
    frameId: `catchup-${deviceId.slice(0, 24)}-${generation}-${offset}`,
    ops: visible,
  })
  if (!ok) {
    try {
...
function sendCatchUpStatus(
  ...
  if (!ok) forceCloseBackpressured(ws, 'catchup status backpressure')
  else if (extra?.done === true && !truncated) socketRestoreReplay.delete(ws)
}

// Keep catch-up chunks small enough for the desktop's bounded durable queue to
// drain between heartbeats. Socket backpressure must pace replay, not repeatedly
// discard the in-flight prefix of a cold client's history.
const CATCHUP_CHUNK_COUNT = Math.min(MAX_OPS_PER_FRAME, 32)
const CATCHUP_CHUNK_BYTES = Math.min(OPS_FRAME_MAX_BYTES, 256 * 1024)
async function waitCatchUpWritable(ws: WebSocket, generation: number): Promise<boolean> {
  const deadline = Date.now() + 60_000
  while (ws.bufferedAmount > Math.min(128 * 1024, FANOUT_BACKPRESSURE_BYTES / 4)) {
    if (!live.has(ws) || ws.readyState !== 1 || socketCatchUpGeneration.get(ws) !== generation) return false
    if (Date.now() >= deadline) {
      forceCloseBackpressured(ws, 'catchup consumer stalled')
      return false
    }
    await new Promise<void>(resolve => setTimeout(resolve, 20))
  }
  return live.has(ws) && ws.readyState === 1 && socketCatchUpGeneration.get(ws) === generation
}

/** New desktops use one durable catch-up frame in flight. Older peers retain streaming. */
async function sendPacedCatchUpChunk(ws: WebSocket, deviceId: string, offset: number, chunk: ModulesSyncOp[], generation: number): Promise<boolean> {
  if (!await waitCatchUpWritable(ws, generation)) return false
  const windowed = socketCatchUpAckWindow.has(ws)
  const receipt = { id: `catchup-${deviceId.slice(0, 24)}-${generation}-${offset}`, processed: false }
  if (windowed) socketCatchUpFrame.set(ws, receipt)
  const session = live.get(ws)
  if (!session) return false
  const needsDelivery = chunk.some(op => catchUpOpNeeded(op, deviceId, session, socketRestoreReplay.has(ws)))
  if (!sendCatchUpChunk(ws, deviceId, offset, chunk, generation)) return false
  if (!windowed || !needsDelivery) return true
  const deadline = Date.now() + 120_000
  // A processed frame grants flow credit only. Durable op acknowledgements
  // remain separate, so refused operations are still replayed on reconnect.
  while (!receipt.processed) {
    if (live.get(ws) !== session || ws.readyState !== 1 || socketCatchUpGeneration.get(ws) !== generation) return false
    if (Date.now() >= deadline) {
      forceCloseBackpressured(ws, 'catchup acknowledgement stalled')
      return false
    }
    await new Promise<void>(resolve => setTimeout(resolve, 20))
  }
...
async function sendFullCatchUpOps(
  ...
  // skip prefix ops.
  const recovery = socketRestoreReplay.has(ws)
  for await (const op of recovery ? store.scanRestoreHistory() : store.scanOpsFromStart()) {
    if (!live.has(ws)) return
    if (socketCatchUpGeneration.get(ws) !== generation) return
    if (!catchUpOpNeeded(op, deviceId, peerSession, socketRestoreReplay.has(ws))) continue
    const encoded = encodedOpByteLength(withContentAuthority(op))
    ...
    if (chunk.length > 0 && (chunk.length >= CATCHUP_CHUNK_COUNT || chunkBytes + add > CATCHUP_CHUNK_BYTES)) {
      if (!await sendPacedCatchUpChunk(ws, deviceId, sent, chunk, generation)) return
      sent += chunk.length
      chunk = []
      chunkBytes = 2
...
async function sendCatchUpOps(ws: WebSocket, deviceId: string, generation?: number): Promise<void> {
  try {
    ...
    if (!peerSession || !sessionStillAuthorized(ws, peerSession)) return
    if (socketRestoreReplay.has(ws) || store.deviceAckCount(deviceId, peerSession.memberId) === 0) {
    ...
    const recentIds = new Set(recent.map(op => op.opId))
    const obligations = store.contentAccess.cleanupNotices().filter(op => !recentIds.has(op.opId))
    // Keep original envelopes until the final send boundary: projecting here
    // would lose the original composite ownership proof across an await.
    const unacked = [...obligations, ...recent].filter((op) => catchUpOpNeeded(op, deviceId, peerSession, socketRestoreReplay.has(ws)))
    ...
    if (windowSaturated) {
      for (const op of recent) {
        const opId = typeof op.opId === 'string' ? op.opId : ''
        if (!opId) continue
        if (op.originDevice === deviceId && op.originMemberId === peerSession.memberId) continue
        if (peerSession && !peerMayReceiveScopedOp(peerSession, op)) continue
        oldestRelevantUnacked = !store.hasAcked(deviceId, opId, peerSession.memberId)
        break
      }
    }
    ...
    for (const chunk of frames) {
      if (!live.has(ws)) return
      if (generation !== undefined && socketCatchUpGeneration.get(ws) !== gen) return
      if (!await sendPacedCatchUpChunk(ws, deviceId, sent, chunk, gen)) return
      sent += chunk.length
    ...
    if (!live.has(ws)) return
    if (generation !== undefined && socketCatchUpGeneration.get(ws) !== gen) return
    sendCatchUpStatus(ws, deviceId, oversized.length > 0, { sent, done: true })
  } catch {
    if (live.has(ws) && socketRestoreReplay.has(ws)) {
      sendCatchUpStatus(ws,deviceId,true,{sent:0,done:true})
      reply(ws,{type:'error',message:'Restore history is incomplete. Original files are preserved; use a verified current server recovery copy.'})
      return
    }
    // A failed scan is incomplete, including a bounded slow-consumer refusal.
    // Force a fresh hello instead of leaving an apparently synced socket idle.
    if (live.has(ws)) forceCloseBackpressured(ws, 'catchup incomplete')
  }
}
...
function handleMessage(ws: WebSocket, data: Buffer | ArrayBuffer | Buffer[], connIp: string): void {
  ...
  // A socket is a transport, not authority. A credential may have rotated
  // while an asynchronous redeem or request was completing on another socket.
  if (session && !sessionStillAuthorized(ws, session)) {
    dropLiveSession(ws)
    reply(ws, { type: 'error', message: 'Session is no longer authorized' })
    try { ws.close(4004, 'session revoked') } catch { /* */ }
    return
  }
  ...
  const chatIdentityError = chatFrameIdentityError(frame)
  if (chatIdentityError) {
    reply(ws, { type: type.startsWith('ephemeral_') ? 'ephemeral_refuse' : 'chat_refuse',
      frameId: isFiniteRequestId(frame.frameId) ? frame.frameId : 'missing-frame', reason: chatIdentityError })
    return
  }
  try {
    switch (type) {
      case 'hello': {
        ...
        // Hello identity fields are selectors too. Never truncate an
        // over-cap value into an existing member/device identity.
        const memberId = exactMemberDeviceId(frame.memberId)
        const deviceId = exactMemberDeviceId(frame.deviceId)
        ...
        const helloOk = reply(ws, {
          type: 'hello_ok',
          catchupAckWindow: frame.catchupAckWindow === true,
          restoreReplay: frame.restoreReplay === true,
          ...
          chatCaps,
          teamContentLock: teamContentLockStore.get(team.teamId),
          fieldAclBaseline: currentFieldAclBaseline(),
          ...(liveMember.role === 'admin' ? { fieldAclAuthority: store.fieldAcl.snapshot() } : {}),
        })
        ...
        if (frame.catchupAckWindow === true) socketCatchUpAckWindow.add(ws)
        else socketCatchUpAckWindow.delete(ws)
        if (frame.restoreReplay === true) socketRestoreReplay.add(ws)
        else socketRestoreReplay.delete(ws)
        pushPresenceSnapshot(ws)
        startCatchUpOps(ws, deviceId)
        return
      }
      case 'catchup_processed': {
        if (!session || !socketCatchUpAckWindow.has(ws)) return
        const receipt = socketCatchUpFrame.get(ws)
        if (receipt && frame.frameId === receipt.id) receipt.processed = true
        return
      }
      case 'invite_create': {
      ...
      case 'invite_redeem': {
        ...
        const deviceId = exactMemberDeviceId(frame.deviceId)
        if (frame.redemptionNonce !== undefined && (typeof frame.redemptionNonce !== 'string' || !/^[a-f0-9]{64}$/.test(frame.redemptionNonce))) {
          reply(ws, { type: 'error', requestId: frameId, message: 'Invalid invitation attempt proof' })
          return
        }
        ...
        void store.redeemInvite({
          token,
          deviceId,
          redemptionNonce: frame.redemptionNonce as string | undefined,
          memberEmail: typeof frame.memberEmail === 'string' ? frame.memberEmail : undefined,
          displayName: typeof frame.displayName === 'string' ? frame.displayName : undefined,
        }).then((redeemed) => {
          ...
          const current = store.findBySession(redeemed.sessionToken)
          if (!current || current.member.memberId !== redeemed.member.memberId || current.deviceId !== deviceId) {
            reply(ws, { type: 'error', requestId: frameId, message: 'Invitation session was superseded. Join again.' })
            return
          }
          // Close only revoked/replaced bearers. Same-role additional devices stay live.
          for (const [peerWs, peer] of live) {
            if (!redeemed.replayed && peer.memberId === redeemed.member.memberId && peerWs !== ws
              && !store.findBySession(peer.sessionToken)) {
              quietDropLiveSession(peerWs)
              try { peerWs.close(4001, 'session rotated') } catch { /* */ }
            }
          }
      ...
      case 'team_content_lock_get': {
        ...
        reply(ws, {
          type: 'team_content_lock_state',
          frameId,
          state: teamContentLockStore.get(boundTeamId()),
        })
        return
      }
      case 'team_content_lock_set': {
        const frameId = isFiniteRequestId(frame.frameId) ? frame.frameId : 'missing-frame'
        if (!requireAdmin(session)) {
          reply(ws, {
            type: 'team_content_lock_refuse',
            frameId,
            reason: 'Admin only',
            state: teamContentLockStore.get(boundTeamId()),
          })
        ...
        if (typeof frame.locked !== 'boolean') {
          reply(ws, {
            type: 'team_content_lock_refuse',
            frameId,
            reason: 'locked must be a boolean',
            state: teamContentLockStore.get(boundTeamId()),
          })
          return
        }
        const expectedRevision = typeof frame.expectedRevision === 'number'
          && Number.isSafeInteger(frame.expectedRevision)
          && frame.expectedRevision >= 0
          ? frame.expectedRevision
          : -1
        const result = teamContentLockStore.set({
          teamId: boundTeamId(),
          actorMemberId: session.memberId,
          actorName: session.displayName,
          actorRole: session.role,
          locked: frame.locked,
          // Keep wire validation strict. Number(null) and Number('0') both
          // become 0 and could otherwise pass the initial CAS epoch.
          expectedRevision,
          mutationId: typeof frame.mutationId === 'string' ? frame.mutationId : '',
        ...
        const requesterReceivedAuthority = reply(ws, {
          type: 'team_content_lock_state',
          frameId,
          state: result.state,
        ...
        if (!requesterReceivedAuthority) {
          // The mutation is already durable. Leaving the initiating socket
          // alive with its old authority would let its renderer continue to
          // present writes as locally admissible. Force a hello resync just
          // as we do for a peer that misses the lock fanout.
          forceCloseBackpressured(ws, 'team content lock response backpressure')
        }
        if (result.changed) {
          const peerText = serializeBridgeFrame({
            type: 'team_content_lock_state',
            state: result.state,
            changed: true,
          })
          for (const [peerWs] of live) {
            if (peerWs === ws || peerWs.readyState !== 1) continue
            if (!sendSerializedFrame(peerWs, peerText, FANOUT_BACKPRESSURE_BYTES)) {
              // Missing the lock transition can reopen writes on that client.
              // Close it so hello resynchronizes the authoritative snapshot.
              forceCloseBackpressured(peerWs, 'team content lock fanout backpressure')
            }
          }
          console.warn(
            `[bridge] team content ${result.state.locked ? 'locked' : 'unlocked'}`
              + ` by Admin ${logId(session.memberId)} at revision ${result.state.revision}`,
          )
        }
        return
      }
      case 'team_content_restore_begin': {
        const frameId = isFiniteRequestId(frame.frameId) ? frame.frameId : 'missing-frame'
        if (!requireAdmin(session)) {
          reply(ws, { type: 'team_content_restore_refuse', frameId, reason: 'Admin only', state: teamContentLockStore.get(boundTeamId()) })
          return
        }
        if (!consumeAdminMutateTokenOrSlowDown(ws, session.memberId, frameId)) {
          reply(ws, { type: 'team_content_restore_refuse', frameId, reason: 'Rate limited - slow down', state: teamContentLockStore.get(boundTeamId()) })
          return
        }
        const expectedRevision = typeof frame.expectedRevision === 'number'
          && Number.isSafeInteger(frame.expectedRevision)
          ...
          ? frame.expectedRevision
          : -1
        const result = teamContentLockStore.beginRestorePermit({
          teamId: boundTeamId(),
          actorMemberId: session.memberId,
        ...
        if (!result.ok) {
          reply(ws, { type: 'team_content_restore_refuse', frameId, reason: result.reason, state: result.state })
          return
        }
        reply(ws, {
          type: 'team_content_restore_permit', frameId,
          permitToken: result.permit.token,
          lockRevision: result.permit.lockRevision,
          expiresAt: result.permit.expiresAt,
        })
        console.warn(`[bridge] shared-content restore permit opened by Admin ${logId(session.memberId)} at lock revision ${result.permit.lockRevision}`)
        return
      }
      case 'team_content_restore_validate':
      case 'team_content_restore_finish': {
        const frameId = isFiniteRequestId(frame.frameId) ? frame.frameId : 'missing-frame'
        if (!requireAdmin(session)) {
          reply(ws, { type: 'team_content_restore_refuse', frameId, reason: 'Admin only', state: teamContentLockStore.get(boundTeamId()) })
          return
        ...
        const permitToken = typeof frame.permitToken === 'string' ? frame.permitToken : ''
        if (frame.type === 'team_content_restore_validate') {
          const result = teamContentLockStore.validateRestorePermit({
            teamId: boundTeamId(), actorMemberId: session.memberId,
            actorDeviceId: session.deviceId, token: permitToken,
          })
          if (!result.ok) {
            reply(ws, { type: 'team_content_restore_refuse', frameId, reason: result.reason, state: result.state })
            return
          ...
          reply(ws, {
            type: 'team_content_restore_permit', frameId, permitToken: '',
            lockRevision: result.lockRevision, expiresAt: result.expiresAt, validated: true,
          })
          return
        }
        const result = teamContentLockStore.finishRestorePermit({
          teamId: boundTeamId(), actorMemberId: session.memberId,
          actorDeviceId: session.deviceId, token: permitToken,
        })
        if (!result.ok) {
          reply(ws, { type: 'team_content_restore_refuse', frameId, reason: result.reason, state: teamContentLockStore.get(boundTeamId()) })
          return
        }
        reply(ws, {
          type: 'team_content_restore_permit', frameId, permitToken: '',
          lockRevision: teamContentLockStore.get(boundTeamId()).revision,
          expiresAt: 0, finished: true,
        })
        console.warn(`[bridge] shared-content restore permit released by Admin ${logId(session.memberId)}`)
        return
      }
      case 'kick_member': {
        const frameId = isFiniteRequestId(frame.frameId) ? frame.frameId : 'missing-frame'
        ...
        const targetId = exactMemberDeviceId(frame.memberId)
        ...
        const chatCleanupFailed = finalizeKickedMembers([targetId]).length > 0
        const composeAclCleanup = removeMembersFromComposeAuthority([targetId])
        reply(ws, {
          type: 'kick_ok',
          frameId,
          memberId: targetId,
          ...(chatCleanupFailed ? { chatCleanupFailed: true } : {}),
          ...(!composeAclCleanup.ok ? { contentAclCleanupFailed: true } : {}),
        })
        return
      }
      case 'kick_members': {
        ...
        if (!Array.isArray(frame.memberIds)) {
          reply(ws, { type: 'error', requestId: frameId, message: 'memberIds required' })
          return
        }
        if (frame.memberIds.length > TEAMSPACE_KICK_MEMBERS_MAX) {
          reply(ws, {
            type: 'error',
            requestId: frameId,
            message: `Too many members (max ${TEAMSPACE_KICK_MEMBERS_MAX})`,
          })
          return
        }
        const targetIds: string[] = []
        for (const raw of frame.memberIds) {
          if (typeof raw !== 'string') {
            reply(ws, {
              type: 'error',
              requestId: frameId,
              message: 'memberIds must contain strings',
            })
            return
          }
          targetIds.push(raw)
        }
        const kicked = store.kickMembers(session!.memberId, targetIds)
        if (!kicked.ok) {
          reply(ws, { type: 'error', requestId: frameId, message: kicked.reason })
          return
        }
        // Backups remain Admin-controlled, exactly like the single-member path.
        const kickedMemberIds = kicked.members.map((member) => member.memberId)
        const chatCleanupFailedMemberIds = finalizeKickedMembers(kickedMemberIds)
        const composeAclCleanup = removeMembersFromComposeAuthority(kickedMemberIds)
        const members = kicked.members.map((member) => {
          const allDeviceIds = [...new Set(
            member.kickedDeviceIds
              .map((deviceId) => capTrim(deviceId, LIST_MEMBER_DEVICE_ID_CAP))
              .filter(Boolean),
          )]
          const kickedDeviceIds = allDeviceIds.slice(
            0,
            TEAMSPACE_KICK_MEMBER_DEVICE_IDS_REPLY_MAX,
          )
          return {
            ...
            kickedDeviceIds,
            kickedDeviceIdsTotal: allDeviceIds.length,
            deviceIdsTruncated: kickedDeviceIds.length < allDeviceIds.length,
          }
        })
        reply(ws, {
          type: 'kick_members_ok',
          frameId,
          members,
          ...(chatCleanupFailedMemberIds.length > 0
            ? { chatCleanupFailedMemberIds }
            : {}),
          ...(!composeAclCleanup.ok
            ? { contentAclCleanupFailedMemberIds: kickedMemberIds }
            : {}),
        })
        return
      }
      case 'leave_team': {
        ...
        // Revoke in-memory authority before any fallible metadata cleanup. In
        // particular, a disk error in chat cleanup must not strand a removed
        // member's old socket in `live` with its former role still attached.
        const leavingSockets: WebSocket[] = []
        for (const [peerWs, peer] of [...live]) {
          if (peer.memberId === session.memberId) {
            leavingSockets.push(peerWs)
            dropLiveSession(peerWs)
          }
        }
        const composeAclCleanup = removeMembersFromComposeAuthority([session.memberId])
        ...
        try {
          ...
          console.error(
            `[bridge] departed member ${logId(session.memberId)} chat cleanup failed: ` +
              `${err instanceof Error ? capStr(err.message, 160) : 'error'}`,
          )
          const retryMemberId = session.memberId
          const timer = setTimeout(() => {
            try { handleMemberLeaveChat(retryMemberId) } catch { /* surfaced above */ }
          }, 1_000)
          timer.unref()
        }
        ...
        // TCC-R1126-BRG-001 + TCC-R1151-BRG-001: every device is already
        // sync-dropped above; close only after the current socket gets its ack.
        for (const peerWs of leavingSockets) {
      ...
      case 'revoke_session': {
        ...
        const targetId = exactMemberDeviceId(frame.memberId)
        const hasDeviceId = Object.prototype.hasOwnProperty.call(frame, 'deviceId')
        const deviceIdRaw = hasDeviceId ? exactMemberDeviceId(frame.deviceId) : ''
        if (!targetId || (hasDeviceId && !deviceIdRaw)) {
          reply(ws, { type: 'error', requestId: frameId, message: 'Invalid member or device id' })
          return
        }
      ...
      case 'set_role': {
        ...
        try { workspaceCoordinatorMonitor.invalidateAccess(undefined, targetId) } catch { console.error('[bridge] Coordinator consent invalidation failed after role change') }
        for (const peer of live.values()) {
          if (peer.memberId === targetId) {
            peer.role = set.role
            if (set.role === 'viewer') peer.editing = []
          }
        }
        schedulePresenceFanoutBroadcast()
        ...
        for (const [peerWs, peer] of live.entries()) {
          ...
          if (peerWs === ws) continue
          if (peerWs.readyState !== 1) continue
          if (!sendSerializedFrame(peerWs, text, FANOUT_BACKPRESSURE_BYTES)) {
            // Missing a role downgrade leaves local authority stale. Force a
            // hello refresh instead of silently keeping the old role.
            forceCloseBackpressured(peerWs, 'role update backpressure')
          }
        }
        return
      }
      ...
      case 'ack_ops': {
        ...
        const opIds = Array.isArray(frame.opIds)
          ...
          : []
        store.markAcked(session.deviceId, opIds, session.memberId)
      ...
      case 'ping': {
        ...
        reply(ws, { type: 'pong', t, serverTimeMs: Date.now() })
        return
      }
      case 'yjs_join': {
        const frameId = isFiniteRequestId(frame.frameId) ? frame.frameId : 'missing-frame'
        ...
        // Record document rooms obey the owning Module's current grant too.
        if (!peerYjsRoomStillAuthorized(session, parsed.recordId, parsed.contentField)) {
          reply(ws, {
            type: 'yjs_refuse',
            frameId,
            room: parsed.room,
        ...
        const acceptedStateB64 = !isYjsComposeRecordId(parsed.recordId) ? store.recordTeamwork.acceptedYjsState(parsed.recordId, parsed.contentField) : undefined
        reply(ws, { type: 'yjs_ok', frameId, room: parsed.room, action: 'join', ...(acceptedStateB64 ? {acceptedStateB64} : {}) })
        return
      }
      case 'yjs_leave': {
        const frameId = isFiniteRequestId(frame.frameId) ? frame.frameId : 'missing-frame'
      ...
      case 'yjs_update': {
        ...
        // Compose uses its own ACL and is outside this lock. Doc/Whiteboard
        // rooms are shared Module content and must obey the server lock.
        if (!isYjsComposeRecordId(parsed.recordId)) {
          const contentLock = teamContentLockStore.get(boundTeamId())
          if (teamContentLockBlocks(contentLock, 'module_yjs')) {
            reply(ws, {
              type: 'yjs_refuse',
              frameId,
              room: parsed.room,
              reason: contentLock.unavailable
                ? 'Team content lock is unavailable on the server'
                : 'Team content is locked by an Admin',
              teamId: contentLock.teamId,
            })
            return
          }
          const lockRevisionRefusal = teamContentLockRevisionRefusal(contentLock, frame.lockRevision)
          if (lockRevisionRefusal) {
            reply(ws, {
              type: 'yjs_refuse',
              frameId,
              room: parsed.room,
              reason: lockRevisionRefusal === 'missing'
                ? 'Update the app before editing shared documents after a team lock'
                : 'Team content lock revision changed; discard this document update',
              teamId: contentLock.teamId,
              permanent: true,
            })
            return
        ...
        const composeAclRefusal = composeYjsAccessRefusal(
          session,
          parsed.recordId,
          parsed.contentField,
          true,
        )
        if (composeAclRefusal) {
          leaveYjsRoom(ws, parsed.room)
          reply(ws, {
            type: 'yjs_refuse',
            frameId,
            room: parsed.room,
            reason: composeAclRefusal,
          })
          return
        }
        ...
        if (!isYjsComposeRecordId(parsed.recordId)) {
          try { store.recordTeamwork.acceptYjs(parsed.recordId, parsed.contentField, frame.updateB64) }
          catch (error) {
            reply(ws, {type:'yjs_refuse',frameId,room:parsed.room,reason:error instanceof Error?error.message:'Document persistence is unavailable'})
            return
          }
        }
      ...
      case 'presence_edit': {
        ...
        if (!consumeOpsTokenOrSlowDown(ws, session, frameId)) {
          reply(ws, { type: 'error', requestId: frameId, message: 'Rate limited' })
          return
        }
        const target = parseModuleEditTarget(frame.target)
        if (!target || typeof frame.active !== 'boolean' || (frame.active && session.role === 'viewer')) {
          reply(ws, { type: 'error', requestId: frameId, message: 'Invalid editing presence' })
          return
        }
        if (frame.active && !store.contentAccess.mayReadEditTarget(target, session.memberId, session.role)) {
          reply(ws, { type: 'error', requestId: frameId, message: 'Item access was revoked or not granted' })
          return
        }
        try {
          session.editing = updateModuleEditLeases(session.editing, target, frame.active)
          session.lastSeen = Date.now()
          pushPresenceSnapshot(ws, frameId)
          schedulePresenceFanoutBroadcast()
        } catch (err) {
          reply(ws, { type: 'error', requestId: frameId, message: err instanceof Error ? err.message : 'Presence refused' })
        }
        return
      }
      case 'presence_get': {
      ...
      case 'chat_send': {
        ...
        void chatStore.append({
          ...
          authorize: chatCommitGuard(ws, session, roomParsed.room),
        }).then((appended) => {
          // A disconnect after commit only prevents replying/issuing another
          // command as this sender. It cannot suppress the durable room event.
          const senderAuthorized = sessionStillAuthorized(ws, session)
          if ('error' in appended) {
            if (!senderAuthorized) return
            reply(ws, {
              type: 'chat_refuse',
              frameId,
              room: roomParsed.room,
          ...
          if (senderAuthorized) reply(ws, { type: 'chat_ok', frameId, message })
      ...
      case 'ephemeral_start': {
        ...
        const targetMemberId = exactChatInviteIdentity(frame.targetMemberId)
      ...
      case 'ephemeral_accept': {
        ...
        maybeHintLimiterApproaching(ws, ephemeralLimiter, `ephstart:${session.memberId}`, EPHEMERAL_START_TOKENS_PER_MIN, 60_000, frameId)
        const inviteId = exactChatInviteIdentity(frame.inviteId)
        ...
        if (invite.expiresAt <= Date.now() || !store.findMember(invite.fromMemberId) || !firstLiveSessionForMember(invite.fromMemberId)) {
        ...
        if (ephemeralRooms.size >= EPHEMERAL_ROOMS_TOTAL_MAX || [invite.fromMemberId, invite.toMemberId].some(
          id => (ephemeralRoomsByMember.get(id)?.size || 0) >= EPHEMERAL_ROOMS_PER_MEMBER_MAX,
        )) {
          expireEphemeralInvite(inviteId)
          reply(ws, { type: 'ephemeral_refuse', frameId, reason: 'Temporary chat capacity changed before acceptance - try again later' })
          return
        }
        expireEphemeralInvite(inviteId)
      ...
      case 'ephemeral_group_start': {
        ...
        maybeHintLimiterApproaching(ws, ephemeralLimiter, `ephstart:${session.memberId}`, EPHEMERAL_START_TOKENS_PER_MIN, 60_000, frameId)
        if (!Array.isArray(frame.targetMemberIds) || frame.targetMemberIds.some(raw => !exactChatInviteIdentity(raw))) {
          reply(ws, { type: 'ephemeral_refuse', frameId, reason: 'Invalid temporary group chat member' })
          return
        }
      ...
      case 'ephemeral_group_accept': {
        ...
        maybeHintLimiterApproaching(ws, ephemeralLimiter, `ephstart:${session.memberId}`, EPHEMERAL_START_TOKENS_PER_MIN, 60_000, frameId)
        const formationId = exactChatInviteIdentity(frame.formationId)
        ...
        if (formation.createdAt + EPHEMERAL_INVITE_TTL_MS <= Date.now()) {
          cancelEphemeralGroupFormation(formationId, 'expired')
          reply(ws, { type: 'ephemeral_refuse', frameId, reason: 'That temporary group chat invite expired' })
          return
        }
        const unavailableMember = formation.allMemberIds.find(id => !store.findMember(id) || !firstLiveSessionForMember(id))
        if (unavailableMember) {
          cancelEphemeralGroupFormation(formationId, 'member_offline', unavailableMember)
          reply(ws, { type: 'ephemeral_refuse', frameId, reason: 'Someone in this temporary group chat is no longer available' })
          return
        }
        formation.pendingMemberIds.delete(session.memberId)
        formation.acceptedMemberIds.add(session.memberId)
        // Keep accepted invitees indexed too: if they leave the team or go
        // offline before the final acceptance, the formation must cancel.
        ...
        ephemeralGroupFormations.delete(formationId)
        for (const id of formation.allMemberIds) {
          const inviteeSet = ephemeralGroupInviteesByMember.get(id)
          inviteeSet?.delete(formationId)
          if (inviteeSet?.size === 0) ephemeralGroupInviteesByMember.delete(id)
        }
      ...
      case 'chat_delete': {
        ...
        void chatStore.softDelete(messageId, session.memberId, roomHint, chatCommitGuard(ws, session, roomHint, true)).then((del) => {
          const senderAuthorized = sessionStillAuthorized(ws, session)
          if ('error' in del) {
            if (!senderAuthorized) return
          ...
          if (senderAuthorized) reply(ws, { type: 'chat_delete_ok', frameId, messageId })
      ...
      case 'chat_rooms_list': {
        ...
        void roomsListProcessPool.run(() => mapWithConcurrency(baseRooms, CHAT_ROOMS_LIST_SCAN_CONCURRENCY, async (r) => {
            ...
            try {
              const summary = await chatStore.readUnreadSummary(r.id, memberId, wm)
              unread = summary.unread
              unreadTruncated = summary.unreadTruncated
      ...
      case 'chat_room_create': {
        ...
        if (kind === 'dm') {
          const target = exactChatInviteIdentity(frame.targetMemberId)
          if (!target) {
        ...
        if (frame.memberIds !== undefined && (!Array.isArray(frame.memberIds) || frame.memberIds.some((id) => !exactChatInviteIdentity(id)))) {
          reply(ws, { type: 'chat_refuse', frameId, reason: 'Invalid member IDs' })
          return
        }
        const memberIds = Array.isArray(frame.memberIds) ? frame.memberIds as string[] : []
      ...
      case 'chat_room_join': {
        ...
        if (!roomParsed.ok || roomParsed.room !== frame.room) {
          reply(ws, { type: 'chat_refuse', frameId, reason: !roomParsed.ok ? roomParsed.reason : 'Invalid room identity' })
          return
        }
        ...
        const inviteToken = frame.inviteToken === undefined || frame.inviteToken === '' ? '' : exactChatInviteIdentity(frame.inviteToken)
        if (frame.inviteToken !== undefined && frame.inviteToken !== '' && !inviteToken) {
          reply(ws, { type: 'chat_refuse', frameId, reason: 'Invalid invite token' })
          return
        }
        if (inviteToken) {
          const redeemed = chatRooms.redeemInvite(inviteToken, session.memberId, roomParsed.room)
        ...
        if (row.kind === 'private' && !row.memberIds.includes(session.memberId)) {
          if (!row.passwordHash) {
            reply(ws, { type: 'chat_refuse', frameId, reason: 'A private room invite is required' })
            return
          }
      ...
      case 'chat_room_add_members': {
        ...
        if (!Array.isArray(frame.memberIds) || frame.memberIds.length > 50 || frame.memberIds.some((id) => !exactChatInviteIdentity(id))) {
          reply(ws, { type: 'chat_refuse', frameId, reason: 'Provide up to 50 exact member IDs' })
          return
        }
        const memberIds = frame.memberIds as string[]
        if (memberIds.some((id) => !store.findMember(id))) {
          reply(ws, { type: 'chat_refuse', frameId, reason: 'Member not found' })
          return
        }
        const added = chatRooms.addMembers(frame.room as string, memberIds)
        if ('error' in added) {
      ...
      case 'chat_room_remove_members': {
        ...
        const memberIds = frame.memberIds as string[]
        const removed = chatRooms.removeMembers(frame.room as string, memberIds, session.memberId)
        if ('error' in removed) {
      ...
      case 'chat_room_promote_owner': {
        ...
        const memberId = exactChatInviteIdentity(frame.memberId)
        const promoted = chatRooms.promoteOwner(frame.room as string, memberId)
      ...
      case 'chat_room_demote_owner': {
        ...
        const demoted = chatRooms.demoteOwner(frame.room as string, memberId, {
      ...
      case 'chat_room_ban_member': {
        ...
        const memberId = exactChatInviteIdentity(frame.memberId)
        const banned = chatRooms.banMember(frame.room as string, memberId, session.memberId, {
      ...
      case 'chat_room_unban_member': {
        ...
        const memberId = exactChatInviteIdentity(frame.memberId)
        const unbanned = chatRooms.unbanMember(frame.room as string, memberId)
      ...
      case 'chat_room_rename': {
        ...
        // Shared names are administrative; the photo/description editInfo
        // preference cannot grant ordinary members permission to rename.
        const renameGateRow = chatRooms.get(roomParsed.room)
        if (
          !renameGateRow
          || !canManageRoom(renameGateRow, session.memberId, session.role)
        ) {
          reply(ws, {
            type: 'chat_refuse',
            frameId,
      ...
      case 'chat_unread_get': {
        ...
        reply(ws, {
          type: 'chat_unread_ok',
          frameId,
          marks: Object.fromEntries(Object.entries(chatUnread.getAllForMember(session.memberId))
            .filter(([room]) => chatRooms.memberCanAccess(room, session.memberId))),
        })
        return
      }
      case 'chat_unread_set': {
        ...
        if (set.advanced) {
          const committed = chatUnread.get(session.memberId, roomParsed.room)
          if (committed) fanoutChatSeenPeer(ws, roomParsed.room, session.memberId, committed.lastReadAt, committed.lastReadMsgId ?? null)
        }
        return
      }
      case 'chat_seen_get': {
      ...
      case 'chat_edit': {
        ...
        void chatStore.findById(messageId, roomHint).then((prev) => {
          ...
          return chatStore.edit(
            ...
            roomHint,
            editWindowMs,
            chatCommitGuard(ws, session, prev?.room),
          ).then((edited) => {
            const senderAuthorized = sessionStillAuthorized(ws, session)
            if ('error' in edited) {
              if (!senderAuthorized) return
            ...
            if (senderAuthorized) reply(ws, { type: 'chat_edit_ok', frameId, message })
      ...
      case 'chat_react': {
        ...
        void chatStore.findById(messageId, roomHint).then((prev) => {
          ...
          const canCommitReaction = chatCommitGuard(ws, session, prev.room)
          return chatStore.react(
            ...
            frame.remove === true,
            roomHint,
            () => canCommitReaction() && (frame.remove === true || (() => {
              const currentRoom = chatRooms.get(prev.room)
              return !!currentRoom && isReactionEmojiAllowed(currentRoom, emoji)
            })()),
          ).then((reacted) => {
            const senderAuthorized = sessionStillAuthorized(ws, session)
            if ('error' in reacted) {
            ...
            if (senderAuthorized) reply(ws, { type: 'chat_react_ok', frameId, message })
      ...
      case 'chat_pin': {
        ...
        const pinWrite = wantPin && namedId
          ? chatStore.pinMessage(roomParsed.room, namedId, true, chatCommitGuard(ws, session, roomParsed.room, true, true))
          : chatStore.unpinMessage(roomParsed.room, namedId, true, chatCommitGuard(ws, session, roomParsed.room, true, true))
      ...
      case 'chat_export': {
        ...
        void chatStore.exportRoom(roomParsed.room, format, { maxBodyBytes: 8_000_000 }).then((res) => {
          ...
          // Disk reads may outlive a re-hello, role change, or device revocation.
          if (!sessionStillAuthorized(ws, session) || session.role !== 'admin') return
          if ('error' in res) {
...
/** Revalidate the same bearer and principal after a body read or queue wait. */
function requestStillAuthorized(
  req: IncomingMessage,
  admitted: NonNullable<ReturnType<BridgeStore['findBySession']>>,
  role: 'admin' | 'writer',
): boolean {
  const current = authFromReq(req)
  return !!current && current.member.memberId === admitted.member.memberId
    && current.deviceId === admitted.deviceId
    && (role === 'admin' ? current.member.role === 'admin' : current.member.role !== 'viewer')
}

/** Revalidate the exact bearer principal without imposing a role floor. */
function requestStillSameSession(
  req: IncomingMessage,
  admitted: NonNullable<ReturnType<BridgeStore['findBySession']>>,
): boolean {
  const current = authFromReq(req, { allowQueryToken: false })
  return !!current && current.member.memberId === admitted.member.memberId
    && current.deviceId === admitted.deviceId
}

function requireCurrentRequestAdmin(
  req: IncomingMessage,
  res: ServerResponse,
  admitted: NonNullable<ReturnType<BridgeStore['findBySession']>>,
): boolean {
  if (requestStillAuthorized(req, admitted, 'admin')) return true
  sendJson(res, 401, { ok: false, error: 'Admin session is no longer authorized' })
  return false
}
...
const mailIdentityExists = (identity: import('./mail-oauth-service.js').MailIdentity) => {
  const member = store.findMember(identity.memberId)
  return identity.teamId === boundTeamId() && !!member && member.role !== 'viewer'
    && Object.prototype.hasOwnProperty.call(member.sessions, identity.deviceId)
}
mailService?.startWorkers(mailIdentityExists)
const handleMailHttp = createMailHttpHandler({
  service: mailService,
  authenticate: req => authFromReq(req, { allowQueryToken: false }),
  teamId: boundTeamId,
  identityExists: mailIdentityExists,
  readBody: readJsonBody,
  releaseBody: releaseJsonBody,
  json: sendJson,
  drain: drainRequestBody,
})

// Existing-member profiles are independent of legacy Workspace Work registrations.
const handleTeamMemberProfileHttp = createTeamMemberProfileHttpHandler({
  store: new TeamMemberProfileStore(AUTHORITY_DIR, atRestKey, id => store.findMember(id) ?? null),
  teamId: boundTeamId,
  authenticate: req => authFromReq(req, { allowQueryToken: false }),
  readBody: readJsonBody, releaseBody: releaseJsonBody, json: sendJson, drain: drainRequestBody,
  takeWrite: memberProfileHttpBudget.take,
  retryAfterSeconds: memberProfileHttpBudget.retryAfterSeconds,
})

// Explicitly associated existing Office objects, independent of legacy directories.
const officeObjectStore = new OfficeObjectStore(AUTHORITY_DIR, atRestKey, boundTeamId, id => store.findMember(id) ?? null)
const handleOfficeObjectsHttp = createOfficeObjectsHttpHandler({ store: officeObjectStore, authenticate: req => authFromReq(req, { allowQueryToken: false }), readBody: readJsonBody, releaseBody: releaseJsonBody, json: sendJson, drain: drainRequestBody })

const assignmentRefreshRequests = new Map<string,number>()
const recordTeamworkHttpDeps: RecordTeamworkHttpDeps = {
  store, teamId: boundTeamId, authenticate: req => authFromReq(req, { allowQueryToken: false }),
  departmentExists: (teamId,id) => officeObjectStore.hasDepartment(teamId,id),
  readBody: readJsonBody, releaseBody: releaseJsonBody, json: sendJson, drain: drainRequestBody,
  takeWrite: recordTeamworkHttpBudget.take,
  retryAfterSeconds: recordTeamworkHttpBudget.retryAfterSeconds,
  publish: ops => sendColleagueScopedOps(null, 'record-teamwork', ops),
  requestAssignmentSnapshot: request => {
    const now=Date.now(),key=JSON.stringify([request.recordId,request.fieldId])
    for(const [id,at] of assignmentRefreshRequests)if(now-at>30000)assignmentRefreshRequests.delete(id)
    if(assignmentRefreshRequests.has(key)||assignmentRefreshRequests.size>=256)return
    assignmentRefreshRequests.set(key,now)
    for(const [ws,session] of live){
      if(!sessionStillAuthorized(ws,session))continue
      const member=store.findMember(session.memberId)
      if(member?.role==='admin'&&store.contentAccess.mayReadRecord(request.recordId,member.memberId,member.role))reply(ws,{type:'assignment_snapshot_request',...request})
    }
  },
  fieldRefusal: (op, member) => currentFieldAccess().writeRefusal(op, member.role, store.contentAccess),
  canRead: (op, member) => !!currentFieldAccess().project(op, member.role, store.contentAccess),
  assertWritable: () => {if(teamContentLockBlocks(teamContentLockStore.get(boundTeamId()),'modules_ops'))throw new Error('Team content is locked by an Admin')},
}
const handleRecordTeamworkHttp = createRecordTeamworkHttpHandler(recordTeamworkHttpDeps)
const handleRecordTreeHttp = createRecordTreeHttpHandler(recordTeamworkHttpDeps)

const handleWorkspaceWorkHttp = createWorkspaceWorkHttpHandler({
  store: workspaceWorkStore,
  coordinator: workspaceCoordinatorMonitor,
  authenticate: req => authFromReq(req, { allowQueryToken: false }),
  members: () => store.listMembers().map(member => ({ memberId: member.memberId, displayName: member.displayName, role: member.role })),
  readBody: readJsonBody,
  releaseBody: releaseJsonBody,
  json: sendJson,
  drain: drainRequestBody,
...
async function handleHttp(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (path.startsWith('/v1/mail/')) {
    if (!mailHttpLimiter.take(`mail-http:${ip}`, MAIL_HTTP_TOKENS_PER_WINDOW, OPS_FRAME_WINDOW_MS)) {
      drainRequestBody(req)
      sendRateLimited(res, limiterPace(mailHttpLimiter, `mail-http:${ip}`, MAIL_HTTP_TOKENS_PER_WINDOW, OPS_FRAME_WINDOW_MS).waitMs, 'Mailbox traffic rate limited - retry shortly')
      return
    }
  } else if (req.method !== 'HEAD') {
    // Resolve a real current session before assigning a member budget. Never
    // key by a supplied token or member-id header: forged identities must keep
    // sharing the anonymous IP budget. A busy office shares an IP, not a quota.
    const memberId = authFromReq(req, { allowQueryToken: false })?.member.memberId
    const httpKey = memberId ? `http:member:${memberId}` : `http:ip:${ip}`
    const httpTokens = memberId ? AUTH_HTTP_TOKENS_PER_WINDOW : HTTP_TOKENS_PER_WINDOW
    if (!takeHttpToken(httpKey, httpTokens)) {
      drainRequestBody(req)
      sendRateLimited(res, limiterPace(httpLimiter, httpKey, httpTokens, OPS_FRAME_WINDOW_MS).waitMs, 'Rate limited - slow down')
      return
    }
  }

  if (await handleMailHttp(req, res, url)) return
  if (await handleTeamMemberProfileHttp(req, res, url)) return
  if (await handleRecordTeamworkHttp(req, res, url)) return
  if (await handleRecordTreeHttp(req, res, url)) return
  if (await handleOfficeObjectsHttp(req, res, url)) return
  if (await handleWorkspaceWorkHttp(req, res, url)) return
  ...
  if (req.method === 'POST' && path === '/v1/chat/avatars') {
    ...
    try {
      if (!requestStillSameSession(req, auth)) {
        sendJson(res, 401, { ok: false, error: 'Session is no longer authorized' })
        return
      }
  ...
  if (req.method === 'POST' && path === '/v1/chat/blobs') {
    ...
    try {
      const uploadStillAuthorized = () => (
        requestStillAuthorized(req, auth, 'writer')
        && chatRooms.memberCanAccess(roomParsed.room, auth.member.memberId)
      )
      // The request may have been held at the body boundary while an Admin
      // kicked/demoted the sender or removed them from this room.
      if (!uploadStillAuthorized()) {
        sendJson(res, 403, { ok: false, error: CHAT_BLOB_UPLOAD_AUTH_REVOKED })
        return
      }
      const registered = await chatBlobs.registerUpload({
        ...
        filename,
        bytes: body.bytes,
        authorize: uploadStillAuthorized,
      })
      if ('error' in registered) {
        sendJson(
          res,
          registered.error === CHAT_BLOB_UPLOAD_AUTH_REVOKED ? 403 : 400,
          { ok: false, error: registered.error },
        )
        return
      }
      sendJson(res, 200, {
  ...
  if (req.method === 'GET' && chatBlobGet) {
    ...
    // Old blobs must obey current scanning policy too. No bytes are exposed
    // until scanning finishes and the original request is still authorized.
    const safety = await runChatAttachmentScan(buf, row.name)
    const currentAuth = authFromReq(req, { allowQueryToken: false })
    const currentRow = chatBlobs.get(sha)
    if (res.destroyed || !currentAuth || currentAuth.member.memberId !== auth.member.memberId || !currentRow ||
      !chatBlobRoomIds(currentRow).some((r) => chatRooms.memberCanAccess(r, auth.member.memberId))) {
      releaseAtt()
      if (!res.destroyed) sendJson(res, 404, { ok: false, error: 'Attachment not found' })
      return
    }
    if (!safety.ok) {
      releaseAtt()
      sendJson(res, 422, { ok: false, error: safety.reason })
      return
    }
    try {
  ...
  if (req.method === 'POST' && path === '/v1/invite/create') {
    ...
    if (!requireCurrentRequestAdmin(req, res, auth)) return
  ...
  if (req.method === 'POST' && path === '/v1/invite/redeem') {
    ...
    const deviceId = exactMemberDeviceId(body.deviceId)
    if (body.redemptionNonce !== undefined && (typeof body.redemptionNonce !== 'string' || !/^[a-f0-9]{64}$/.test(body.redemptionNonce))) {
      sendJson(res, 400, { ok: false, error: 'Invalid invitation attempt proof' })
      return
    }
    const redeemed = await store.redeemInvite({
      token,
      deviceId,
      redemptionNonce: body.redemptionNonce as string | undefined,
    ...
    const current = store.findBySession(redeemed.sessionToken)
    if (!current || current.member.memberId !== redeemed.member.memberId || current.deviceId !== deviceId) {
      sendJson(res, 409, { ok: false, error: 'Invitation session was superseded. Join again.' })
      return
    }
    // TCC-R1125-BRG-001: HTTP and WS redemption revoke replaced device bearers and all
    // bearers on role change. Additional same-role devices remain authorized.
    ...
    for (const [peerWs, peer] of live) {
      if (!redeemed.replayed && peer.memberId === redeemed.member.memberId
        && !store.findBySession(peer.sessionToken)) {
        quietDropLiveSession(peerWs)
        try { peerWs.close(4001, 'session rotated') } catch { /* */ }
      }
  ...
  if (blobMatch) {
    ...
    if (req.method === 'GET') {
      if (!store.contentAccess.mayReadBlob(sha, auth.member.memberId, auth.member.role)) {
      ...
      try {
        ...
        stream.on('data', () => {
          const current = authFromReq(req, { allowQueryToken: false })
          if (!current || !requestStillSameSession(req, auth)
            || !store.contentAccess.mayReadBlob(sha, current.member.memberId, current.member.role)) {
            stopRead()
            res.destroy()
          }
        })
    ...
    if (req.method === 'POST' || req.method === 'PUT') {
      ...
      try {
        const put = await store.putBlobFromStream(sha, req, len, {
          ...
          authorize: () => requestStillAuthorized(req, auth, 'writer'),
        })
        if (!put.ok) {
          sendJson(res, put.status ?? 400, { ok: false, error: put.error })
          return
        }
        store.contentAccess.noteUpload(sha, auth.member.memberId)
  ...
  // The restore daemon is intentionally stopped during the synchronous local
  // merge, so permit release also has a header-authenticated HTTP path. The
  // bearer token is never placed in a URL or renderer response.
  if (req.method === 'POST' && (path === '/v1/team-content-restore/finish' || path === '/v1/team-content-restore/validate')) {
    const auth = authFromReq(req, { allowQueryToken: false })
    if (!auth || auth.member.role !== 'admin') {
      drainRequestBody(req)
      sendJson(res, 403, { ok: false, error: 'Admin session required' })
    ...
    if (!takeBackupToken(auth.member.memberId)) {
      drainRequestBody(req)
      sendRateLimited(res, limiterPace(backupLimiter, `backup:${auth.member.memberId}`, BACKUP_TOKENS_PER_WINDOW, OPS_FRAME_WINDOW_MS).waitMs, 'Rate limited - slow down')
      return
    }
    let body: unknown = null
    try {
      body = await readJsonBody(req, 4_096, { reserveBytes: 4_096, memberId: auth.member.memberId })
      const fresh = authFromReq(req, { allowQueryToken: false })
      if (!fresh || fresh.member.role !== 'admin' || fresh.member.memberId !== auth.member.memberId || fresh.deviceId !== auth.deviceId) {
        sendJson(res, 401, { ok: false, error: 'Session changed' })
        return
      }
      const permitToken = body && typeof body === 'object' && !Array.isArray(body)
        && Object.prototype.hasOwnProperty.call(body, 'permitToken')
        ? String((body as Record<string, unknown>).permitToken ?? '')
        : ''
      const identity = {
        teamId: boundTeamId(), actorMemberId: fresh.member.memberId,
        actorDeviceId: fresh.deviceId, token: permitToken,
      }
      if (path.endsWith('/validate')) {
        const result = teamContentLockStore.validateRestorePermit(identity)
        if (!result.ok) {
          sendJson(res, 409, { ok: false, error: result.reason })
          return
        }
        sendJson(res, 200, { ok: true, lockRevision: result.lockRevision, expiresAt: result.expiresAt })
        return
      }
      const result = teamContentLockStore.finishRestorePermit(identity)
      if (!result.ok) {
        sendJson(res, 409, { ok: false, error: result.reason })
        return
      }
      console.warn(`[bridge] shared-content restore permit released by Admin ${logId(fresh.member.memberId)} over HTTP`)
  ...
  if (path === '/v1/backups' || path.startsWith('/v1/backups/')) {
    ...
    const currentBackupAuth = (adminOnly = false, writerOnly = false) => {
      const fresh = authFromReq(req, { allowQueryToken: false })
      if (!fresh || fresh.member.memberId !== memberId) return null
      if (adminOnly && fresh.member.role !== 'admin') return null
      if (writerOnly && fresh.member.role === 'viewer') return null
      return fresh
    }
    if (req.method === 'POST' && path === '/v1/backups/read-scope') {
      if (!takeBackupToken(memberId)) {
        drainRequestBody(req)
        sendRateLimited(res, limiterPace(backupLimiter, `backup:${memberId}`, BACKUP_TOKENS_PER_WINDOW, OPS_FRAME_WINDOW_MS).waitMs, 'Backup permission checks are rate limited. Try again shortly.')
        return
      }
      let body: unknown
      try {
        body = await readJsonBody(req, 128_000, { memberId })
        const targets = parseBackupReadTargets(body)
        const fresh = currentBackupAuth()
        if (!fresh) { sendJson(res, 403, { ok: false, error: 'Current session required' }); return }
        // Initialize/refresh the same policy used by live sync. The following
        // read and response are synchronous; no body/queue await may use the
        // earlier role or content grants.
        currentFieldAccess()
        const scope = backupReadScope({ targets, authority: store.contentAccess,
          teamId: store.ensureTeam().teamId, memberId, role: fresh.member.role,
          fieldAuthorityHash: store.fieldAcl.currentHash() ?? '', composeAuthorityRevision: composeAclStoreStatus().revision,
          secondaryAuthorityStamp: `${officeObjectStore.backupRevision()}:${chatRooms.backupAccessRevision()}:${chatBlobs.backupAccessRevision()}`,
          secondaryTarget: target => {
            if(target.kind==='chat-blob'){
              const blob=chatBlobs.get(target.id),room=blob&&chatBlobRoomIds(blob).find(id=>chatRooms.memberCanAccess(id,memberId))
              return room?{...target,root:`chat:${room}`,entityId:'',parent:'',revision:chatBlobs.backupAccessRevision(),privateFields:false}:null
            }
            if(target.kind==='chat-room')return chatRooms.memberCanAccess(target.id,memberId)
              ? {...target,root:`chat:${target.id}`,entityId:'',parent:'',revision:chatRooms.backupAccessRevision(),privateFields:false}:null
            const office=officeObjectStore.backupObject(fresh.member,target.kind.slice('office-'.length),target.id)
            return office?{...target,root:`office:${office.kind}:${office.sourceId}`,entityId:'',parent:'',revision:office.version,privateFields:false,office,
              officeLocalOwner:officeObjectStore.backupCreator(office.kind,office.sourceId)===memberId}:null
          } })
        const expected = (body as Record<string, unknown>).fingerprint
        const expectedAuthority = (body as Record<string, unknown>).authorityStamp
        if (expected !== undefined && expected !== scope.fingerprint || expectedAuthority !== undefined && expectedAuthority !== scope.authorityStamp) {
          sendJson(res, 409, { ok: false, error: 'Backup permissions changed. Restart the operation.' }); return
        }
        sendJson(res, 200, { ok: true, ...scope })
      } catch (err) {
        sendCatchError(res, err)
      } finally { releaseJsonBody(body) }
      return
    }
    // Archives are opaque client snapshots, not a projection of this reader's
    // current module/field permissions. Even an OWN archive may contain a
    // hidden field. Enforce containment here too so old/modified clients and
    // allowMemberDownloadOthers cannot bypass the desktop refusal.
    const rawArchiveTransfer = (req.method === 'POST' && path === '/v1/backups') ||
      (req.method === 'GET' && /^\/v1\/backups\/[a-zA-Z0-9._-]{8,128}$/.test(path))
    if (rawArchiveTransfer && !isAdmin) {
      sendJson(res, 403, { ok: false, error: 'Member archive transfers are temporarily unavailable until permission-aware backups are supported.' })
      return
    }
    ...
    if (req.method === 'POST' && path === '/v1/backups') {
      ...
      const keepHint =
        typeof keepHdr === 'string' && /^\d{1,3}$/.test(keepHdr.trim())
          ? Number(keepHdr.trim())
          : undefined
      const claimedOwner = parseBackupMetaHeader(req.headers['x-backup-meta'])
      const team = store.ensureTeam()
      const owner = {
        ...claimedOwner,
        memberId,
      ...
      const put = await backupStore.putSnapshotFromStream({
        ...
        keepHint,
        owner,
        authorize: () => !!currentBackupAuth(true),
      })
      if (!put.ok) {
        sendJson(res, put.status ?? 400, { ok: false, error: put.error })
        return
    ...
    const memberFolderMatch =
      /^\/v1\/backups\/members\/(?!\.{1,2}$)([a-zA-Z0-9._%:-]{1,384})$/.exec(path)
    if (memberFolderMatch && req.method === 'DELETE') {
      ...
      let targetMemberId = ''
      try { targetMemberId = decodeURIComponent(memberFolderMatch[1] || '') } catch { /* invalid identity refused below */ }
      ...
      const wiped = await backupStore.deleteAllForMember(targetMemberId, () =>
        !!currentBackupAuth(true) && !store.findMember(targetMemberId))
      if (!wiped.ok) {
    ...
    if (req.method === 'GET' && path === '/v1/backups/export.zip') {
      ...
      let metadataSnapshot: string | undefined
      const onExportClose = () => abort.abort()
      res.once('close', onExportClose)
      try {
      ...
      const requestedIds = idsRaw
        ...
        .map((s) => s.trim())
        .filter(Boolean)
      const ids = requestedIds.slice(0, maxAimoves)
      ...
      const chatPick = backupStore.pickChatExportEntries(MAX_BACKUP_ZIP_CHAT_ENTRIES, AUTHORITY_DIR)
      const zipEntries = [
      ...
      metadataSnapshot = await mkdtemp(join(tmpdir(), 'aitomation-chat-backup-'))
      const stableEntries = await snapshotChatBackupMetadata(zipEntries, metadataSnapshot, abort.signal)
      const planned = await planStoredBackupZip(stableEntries, { maxEntries: combinedCeiling, signal: abort.signal })
      if (!planned.ok) {
      ...
      const idsRequested = mode === 'ids' ? requestedIds.length : 0
      ...
      const zipped = await streamPlannedBackupZip(res, planned.planned, planned.contentLength, { authorize: () => !!currentBackupAuth(true) })
      if (!zipped.ok) {
        console.warn('[bridge] backup export stream failed:', zipped.error)
      ...
      return
        ...
        res.off('close', onExportClose)
        if (metadataSnapshot) await removeExportSnapshot(metadataSnapshot, { recursive: true, force: true }).catch(() => {})
        backupExportLease.release()
      }
    }
    ...
    if (oneMatch) {
      ...
      if (req.method === 'GET') {
        ...
        const fresh = currentBackupAuth(true)
        if (!fresh) {
          opened.stream.destroy()
          sendJson(res, 403, { ok: false, error: 'Backup access is no longer allowed' })
          return
        }
        res.writeHead(200, {
          'content-type': 'application/octet-stream',
        ...
        try {
          const guard = new BackupDownloadGuard({
            transform(chunk, _encoding, callback) {
              const current = currentBackupAuth(true)
              if (!current) {
                callback(new Error('Backup access is no longer allowed'))
                return
              }
              callback(null, chunk)
            },
          })
          await pipelineBackupDownload(opened.stream, guard, res)
        } catch {
          // Cancellation must close the pinned source descriptor, and an I/O
          // failure must terminate the response rather than crash the bridge.
          res.destroy()
        }
        return
      }
      ...
      if (req.method === 'DELETE') {
        ...
        const del = await backupStore.deleteSnapshot(ownerId, snapId, () => !!currentBackupAuth(ownerId !== memberId))
        if (!del.ok) {
  ...
  if (req.method === 'POST' && path === '/v1/public-share/register') {
    ...
    try {
      ...
      if (!store.fieldAcl.isCurrentHash(body.fieldAclBaseHash)) {
        sendJson(res, 409, { ok: false, error: 'Team field access changed; rebuild the shared payload before publishing' })
        return
      }
      if (!mode) {
      ...
      const result = publicShares.upsertShare(auth.member.memberId, {
        fieldAclBaseHash: store.fieldAcl.snapshot()!.hash,
  ...
  if (req.method === 'POST' && path === '/v1/portal/register') {
    ...
    try {
      ...
      // The request body may have awaited a concurrent access change.
      if (!store.fieldAcl.isCurrentHash(body.fieldAclBaseHash)) {
        sendJson(res, 409, { ok: false, error: 'Team field access changed; rebuild the portal payload before publishing' })
        return
      }
  ...
  // --- Compose live co-edit ACL (TCC-FIX-CMP-001 / TCC-FIX-CMP-002) ---
  // Current clients mutate one document at a time. A full snapshot is accepted
  // only as the first durable bootstrap from the earliest-created live Admin;
  // later reconnect snapshots are validated no-ops. This prevents one stale
  // co-Admin from replacing (and widening) another Admin's exact audiences.
  // See `compose-acl-store.ts` for durable/tombstone semantics.
  if (req.method === 'POST' && path === '/v1/teamspace/compose-acl') {
    ...
    try {
      ...
      const before = composeAclStoreStatus()
      if (before.unavailable || !store.contentAccess.healthy()) {
        sendJson(res, 503, { ok: false, error: 'Compose ACL store is unavailable' })
        return
      }
      const hasMutation = Object.prototype.hasOwnProperty.call(body, 'mutation')
      if (!hasMutation && !before.bootstrapped) {
        const primaryAdmin = store.listMembers()
          .filter((member) => member.role === 'admin')
          .sort((a, b) => a.createdAt - b.createdAt || a.memberId.localeCompare(b.memberId))[0]
        if (!primaryAdmin || primaryAdmin.memberId !== auth.member.memberId) {
          sendJson(res, 409, {
            ok: false,
            error: 'The primary Admin must initialize live board access',
          })
          return
        }
      }
      // Prefer an explicitly present modern field even when malformed. A bad
      // mutation/snapshot must never fall through to the legacy whole-team bag.
      const hasDocuments = Object.prototype.hasOwnProperty.call(body, 'documents')
      const result = hasMutation
        ? mutateComposeAclDocument(body.mutation)
        : hasDocuments
          ? bootstrapComposeAclDocuments(body.documents)
          : bootstrapComposeAclSharedDocIds(body.documentIds)
      if (!result.ok) {
        sendJson(
          res,
          result.reason === 'Compose ACL store is unavailable'
            || result.reason === 'Could not persist Compose ACL'
            ? 503
            : 400,
      ...
      const changedAccess = syncComposeContentAccess(store)
      const evicted = evictUnsharedComposeYjsPeers()
      if (changedAccess.length) sendColleagueScopedOps(null, 'compose-access', changedAccess)
      sendJson(res, 200, { ok: true, count: result.count, evicted })
    } catch (err) {
      sendCatchError(res, err)
    } finally {
      releaseJsonBody(body)
...
${GUEST_FORM_FIELD_JS}
${GUEST_LIVE_JS}
  var lastSubmissionData = null, lastSubmissionKey = null;
  function submissionRetryKey(data) {
    var serialized = JSON.stringify(data);
    // This local comparison hash stores no form text and works on HTTP hosts.
    // A collision is refused by the server's SHA-256 content binding.
    var a = 2166136261, b = 5381;
    for (var i = 0; i < serialized.length; i++) { a = Math.imul(a ^ serialized.charCodeAt(i), 16777619); b = Math.imul(b, 33) ^ serialized.charCodeAt(i); }
    var fingerprint = serialized.length + ':' + (a >>> 0) + ':' + (b >>> 0);
    var slot = 'aitomation-form-retry:' + location.pathname;
    if (!lastSubmissionKey) try {
      var saved = JSON.parse(sessionStorage.getItem(slot) || 'null');
      if (saved && saved.fingerprint === fingerprint && /^[a-f0-9]{32}$/.test(saved.key)) { lastSubmissionKey = saved.key; lastSubmissionData = serialized; }
    } catch (_) {}
    if (lastSubmissionData !== serialized || !lastSubmissionKey) {
      var bytes = new Uint8Array(16); crypto.getRandomValues(bytes);
      lastSubmissionKey = Array.from(bytes, function(b){ return b.toString(16).padStart(2, '0'); }).join('');
      lastSubmissionData = serialized;
      try { sessionStorage.setItem(slot, JSON.stringify({ fingerprint: fingerprint, key: lastSubmissionKey })); } catch (_) {}
    }
    return lastSubmissionKey;
  }
  function receivedSubmission(btn) {
    btn.disabled = true;
    var another = document.createElement('button'); another.type = 'button'; another.textContent = 'Submit another response';
    another.style.marginLeft = '12px'; btn.after(another);
    another.onclick = function(){
      lastSubmissionKey = null; lastSubmissionData = null;
      try { sessionStorage.removeItem('aitomation-form-retry:' + location.pathname); } catch (_) {}
      another.remove(); btn.disabled = false; document.getElementById('msg').textContent = '';
    };
  }
  function api(method, body, signal){
    ...
    }).then(function(r){ return r.json().then(function(j){ return {status:r.status, j:j, retryAfter:r.headers.get('Retry-After')}; }); });
  }
  ...
  function render(data, unlocked){
    ...
    if (share.mode === 'read' || share.mode === 'create') content.mode = share.mode;
  ...
  var live = installGuestLivePage({app:app, api:api, render:render,
    read:function(){ return unlockedPassword ? {method:'POST',body:{password:unlockedPassword}} : {method:'GET'}; },
    clearCredentials:function(){ unlockedPassword = ''; },
    locked:function(){ return {ok:true,needs_password:true,unlocked:false}; }
  });
  api = live.api; render = live.render; live.start();
})();
`,
  })
}
...
async function handleGuestPortal(
  ...
  if (isCreate) {
    const contentLock = teamContentLockStore.get(capRequestTeamId(row.teamId) || boundTeamId())
    if (teamContentLockBlocks(contentLock, 'portal_intake')) {
      sendJson(res, 423, {
        error: contentLock.unavailable
          ? 'Team content lock is unavailable on the server'
          : 'This team has temporarily locked shared content changes',
        error_code: 'portal.team_content_locked',
      })
      return
    }
...
${GUEST_LIVE_JS}
  ...
  function render(data, unlocked){
    if (data && data.auth_mode) lastAuthMode = data.auth_mode;
    if (!data || !data.ok) { showError(data && data.error); return; }
    ...
    if (Array.isArray(portal.allowed_actions)) actions = actions.filter(function(action){ return portal.allowed_actions.indexOf(action) >= 0; });
    ...
    document.getElementById('sub').onclick = function(){
      ...
      body.submission_key = submissionRetryKey(dataBag);
  ...
  var live = installGuestLivePage({app:app, api:api, render:render,
    read:function(){ return unlockedPin ? {method:'POST',body:{pin:unlockedPin,session_id:sessionId}} : {method:'GET'}; },
    clearCredentials:function(){ unlockedPin = ''; },
    locked:function(data){ var otp = !!(data && data.needs_otp) || lastAuthMode === 'magic_link'; return {ok:true,needs_otp:otp,needs_pin:!otp,unlocked:false}; }
  });
  api = live.api; render = live.render; live.start();
})();
`,
...
async function handleGuestPublicShare(
  ...
  if (isCreate) {
    const contentLock = teamContentLockStore.get(capRequestTeamId(row.teamId) || boundTeamId())
    if (teamContentLockBlocks(contentLock, 'public_intake')) {
      releaseJsonBody(body)
      sendJson(res, 423, {
        error: contentLock.unavailable
          ? 'Team content lock is unavailable on the server'
          : 'This team has temporarily locked shared content changes',
        error_code: 'share.team_content_locked',
      })
      return
    }
...
const GRACEFUL_SHUTDOWN_TIMEOUT_MS = Math.max(
  1_000,
  Math.min(60_000, Number(process.env.TEAMSPACE_SHUTDOWN_TIMEOUT_MS || (mailService ? 30_000 : 5_000)) || (mailService ? 30_000 : 5_000)),
)
...
function gracefulShutdown(signal: string, exitCode = 0): void {
  ...
  shuttingDown = true
  workspaceCoordinatorMonitor.stop()
  // Stop queue admission/provider dispatch now, but let admitted HTTP work
  // finish before closing its SQLite worker underneath it.
  mailService?.beginShutdown()
  let mailClosePromise: Promise<void> | undefined
  const closeMail = () => {
    mailClosePromise ??= mailService?.close().catch(() => {
      console.warn('[bridge] mailbox shutdown was not fully confirmed; uncertain dispatches require reconciliation')
    }) ?? Promise.resolve()
    return mailClosePromise
  }
...
server.listen(PORT, BIND_HOST, () => {
  workspaceCoordinatorMonitor.start()
  ...
  if (teamAtBoot) {
    console.log(
      `[bridge] team: ${teamAtBoot.teamId} "${teamAtBoot.name}"`
        + ` (created ${new Date(teamAtBoot.createdAt).toISOString()}, loaded from ${AUTHORITY_DIR})`,
    )
  } else {
    console.warn(
      `[bridge] no team yet - no team.json in ${AUTHORITY_DIR}, so the first computer that connects`
