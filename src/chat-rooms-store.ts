...
import { CHAT_ROOMS_TOTAL_MAX } from './throughput.js'
...
/**
 * TS-CHAT-033 / TS-CHAT-027: a room that still has members must keep at
 * least one owner who is actually in the room. Load back-fill can seed
 * `createdBy` after that person already left; ban / leave / demote must
 * never leave `ownerIds` empty (or ghost-only) while members remain.
 * `preferNot` is the member we just dropped from ownership (still in
 * `memberIds` on demote) so succession does not immediately undo the drop.
 */
function ensureOwnersWhileMembersRemain(row: ChatRoomRow, preferNot?: string): boolean {
  if (row.kind === 'team') return false
  if (typeof row.closedAt === 'number' && Number.isFinite(row.closedAt) && row.closedAt > 0) {
    return false
  }
  if (row.memberIds.length === 0) return false
  const members = new Set(row.memberIds)
  const next = row.ownerIds.filter((id) => members.has(id))
  if (next.length === 0) {
    const skip = typeof preferNot === 'string' ? preferNot : ''
    const successor = row.memberIds.find((id) => id !== skip) ?? row.memberIds[0]
    if (successor) next.push(successor)
  }
  const changed =
    next.length !== row.ownerIds.length || next.some((id, i) => id !== row.ownerIds[i])
  if (changed) row.ownerIds = next
  return changed
}

/**
 * Cap for rooms listed per member (newest-created kept first after sort).
 * G11 residual: this used to be a hardcoded 2000, so raising
 * TEAMSPACE_CHAT_ROOMS_TOTAL_MAX (env ceiling 20_000) minted rooms the
 * list then hid. Stay at that env ceiling so create can raise without
 * hiding, and a later lower of the create cap does not wipe surplus
 * rooms from the list (create-refuse is not a delete). Load still
 * admits up to 50_000 rows; this is the visible-list hard cap.
 */
...
function normalizeEmojiAllowlist(raw: unknown): string[] {
  ...
  for (const x of raw) {
    if (typeof x !== 'string') continue
    const emoji = capTrim(x.replace(/\0/g, ''), 32)
...
/**
 * P2 permission model: whether `memberId` may change the
...
/** Authority selectors must never truncate/coerce onto another member. */
function exactRoomMemberId(raw: unknown): string {
  return typeof raw === 'string' && raw.length > 0 && raw.length <= 128 && raw.trim() === raw
    && !raw.includes('\0') && raw.isWellFormed() ? raw : ''
}

function syncRoomDirectory(path: string): void {
...
export class ChatRoomsStore {
  ...
  backupAccessRevision(): number { return this.backupRevision }
  private readonly dir: string
  private readonly path: string
  private rooms = new Map<string, ChatRoomRow>()
  private mutationDepth = 0
  private mutationDirty = false
  private mutationUndo = new Map<string, { row: ChatRoomRow; snapshot: ChatRoomRow } | null>()
  /**
   * File existed but could not be read (corrupt JSON, encrypted envelope
   * without a key, decrypt throw). Never persist over that file - creating
   * the built-in team room after a failed load used to overwrite every room.
   */
  private registryUnreadable = false
  private registryWriteUncertain = false
  ...
  constructor(
    ...
    if (!existsSync(this.dir)) {
      ...
      syncRoomDirectory(dataDir)
    }
    this.path = join(this.dir, 'rooms.json')
    this.load()
    this.ensureTeamRoom()
    // Nested operations (ban -> remove, redeem -> admit) form one durable
    // operation. No caller can observe success before the registry is written.
    for (const name of [
      'trimMembersToCap', 'getOrCreateDm', 'createGroup', 'admitMember',
      'addMembers', 'removeMembers', 'promoteOwner', 'demoteOwner', 'banMember',
      'unbanMember', 'setAllowedReactionEmojis', 'leave', 'closeRoom', 'setTitle',
      'setDescription', 'setIcon', 'setPermissions', 'setPassword',
      'mintInviteToken', 'redeemInvite',
    ] as const) {
      const method = this[name]
      Object.defineProperty(this, name, {
        value: (...args: unknown[]) => this.mutateDurably(() => Reflect.apply(method, this, args)),
      })
    }
  }

  private captureRoom(id: string): void {
    if (!this.mutationDepth || this.mutationUndo.has(id)) return
    const row = this.rooms.get(id)
    this.mutationUndo.set(id, row ? { row, snapshot: structuredClone(row) } : null)
  }

  private setRoom(id: string, row: ChatRoomRow): void {
    this.captureRoom(id)
    this.rooms.set(id, row)
  }

  private mutateDurably<T>(work: () => T): T | { error: string } {
    if (this.mutationDepth) return work()
    this.mutationDepth = 1
    this.mutationDirty = false
    this.mutationUndo.clear()
    try {
      const result = work()
      if (this.mutationDirty) this.persistSync()
      return result
    } catch {
      if (!this.registryWriteUncertain) {
        for (const [id, previous] of this.mutationUndo) {
          if (!previous) this.rooms.delete(id)
          else {
            // Restore retained row references as well as future ACL lookups.
            for (const key of Object.keys(previous.row)) delete (previous.row as unknown as Record<string, unknown>)[key]
            Object.assign(previous.row, previous.snapshot)
            this.rooms.set(id, previous.row)
          }
        }
      }
      return this.refuseIfRegistryUnreadable() ?? { error: 'Chat room changes could not be saved. Please try again.' }
    } finally {
  ...
  private refuseIfRegistryUnreadable(): { error: string } | null {
    if (this.registryWriteUncertain) return {
      error: 'Chat room saved state is uncertain. Stop the bridge, repair storage, then restart to reload the complete registry. Do not delete the registry.',
    }
    if (!this.registryUnreadable) return null
    return {
      error: 'Chat rooms could not be loaded. Check the team server data file and the encryption key.',
    }
  }
  ...
  private load(): void {
    ...
    try {
      ...
      let parsed: RoomsFile | null = null
      if (this.atRest) {
        parsed = decryptJsonFile(this.atRest, raw, null as unknown as RoomsFile)
      } else {
        parsed = JSON.parse(raw) as RoomsFile
      }
      ...
      for (const row of parsed.rooms.slice(0, 50_000)) {
        ...
        const next: ChatRoomRow = {
          id: p.room,
          kind: p.kind,
          title: typeof row.title === 'string' ? capStr(row.title, 120) : '',
          memberIds,
          createdBy,
          createdAt:
        ...
        if (ensureOwnersWhileMembersRemain(next)) healedOwners = true
        this.setRoom(p.room, next)
      }
      if (healedOwners) this.queuePersist()
    } catch {
      this.registryUnreadable = true
      this.rooms.clear()
    }
  ...
  private persistSync(): void {
    retainIndependentCheckpoint(dirname(this.dir),'chat/rooms.json')
    this.backupRevision++
    ...
    let fd: number | undefined, ownedTemp = false, renameAttempted = false, renamed = false
    try {
      ...
      renamed = true
      syncRoomDirectory(this.dir)
    } catch (error) {
      if (renamed || (renameAttempted && !existsSync(tmp))) {
        // The complete new registry is visible (or rename reported an error
        // after the source disappeared), but its rename is not proven
        // crash-durable. Neither the previous nor the next grants may be served
        // as an acknowledged state. Freeze retained views as well as lookups.
        this.registryUnreadable = this.registryWriteUncertain = true
        for (const row of this.rooms.values()) {
          row.closedAt = Date.now()
          row.memberIds = []
          row.ownerIds = []
          row.inviteHash = row.inviteExpiresAt = row.passwordHash = null
        }
      }
      throw error
    } finally {
      if (fd !== undefined) { try { closeSync(fd) } catch { /* original failure remains */ } }
      if (ownedTemp && !renamed) { try { unlinkSync(tmp) } catch { /* never delete a pre-existing path */ } }
    }
  }
  ...
  private queuePersist(): void {
    ...
    // Startup repairs must also fail closed; constructor/load own that error.
    this.persistSync()
  }
  ...
  /**
   * BRG-009 lasting-room twin of the ephemeral process ceiling.
   * Counts OPEN rooms via listAllOpenRooms (closed rows do not take a slot).
   * Call only when minting a NEW lasting room - never on existing/reopen.
   */
  private assertCanMintLastingRoom(): { error: string } | null {
    const open = this.listAllOpenRooms().length
    if (open >= CHAT_ROOMS_TOTAL_MAX) {
      return {
        error: `This team already has ${open} chat rooms, but the limit is ${CHAT_ROOMS_TOTAL_MAX}`,
      }
    }
    return null
  }
   ...
   * The full change is committed atomically, including trims over 50 members.
   */
  trimMembersToCap(
    roomId: string,
    cap: number,
  ): { ok: true; removed: string[]; memberIds: string[] } | { error: string } {
    const blocked = this.refuseIfRegistryUnreadable()
    ...
    // A lowered team limit can remove more than one public 50-member batch.
    // Apply the complete trim in this outer durable mutation, without a synthetic
    // actor whose name could collide with a legitimate opaque member ID.
    row.memberIds = next
    row.ownerIds = row.ownerIds.filter((id) => next.includes(id))
    ensureOwnersWhileMembersRemain(row)
    this.setRoom(row.id, row)
    this.queuePersist()
  ...
  /**
   * TCC-CHAT-005: same list as listForMember plus truncated when over
   * CHAT_ROOMS_LIST_MAX. Fail-closed load (corrupt / encrypted-without-key)
   * must not look like "this person has no rooms" - mutators already refuse;
   * the list must carry the same sentence so rooms_list can refuse too.
   */
  listForMemberWithHonesty(memberId: string): {
    rooms: ChatRoomRow[]
    truncated: boolean
    error?: string
  } {
    const blocked = this.refuseIfRegistryUnreadable()
    if (blocked) {
      return { rooms: [], truncated: false, error: blocked.error }
    }
    const out: ChatRoomRow[] = []
    for (const row of this.rooms.values()) {
      if (row.closedAt) continue
  ...
  get(roomId: string): ChatRoomRow | null {
    if (this.registryUnreadable) return null
    const p = parseChatRoomId(roomId)
    if (!p.ok || p.room !== roomId) return null
    this.captureRoom(p.room)
  ...
  getOrCreateDm(memberA: string, memberB: string): ChatRoomRow | { error: string } {
    const blocked = this.refuseIfRegistryUnreadable()
    if (blocked) return blocked
    const a = exactRoomMemberId(memberA)
    const b = exactRoomMemberId(memberB)
    ...
    const existing = this.get(id) ?? (legacyId ? this.get(legacyId) : undefined)
    ...
    if (existing && !existing.closedAt) {
      ...
      if (ensureOwnersWhileMembersRemain(existing) || changed) {
        this.setRoom(existingId, existing)
        this.queuePersist()
      }
      return existing
    }
  ...
  createGroup(input: {
    ...
    const blocked = this.refuseIfRegistryUnreadable()
    if (blocked) return blocked
    const title = capTrim(String(input.title || '').replace(/\0/g, ''), 120)
    if (!title) return { error: 'Title required' }
    const createdBy = exactRoomMemberId(input.createdBy)
    if (!createdBy) return { error: 'createdBy required' }
    if (!Array.isArray(input.memberIds) || input.memberIds.some((id) => !exactRoomMemberId(id))) {
      return { error: 'Invalid member IDs' }
    }
    const members = new Set<string>([createdBy, ...input.memberIds])
  ...
  admitMember(roomId: string, memberId: string): { ok: true } | { error: string } {
    ...
    const mid = exactRoomMemberId(memberId)
  ...
  addMembers(
    ...
    if (!Array.isArray(memberIds) || memberIds.length > 50 || memberIds.some((id) => !exactRoomMemberId(id))) {
      return { error: 'Provide up to 50 exact member IDs' }
    }
  ...
  removeMembers(
    ...
    const actor = exactRoomMemberId(actorId)
  ...
  demoteOwner(
    ...
    ensureOwnersWhileMembersRemain(row, mid)
    this.setRoom(row.id, row)
    this.queuePersist()
    return { ok: true, ownerIds: [...row.ownerIds] }
  }
  ...
  banMember(
    ...
    if (!fresh.bannedMemberIds.includes(mid)) {
      ...
      this.setRoom(fresh.id, fresh)
      this.queuePersist()
    } else if (!removed) {
      unchanged = true
    }
    if (ensureOwnersWhileMembersRemain(fresh)) {
      this.setRoom(fresh.id, fresh)
      this.queuePersist()
    }
    return {
  ...
  leave(
    ...
    if (row.memberIds.length === 0) {
      ...
      // becomes the new sole owner when the leave emptied ownerIds, including
      // after a ghost createdBy back-fill. Never a silent no-owner state.
      ensureOwnersWhileMembersRemain(row)
    }
    this.setRoom(row.id, row)
    this.queuePersist()
  ...
  verifyPassword(roomId: string, password: string): boolean {
    const row = this.get(roomId)
    // Missing room is not "no password". `!row?.passwordHash` is true when
    // get() returns null, which would pass a join that never proved the room.
    if (!row) return false
    if (row.closedAt) return false
    if (!row.passwordHash) return row.kind !== 'private'
    try {
  ...
  setTitle(
    ...
    const next = capTrim(String(title || '').replace(/\0/g, ''), 120)
  ...
  setPassword(roomId: string, password: string): { ok: true } | { error: string } {
    ...
    if (row.closedAt) return { error: 'Room is closed' }
    const pw = capStr(String(password || '').replace(/\0/g, ''), 128)
  ...
  redeemInvite(token: string, memberId: string, expectedRoomId?: string): ChatRoomRow | { error: string } {
    const blocked = this.refuseIfRegistryUnreadable()
    if (blocked) return blocked
    if (!exactRoomMemberId(memberId)) return { error: 'Invalid member' }
    if (typeof token !== 'string' || !/^[A-Za-z0-9_-]{32}$/.test(token)) return { error: 'Invalid invite token' }
    if (expectedRoomId !== undefined) {
      const parsed = parseChatRoomId(expectedRoomId)
      if (!parsed.ok || parsed.room !== expectedRoomId || parsed.kind !== 'private') return { error: 'Private room required' }
    }
    const h = hashInvite(token)
    const now = Date.now()
    for (const row of this.rooms.values()) {
      if (row.kind !== 'private' || !row.inviteHash || row.closedAt) continue
      if (row.inviteHash !== h) continue
      if (expectedRoomId !== undefined && row.id !== expectedRoomId) return { error: 'Invite belongs to another room' }
      this.captureRoom(row.id)
