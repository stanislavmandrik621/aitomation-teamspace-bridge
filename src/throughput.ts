...
/** Anonymous HTTP requests per IP per window (including invite redemption). */
export const HTTP_TOKENS_PER_WINDOW = envInt('TEAMSPACE_HTTP_TOKENS', 60, 10, 1_000)

/** Authenticated HTTP requests per member, shared across that member’s devices.
 * Independent of the office NAT budget; expensive routes retain their own caps. */
export const AUTH_HTTP_TOKENS_PER_WINDOW = envInt('TEAMSPACE_AUTH_HTTP_TOKENS', 600, 10, 10_000)

/** Routine existing-record collaboration, independent of share/portal Admin work.
 * The general authenticated HTTP ceiling still applies across all routes. */
export const RECORD_TEAMWORK_HTTP_TOKENS_PER_WINDOW = envInt('TEAMSPACE_RECORD_TEAMWORK_HTTP_TOKENS', AUTH_HTTP_TOKENS_PER_WINDOW, 10, 10_000)
/** Personal/team profile saves are infrequent, but never spend the Admin bucket. */
export const MEMBER_PROFILE_HTTP_TOKENS_PER_WINDOW = envInt('TEAMSPACE_MEMBER_PROFILE_HTTP_TOKENS', 60, 5, 10_000)

/** One instance per operation family; every device of a member shares its key.
 * Uses the existing bounded limiter and reports the remaining window accurately. */
export function createMemberHttpWriteBudget(tokens: number, windowMs = OPS_FRAME_WINDOW_MS, maxKeys = 10_000) {
  const limiter = new TokenBucketLimiter(maxKeys)
  return {
    take: (memberId: string): boolean => limiter.take(memberId, tokens, windowMs),
    retryAfterSeconds: (memberId: string): number => Math.max(1, Math.ceil((limiter.waitMsUntilRefill(memberId, tokens, windowMs) || windowMs) / 1000)),
  }
}
 ...
 * axis (how many sockets one room can hold). The default admits 128 room
 * participants. The local Docker qualification exercised 128 real Yjs
 * replicas and 1,280 concurrent burst updates on a one-CPU server. This is
 * tested workload coverage, not a guarantee for arbitrary document sizes
 * or update rates. Self-hosters can tune the ceiling via env, and admins
 * can choose a lower working limit in Team Space settings.
 */
export const YJS_ROOM_MAX_PEERS = envInt('TEAMSPACE_YJS_ROOM_MAX_PEERS', 128, 5, 500)
