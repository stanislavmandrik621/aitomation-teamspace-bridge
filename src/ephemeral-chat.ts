...
/** Two maximum-size member tokens plus the prefix and unambiguous separator. */
export const EPHEMERAL_ROOM_MAX_LEN = 261
export const EPHEMERAL_ROOM_PAIR_SEP = '.'
 ...
 * `eph:<a>.<b>` id even though both start with `eph:`.
 */
...
function isMemberToken(s: string): boolean {
  // Host identities use device:<id>; ':' cannot collide with the pair separator.
  return s.length <= 128 && /^(?:device:)?[a-zA-Z0-9_-]{1,128}$/.test(s)
}
...
/** `_` is legal in member IDs. Only accept legacy pairs with exactly one `_`. */
export function splitEphemeralRoomPair(pair: string): [string, string] | null {
  const parts = pair.split(pair.includes(EPHEMERAL_ROOM_PAIR_SEP) ? EPHEMERAL_ROOM_PAIR_SEP : '_')
...
export function ephemeralRoomId(memberA: string, memberB: string): string | null {
  ...
  return `${EPHEMERAL_ROOM_PREFIX}${x}${EPHEMERAL_ROOM_PAIR_SEP}${y}`
}
...
export function parseEphemeralRoomId(room: unknown): { ok: true; room: string } | { ok: false; reason: string } {
  ...
  const parts = splitEphemeralRoomPair(rest)
  if (!parts) return { ok: false, reason: 'invalid temporary chat room id' }
  ...
  const sep = rest.includes(EPHEMERAL_ROOM_PAIR_SEP) ? EPHEMERAL_ROOM_PAIR_SEP : '_'
  return { ok: true, room: `${EPHEMERAL_ROOM_PREFIX}${a}${sep}${b}` }
}
...
export function isEphemeralGroupRoomId(room: unknown): boolean {
  return parseEphemeralGroupRoomId(room).ok
}
...
export function parseAnyEphemeralRoomId(room: unknown): { ok: true; room: string } | { ok: false; reason: string } {
  if (typeof room === 'string' && room.replace(/\0/g, '').trim().startsWith(EPHEMERAL_GROUP_ROOM_PREFIX)) {
...
export function validateEphemeralGroupTargetMemberIds(
  ...
  for (const entry of raw) {
    ...
    const id = entry.replace(/\0/g, '').trim()
...
export function ephemeralPeerMemberId(room: string, ownMemberId: string | null | undefined): string | null {
  ...
  if (!own) return null
  const pair = splitEphemeralRoomPair(parsed.room.slice(EPHEMERAL_ROOM_PREFIX.length))
  if (!pair) return null
