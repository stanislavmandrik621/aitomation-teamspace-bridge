...
export class PortalBridgeStore {
  ...
  constructor(dataDir: string, atRest: AtRestKey | null = null) {
    ...
    this.submissionReceipts = new GuestSubmissionReceipts(join(dataDir, 'portal-submission-receipts'), atRest)
    this.otpPendingDir = join(dataDir, OTP_PENDING_DIR)
    this.atRest = atRest
    this.retiredTokens = new GuestTokenRetirementStore(join(dataDir, 'portal-retired-tokens'), atRest)
    ensureDir(dataDir)
  ...
  private save(portals: PortalRow[]): void {
    retainIndependentCheckpoint(dirname(this.registryPath),'portals.json')
  ...
  upsertPortal(
    ...
    // Migrate existing tombstones before any registry overwrite or GC.
    if (prev && isPortalRevoked(prev)) this.retiredTokens.retire(prev.tokenHash)
    if (this.retiredTokens.isRetired(tokenHash)) {
      return { ok: false, error: 'Portal token was revoked; publish a new link' }
    }
    ...
    if (args.payload != null) {
      const set = this.writePayloadFile(tokenHash, stampGuestFieldAclPayload(args.payload, args.fieldAclBaseHash))
    ...
    if (idx >= 0) {
      ...
      if (prev && prev.tokenHash !== tokenHash) {
        // Persist negative identity before replacing the row. A late old-token
        // update cannot rotate a newer publication back, even after restart.
        this.retiredTokens.retire(prev.tokenHash)
        try {
  ...
  writePayload(
    ...
    if (/^[0-9a-f]{64}$/.test(h) && this.retiredTokens.isRetired(h)) {
      return { ok: false, error: 'Portal is not active' }
    }
  ...
  revokePortal(args: {
    ...
    const idx = found.idx
    const row = idx >= 0 ? portals[idx] : null
    const validHash = /^[0-9a-f]{64}$/.test(tokenHash)
    const alreadyRetired = validHash && this.retiredTokens.isRetired(tokenHash)
    if (validHash) this.retiredTokens.retire(tokenHash)
    // An old compensation/retry must not revoke a newer publication B.
    // Unknown token C is an overtaking revoke of a pending publish, so it
    // still closes the old current row while C itself is now retired.
    if (row && validHash && row.tokenHash !== tokenHash && alreadyRetired) return { ok: true }
    if (idx < 0) {
      // A revoke can overtake the first publish while its larger request body
      // is still arriving. Remember that token even without a registry row so
      // the delayed register cannot create a guest URL after successful revoke.
      if (!localPortalId || !/^[0-9a-f]{64}$/.test(tokenHash)) {
        return { ok: false, error: 'Portal not found' }
      }
      return { ok: true }
    }
    const current = portals[idx]
    const rowTokenHash = current.tokenHash
    this.retiredTokens.retire(rowTokenHash)
    portals[idx] = {
      ...current,
      revokedAt: Date.now(),
      updatedAt: Date.now(),
    ...
    this.scrubSubmissionsForLocalPortalId(current.localPortalId)
  ...
  readPayload(tokenHash: string): PortalPayload | null {
    ...
    const raw = readGuestFieldAclPayload(readJsonFile<unknown>(path, this.atRest, null), this.fieldAclGuard)
  ...
  resolveGuest(tokenPlain: string): {
    ...
    try {
      if (this.retiredTokens.isRetired(tokenHash)) return { row, active: false, reason: 'revoked' }
    } catch {
  ...
  enqueueSubmission(args: {
    ...
    try {
      replay = this.submissionReceipts.probe(args.row.tokenHash, args.submissionKey, { data: args.rawData, contactLabel: args.contactLabel ?? null })
      ...
      return { ok: false, status: failure.status, error_code: "portal." + failure.code, error: failure.message }
    }
  ...
  listPendingSubmissions(limit = 50, ownerMemberId?: string, teamId?: unknown): PortalSubmission[] {
    ...
    for (const name of names) {
      ...
      if (this.submissionReceipts.isComplete(raw.id)) continue
