...
const GUEST_RENDER_CORE_JS = `
  ...
  function canonicalViewType(vt){
    ...
    if (t === 'listing' || t === 'rows' || t === 'items') return 'list';
    ...
    if (t === 'agenda' || t === 'month') return 'calendar';
    ...
    if (t === 'bookings' || t === 'resources' || t === 'resource_scheduler' || t === 'schedule') return 'scheduler';
  ...
  var GUEST_CHART_CANVAS_WIDTH = 960;
  function guestChartCanvasWidth(){ return GUEST_CHART_CANVAS_WIDTH; }
  function chartFills(){
  ...
  function chartBuckets(content){
    ...
    rows.forEach(function(r){
      ...
      if (!Object.prototype.hasOwnProperty.call(counts, key)) { counts[key] = 0; sums[key] = 0; mins[key] = Infinity; maxs[key] = -Infinity; order.push(key); }
    ...
    var out = [];
    var cap = 12;
    var otherCount = 0, otherSum = 0, otherMin = Infinity, otherMax = -Infinity, otherCategories = 0;
    for (var i=0;i<order.length;i++){
      ...
      if (out.length >= cap) { otherCategories++; otherCount += counts[k]; otherSum += sums[k]; otherMin = Math.min(otherMin, mins[k]); otherMax = Math.max(otherMax, maxs[k]); continue; }
      out.push({ k: k, v: v });
    }
    if (otherCategories) {
      var other = agg === 'count' ? otherCount : agg === 'avg' ? (otherCount ? otherSum / otherCount : 0) : agg === 'min' ? otherMin : agg === 'max' ? otherMax : otherSum;
      out.push({ k: 'Other', v: isFinite(other) ? Math.round(other * 100) / 100 : 0 });
    }
    return out;
  }
  ...
  function renderBarSvg(buckets){
    ...
    for (i=0;i<buckets.length;i++){
      ...
      parts.push('<text x="' + cx + '" y="' + (y - 6) + '" text-anchor="middle" fill="var(--guest-fg)" font-size="11">' + esc(String(buckets[i].v)) + '</text>');
      parts.push('<text x="' + cx + '" y="' + (h - 18) + '" text-anchor="middle" fill="var(--guest-muted)" font-size="11">' + esc(label) + '</text>');
    }
  ...
  function renderLineSvg(buckets){
    ...
    for (i=0;i<buckets.length;i++){
      ...
      dots.push('<text x="' + x + '" y="' + (y - 10) + '" text-anchor="middle" fill="var(--guest-fg)" font-size="11">' + esc(String(buckets[i].v)) + '</text>');
      var label = buckets[i].k.length > 14 ? capStr(buckets[i].k, 13) + '...' : capStr(buckets[i].k, 14);
      dots.push('<text x="' + x + '" y="' + (h - 18) + '" text-anchor="middle" fill="var(--guest-muted)" font-size="11">' + esc(label) + '</text>');
    }
  ...
  function renderPieSvg(buckets, donut){
    ...
    var center = donut
      ? '<text x="' + cx + '" y="' + (cy + 5) + '" text-anchor="middle" fill="var(--guest-fg)" font-size="18" font-weight="600">' + esc(String(Math.round(total * 100) / 100)) + '</text>'
      : '';
