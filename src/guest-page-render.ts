 ...
 *   and per-row display maps of preformatted strings.
 ...
 *
 * Cell text can never emit "[object Object]": v2 uses row.display[slug];
 * otherwise primitives stringify, arrays of primitives join with ", ", and
 * objects fall back to a name/label/title/display/value/id string property
 * or empty string.
 */
const GUEST_RENDER_CORE_JS = `
  ...
  function rowCellText(row, slug){
    if (!slug) return '';
    var disp = row && row.display && typeof row.display === 'object' && !Array.isArray(row.display) ? row.display[slug] : undefined;
  ...
  function cellHtml(field, row){
    ...
    if (t === 'url') {
      var u = text.trim();
      if (isSafeHttpUrl(u)) return '<a href="' + esc(u) + '" target="_blank" rel="noopener noreferrer">' + esc(u) + '</a>';
      return esc(text);
    }
    if (t === 'email') {
      var em = text.trim();
  ...
  function renderTableHtml(content){
    ...
    var thead = '<tr>' + cols.map(function(c){ return '<th>' + esc(c.name || c.slug) + '</th>'; }).join('') + '</tr>';
  ...
  function renderListHtml(content, wrapClass){
    ...
    return '<div class="' + wrap + '">' + rows.map(function(r){
      ...
      for (var i=0;i<cols.length;i++){
        ...
        if (val === '') continue;
        kv += '<div class="kv"><div class="kv-k">' + esc(c.name || c.slug) + '</div><div class="kv-v">' + val + '</div></div>';
      }
  ...
  function renderBarSvg(buckets){
    ...
    for (i=0;i<buckets.length;i++){
      ...
      var cx = x + bw / 2;
      var label = buckets[i].k.length > 14 ? buckets[i].k.slice(0, 13) + '...' : buckets[i].k;
  ...
  function csvFromContent(content){
    var cols = visibleFields(content);
    var rows = Array.isArray(content && content.rows) ? content.rows : [];
    var lines = [cols.map(function(c){ return csvCell(c.name || c.slug); }).join(',')];
    rows.forEach(function(r){
