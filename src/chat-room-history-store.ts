...
import { exactChatMessageId } from './chat-frame-identity.js'
import { chatBodyMentionsMember } from './chat-mentions.js'
import {
  existsSync,
  mkdirSync,
  appendFileSync,
...
/**
 * Internal working window, never a room's lifetime message limit. History
 * and search walk backward through these windows and reconcile later edits
 * against the full log while retaining only a bounded set of message bodies.
 */
const MAX_TAIL_LINES_SCAN = 50_000
const SEARCH_TERM_POSTING_CAP = 500
...
export class ChatRoomHistoryStore {
  ...
  // IDs are global (including the cache), while disk writes queue per room.
  // Reserve before the first await so two rooms cannot create the same ID.
  private pendingAppendIds = new Map<string, { room: string; count: number }>()
  private recentById = new Map<string, ChatMessageRow>()
  // Async reads may finish after a write/prune. Never warm the mutation
  // cache from such a stale snapshot, including when prune evicted its ID.
  ...
  private findByIdCached(id: string): ChatMessageRow | null {
    const key = exactChatMessageId(id)
    if (!key) return null
   ...
   * restrict lookup to the caller-supplied `roomHint` (desktop sends the
   * owning room on edit/react/unsend frames). Only legacy calls without a
   * room may use the capped cross-room scan. Found rows are remembered so
   * repeat lookups for the same id are cache hits again.
   */
  async findById(id: string, roomHint?: string): Promise<ChatMessageRow | null> {
    const key = exactChatMessageId(id)
    if (!key) return null
    const parsedHint = typeof roomHint === 'string' ? parseChatRoomId(roomHint) : null
    if (parsedHint && !parsedHint.ok) return null
    const hint = parsedHint?.ok ? parsedHint.room : ''
    ...
    const current = this.touchRecent(key)
    if (current && (!hint || current.room === hint)) return current
    if (found) {
      bumpChatMetric('findByIdDiskHits')
      if (readEpoch === this.mutationEpoch) this.remember(found)
    }
    return found
  }
  ...
  /** A bounded physical window; endLine lets callers continue toward the
   * beginning without loading the whole log into memory. */
  private async readTailLines(path: string, endLine?: number): Promise<{ lines: string[]; totalLines: number }> {
    const ring: string[] = []
    let totalLines = 0
    ...
    for await (const line of rl) {
      ...
      ring[totalLines % MAX_TAIL_LINES_SCAN] = line
      totalLines++
    }
    const head = totalLines > MAX_TAIL_LINES_SCAN ? totalLines % MAX_TAIL_LINES_SCAN : 0
    const lines = head === 0 ? ring : [...ring.slice(head), ...ring.slice(0, head)]
    return { lines, totalLines }
  }

  /** Full scoped lookup with constant memory, including IDs older than the hot window. */
  ...
  private async findByIdDiskFallback(
    ...
    if (typeof roomHint === 'string' && roomHint) {
      ...
      // An explicit room is an isolation boundary, never a search preference.
      return null
    }
    let names: string[]
    try {
  ...
  private updateIndexOnAppend(
    ...
    for (const term of nextTerms) {
      ...
      if (list.length > SEARCH_TERM_POSTING_CAP) list.shift()
    }
  ...
  async append(input: {
    ...
    const role: BridgeRole =
      input.role === 'admin' || input.role === 'member' || input.role === 'viewer'
        ? input.role
        : 'member'
    if (input.id != null && input.id !== '' && !exactChatMessageId(input.id)) return { error: 'Invalid message id' }
    if (input.replyToId != null && input.replyToId !== '' && !exactChatMessageId(input.replyToId)) return { error: 'Invalid reply message id' }
    const id = exactChatMessageId(input.id) || mintChatId()
    const replyToId =
    ...
    const pending = this.pendingAppendIds.get(id)
    if (pending && pending.room !== parsed.room) return { error: 'Message id already used in another room' }
    const claim = pending ?? { room: parsed.room, count: 0 }
    claim.count++
    this.pendingAppendIds.set(id, claim)
    return this.queueRoomWrite(parsed.room, async () => {
      // Message IDs are team-global; creation must also detect another room.
      const existing = await this.findById(id)
      if (input.authorize && !input.authorize()) return { error: 'Session is no longer authorized' }
      if (existing) {
      ...
      if (--claim.count === 0) this.pendingAppendIds.delete(id)
    })
  }
  ...
  async edit(
    ...
    const id = exactChatMessageId(messageId)
    ...
    return this.queueRoomWrite(prevPeek.room, async () => {
      // Prune may remove the row (and evict its cache entry) while waiting.
      // The pre-queue peek is never an authoritative fallback for a write.
      const prev = await this.findById(id, prevPeek.room)
      if (authorize && !authorize()) return { error: 'Session is no longer authorized' }
      if (!prev || prev.room !== prevPeek.room) return { error: 'Message not found' }
      ...
      const row: ChatMessageRow = {
        ...prev,
        body: capChatText(body, CHAT_BODY_MAX),
        editedAt: Math.max(Date.now(), (prev.editedAt ?? 0) + 1),
      }
  ...
  async react(
    ...
    return this.queueRoomWrite(prevPeek.room, async () => {
      ...
      const row: ChatMessageRow = { ...prev, reactions, lastReactAt: Math.max(Date.now(), (prev.lastReactAt ?? 0) + 1) }
      const roomDir = this.ensureRoomDir(prev.room)
      if (typeof roomDir !== 'string') return roomDir
      const written = this.appendLineSync(roomDir, row)
      if ('error' in written) return written
  ...
  async pinMessage(
    ...
    return this.writePins(roomId, messageId, byAdmin, 'pin', authorize)
  }
  ...
  async unpinMessage(
    ...
    return this.writePins(roomId, messageId, byAdmin, 'unpin', authorize)
  }
  ...
  async setPinned(
    ...
    return typeof messageId === 'string' && messageId.trim()
      ? this.pinMessage(roomId, messageId, byAdmin, authorize)
      : this.unpinMessage(roomId, null, byAdmin, authorize)
  }
  ...
  private async writePins(
    ...
    if (!parsed.ok) return { error: parsed.reason }
    if (messageId !== null && !exactChatMessageId(messageId)) return { error: 'Invalid message id' }
  ...
  async softDelete(
    ...
    roomHint?: string,
    authorize?: () => boolean,
    validateRow?: (row: ChatMessageRow) => string | null,
    ...
    return this.queueRoomWrite(room, async () => {
      // Re-read under the room lock: the peek may predate prune or delete.
      const prev = await this.findById(id, room)
      if (authorize && !authorize()) return { error: 'Session is no longer authorized' }
      if (!prev || prev.room !== room) return { error: 'Message not found' }
      if (typeof prev.deletedAt === 'number' && prev.deletedAt > 0) {
        // TCC-R1151-CHAT-011: idempotent tomb - ok to caller, no second fanout.
        return { unchanged: true as const, tomb: prev }
      }
      const rowError = validateRow?.(prev)
  ...
  async authorUnsend(
    ...
    // The queue can outlive the unsend window, or a prune/replacement can
    // change the row after the initial peek. Check the committed row again.
    return this.softDelete(id, mid, prev.room, authorize, (current) => {
      if (!isAdmin && current.memberId !== mid) return 'Only the author can unsend this message'
      if (!isAdmin && Date.now() - (current.createdAt ?? 0) > CHAT_UNSEND_MS) {
        return 'Unsend window expired (15 minutes)'
      }
      return null
    })
  }
  ...
  /** Scan bounded windows so an old unread mention is not lost behind the
   * newest history page. Count each message in its original log window;
   * scanRoomFile resolves its latest edit/deletion across all windows. */
  async readUnreadSummary(room: string, memberId: string, watermark: number): Promise<{ unread: number; unreadTruncated: boolean; unreadMentions: number }> {
    const empty = { unread: 0, unreadTruncated: false, unreadMentions: 0 }
    const parsed = parseChatRoomId(room)
    if (!parsed.ok || !memberId) return empty
    const dir = this.roomDirFor(parsed.room)
    if (!dir || !existsSync(messagesPath(dir))) return empty
    const wm = Number.isFinite(watermark) ? Math.max(0, watermark) : 0
    let unread = 0, unreadMentions = 0, endLine: number | undefined
    do {
      const scan = await this.scanRoomFile(messagesPath(dir), parsed.room, { endLine })
      for (const [id, row] of scan.byId) {
        const origin = scan.seqById.get(id)
        if (origin === undefined || origin < scan.windowStartLine) continue
        if (row.kind === 'system' || row.deletedAt || row.memberId === memberId || row.createdAt <= wm) continue
        unread++
        if (chatBodyMentionsMember(row.body, memberId)) unreadMentions++
      }
      if (!scan.scanTruncated) break
      endLine = scan.windowStartLine
    } while (endLine > 0)
    return { unread: Math.min(unread, 50), unreadTruncated: unread > 50, unreadMentions }
  }
  ...
  async readRecent(
    ...
    const readEpoch = this.mutationEpoch
    const selected = new Map<string, ChatMessageRow>()
    const order = new Map<string, number>()
    let endLine: number | undefined
    let beforeSeq: number | undefined
    let more = false
    do {
      const scan = await this.scanRoomFile(path, parsed.room, { endLine, cursorId: beforeMsgId || undefined })
      if (beforeSeq === undefined) beforeSeq = scan.cursorSeq
      for (const [id, row] of scan.byId) {
        if (row.deletedAt && row.deletedAt > 0) continue
        if (beforeAt !== null && !isBeforeCreatedAtSeqCursor(row.createdAt ?? 0, id, beforeAt, beforeMsgId, beforeSeq, scan.seqById)) continue
        selected.set(id, row)
        const seq = scan.seqById.get(id)
        if (seq !== undefined) order.set(id, seq)
      }
      const ranked = [...selected.values()].sort((a, b) => compareByCreatedAtThenSeq(a, b, order))
      selected.clear()
      for (const row of ranked.slice(-(lim + 1))) selected.set(row.id, row)
      for (const id of order.keys()) if (!selected.has(id)) order.delete(id)
      more = scan.scanTruncated
      endLine = scan.windowStartLine
      // Later log windows can consist entirely of edits to old messages.
      // Visit every older window to rank by original creation, not update order.
      if (!more) break
    } while (endLine > 0)
    const list = [...selected.values()].sort((a, b) => compareByCreatedAtThenSeq(a, b, order))
    const truncated = list.length > lim || more
    ...
    if (readEpoch === this.mutationEpoch) for (const row of page) this.remember(row)
    return { messages: page, truncated }
  }

  /**
   * CJK / index-miss fallback traverses bounded windows across the whole
   * history, including matches older than the most recent page.
   */
  private async searchRoomByIncludes(
    ...
    const readEpoch = this.mutationEpoch
    const matches = new Map<string, ChatMessageRow>()
    const order = new Map<string, number>()
    let endLine: number | undefined
    let more = false
    do {
      const scan = await this.scanRoomFile(path, room, endLine === undefined ? undefined : { endLine })
      for (const row of scan.byId.values()) {
        if (row.deletedAt && row.deletedAt > 0) continue
        const body = String(row.body || '').toLowerCase()
        const tokens = terms?.length ? new Set(tokenizeForSearch(body)) : undefined
        if (!body.includes(q) && !(tokens && terms!.every(term => tokens.has(term)))) continue
        matches.set(row.id, row)
        order.set(row.id, scan.seqById.get(row.id) ?? 0)
      }
      const ranked = [...matches.values()].sort((a, b) => compareByCreatedAtThenSeq(a, b, order))
      matches.clear()
      for (const row of ranked.slice(-(lim + 1))) matches.set(row.id, row)
      for (const id of order.keys()) if (!matches.has(id)) order.delete(id)
      more = scan.scanTruncated
      endLine = scan.windowStartLine
      if (!more) break
    } while (endLine > 0)
    const page = [...matches.values()].sort((a, b) => compareByCreatedAtThenSeq(b, a, order)).slice(0, lim)
    if (readEpoch === this.mutationEpoch) for (const row of page) this.remember(row)
    return { messageIds: page.map(row => row.id), truncated: matches.size > lim || more }
  }
  ...
  async searchRoom(
    ...
    // A full posting list may have evicted the older match from a common
    // term while its unique term remains indexed. Intersecting those lists
    // would report a false negative. Scan the bounded history and preserve
    // AND-term matching plus scan-truncation honesty instead.
    if (sets.some(set => set.size >= SEARCH_TERM_POSTING_CAP)) {
      return this.searchRoomByIncludes(parsed.room, q, limit, terms)
    }
  ...
  async jumpToMessage(
    ...
    if (found && readEpoch === this.mutationEpoch) this.remember(found)
  ...
  /**
   * Read one physical window and reconcile each included ID against its
   * original append order and latest saved state. scanTruncated means an
   * earlier physical window remains; callers must visit it before claiming
   * an exhaustive history/search result. It is not a user-visible page cap.
   */
  private async scanRoomFile(
    path: string,
    room: string,
    windowOptions?: { cursorId?: string; endLine?: number },
  ): Promise<{ byId: Map<string, ChatMessageRow>; seqById: Map<string, number>; scanTruncated: boolean; windowStartLine: number; cursorSeq?: number }> {
    const { lines: window, totalLines } = await this.readTailLines(path, windowOptions?.endLine)
    ...
    // A window may contain a late edit of an old message. Recover the original
    // ordering and newest state (including edits/deletes beyond this window)
    // without retaining bodies or IDs from the rest of the room.
    let cursorSeq = windowOptions?.cursorId ? seqById.get(windowOptions.cursorId) : undefined
    if (totalLines > MAX_TAIL_LINES_SCAN || windowOptions?.endLine !== undefined) {
      seqById.clear()
      cursorSeq = undefined
      let seq = 0
      const lines = createInterface({ input: createReadStream(path, { encoding: 'utf8' }) })
      for await (const line of lines) {
        ...
        const at = seq++
        try {
          const row = JSON.parse(decryptOpsLine(this.atRest, line)) as ChatMessageRow
          if (row.room !== room) continue
          if (row.id === windowOptions?.cursorId && cursorSeq === undefined) cursorSeq = at
          if (!byId.has(row.id)) continue
          if (!seqById.has(row.id)) seqById.set(row.id, at)
          if ((row.createdAt ?? 0) >= (byId.get(row.id)?.createdAt ?? 0)) byId.set(row.id, row)
        } catch { /* skip corrupt line */ }
      }
    }
    return { byId, seqById, scanTruncated: totalLines > MAX_TAIL_LINES_SCAN, windowStartLine: Math.max(0, totalLines - window.length), cursorSeq }
  }

  async prune(): Promise<number> {
    // Keep-forever rooms need no hourly rewrite of their entire log. In
    // particular, never expire a tombstone while its original row is retained:
    // doing so would make a deleted message visible again after a restart.
    ...
    const tombCutoff =
      this.tombstoneDays <= 0 ? 0 : Math.min(msgCutoff, Date.now() - this.tombstoneDays * 24 * 60 * 60 * 1000)
  ...
  private async pruneRoomFile(
    ...
    for await (const line of rl) {
      ...
      if (isTomb) {
        if (tombCutoff > 0 && row.deletedAt! < tombCutoff) {
          removed++
          continue
        }
  ...
  private runMigrationIfNeeded(): void {
    ...
    try {
      ...
      throw new Error('Chat migration could not back up legacy history; source preserved. Repair storage and restart.')
    }
    let legacyCount = 0
    let migratedCount = 0
    let raw: string
    try {
      ...
      if (st.length > 50 * 1024 * 1024) {
        throw new Error('Legacy history exceeds the synchronous migration limit')
      }
      raw = st.toString('utf8')
    } catch {
      throw new Error('Chat migration could not read legacy history; source preserved. Repair storage or migrate manually before restarting.')
    }
    // A prior import can have committed some rows before losing power or
    // failing to quarantine its source. Existing per-room rows are newer
    // authority: replaying old snapshots must not undo edits or tombstones.
    const existingByRoom = new Map<string, Set<string>>()
    const latestLegacy = new Map<string, { row: ChatMessageRow; lines: number }>()
    for (const line of raw.split('\n')) {
      ...
      try { plain = decryptOpsLine(this.atRest, line) } catch {
        throw new Error('Chat migration could not decrypt legacy history; source preserved. Verify the encryption key before restarting.')
      }
      try {
        row = JSON.parse(plain) as ChatMessageRow
        if (!row?.id || !row.room) continue
        const key = JSON.stringify([row.room, row.id])
        latestLegacy.set(key, { row, lines: (latestLegacy.get(key)?.lines ?? 0) + 1 })
      } catch {
        // Malformed source lines remain recoverable in the complete backup.
        continue
      }
    }
    // Commit only the last snapshot of each legacy ID. A retry can then
    // safely skip an already imported ID, even after a mid-import crash.
    for (const { row, lines } of latestLegacy.values()) {
      try {
        const roomDir = this.ensureRoomDir(row.room)
        if (typeof roomDir !== 'string') continue
        let existing = existingByRoom.get(row.room)
        if (!existing) {
          existing = new Set<string>()
          const path = messagesPath(roomDir)
          if (existsSync(path)) {
            if (statSync(path).size > 50 * 1024 * 1024) throw new Error('Existing history requires manual migration')
            this.repairTornMessagesTail(roomDir)
            for (const current of readFileSync(path, 'utf8').split('\n')) {
              if (!current.trim()) continue
              const saved = JSON.parse(decryptOpsLine(this.atRest, current)) as ChatMessageRow
              if (!saved.id || saved.room !== row.room) throw new Error('Invalid existing room history')
              existing.add(saved.id)
            }
          }
          existingByRoom.set(row.room, existing)
        }
        if (existing.has(row.id)) { migratedCount += lines; continue }
        ...
        throw new Error('Chat migration write failed; source preserved. Repair storage and restart.')
      }
    }
    ...
    try {
      ...
      throw new Error('Chat migration could not retire its source; source preserved. Repair storage and restart.')
    }
  }
}
