...
/** Whole-team field/table grants: revision is minted only by the bridge. */
export type TeamFieldAclAuthority = { revision: number; hash: string; bag: unknown | null }

/**
 * Machine-readable permanent hello refusals. The bridge emits these only
 * after an exact match against a previously-valid revoked bearer session;
 * ordinary invalid credentials intentionally remain uncoded.
 */
export type HelloRefuseCode = 'membership_revoked' | 'session_revoked'

/**
 * Server-authoritative content safety lock for the Team Space sync surface.
 * This is deliberately separate from desktop subscription/entitlement locks.
 * It covers shared Modules and custom Playbooks only.
 */
export type TeamContentLockSnapshot = {
  teamId: string
  locked: boolean
  revision: number
  changedAt: number
  ...
  reason?: string
  /** A damaged/unreadable durable lock file is represented as locked. */
  unavailable?: boolean
}
...
const MODULES_SYNC_OP_KINDS = [
  'record.create', 'record.update', 'record.delete', 'record.teamwork',
  ...
  // Custom project Playbooks share the same durable ops log and desktop
  // apply pipeline as Modules. Official catalog Playbooks are filtered at
  // the desktop enqueue boundary and never ride this wire.
  'playbook.create', 'playbook.update', 'playbook.delete',
  // Compose snapshots/deletes use the same durable log; live graph edits still
  // travel over the separately ACL-gated Yjs channel.
  'compose.doc.create', 'compose.doc.delete',
  'compose.access', // server-minted revision notification, never client-authored
  'cascade.patch',
  // Authoritative, Admin-minted per-recipient lifecycle revokes. Older module
  // notices without the destructive bit remain notice-only on the desktop.
  'module.share_revoked', 'playbook.share_revoked', 'compose.share_revoked',
] as const
...
export type ModulesSyncOp = {
  /** Server-owned workflow projection; client values are always replaced at admission. */
  teamwork?: { state: import('./record-teamwork-types.js').RecordTeamworkState; event?: import('./record-teamwork-types.js').RecordTeamworkEvent }
  ...
  /** Safety-lock revision captured when this edit was authored. */
  lockRevision?: number
  /** Current item ACL revision captured at edit time (independent of the team lock). */
  contentAclRevision?: number
  /** Server-resolved item identity; never a client-provided grant. */
  contentAclItem?: string
  /** Exact prior grant bag observed when this permission change was authored. */
  fieldAclBaseHash?: string
  /** Bridge-owned ordering, independent of a desktop's optimistic HLC. */
  fieldAclRevision?: number
  originMemberId?: string
  originMemberName?: string
  /** Authenticated bridge team; both aliases overwrite client-supplied routing. */
  teamId?: string
  team_id?: string
  /**
...
export type BridgeFrame =
  | { type: 'workspace_work_changed'; workspaceId: string }
  | {
      type: 'hello'
      /** Opt in to one durable catch-up frame at a time; keeps control traffic responsive. */
      catchupAckWindow?: boolean
      /** Restored local databases need permitted history even if previously acknowledged. */
  ...
  | {
      ...
      /** Optional so older protocol-v2 clients safely ignore the new field. */
      teamContentLock?: TeamContentLockSnapshot
      /** Admin only: other members must not discover unshared schema IDs. */
      fieldAclAuthority?: TeamFieldAclAuthority
      /** Opaque current edit baseline; contains no private grant metadata. */
      fieldAclBaseline?: { revision: number; hash: string }
    }
  | { type: 'field_acl_baseline'; teamId: string; fieldAclBaseline: { revision: number; hash: string } }
  | {
      type: 'chat_caps'
      chatCaps: {
        voiceMessageMaxSec: number
  ...
  | { type: 'hello_refuse'; reason: string; code?: HelloRefuseCode }
  | {
      type: 'invite_create'
      frameId: string
      email?: string
  ...
  | {
      ...
      token: string
      deviceId: string
      /** Optional secret 256-bit attempt proof; exact retry never creates a new grant. */
  ...
  | { type: 'team_content_lock_get'; frameId: string }
  | {
      type: 'team_content_lock_set'
      frameId: string
      locked: boolean
  ...
  | {
      ...
      /** False for an idempotent replay or a request that already matched. */
      changed?: boolean
      idempotent?: boolean
    }
  | {
  ...
  | { type: 'team_content_restore_begin'; frameId: string; expectedRevision: number }
  | { type: 'team_content_restore_validate'; frameId: string; permitToken: string }
  | { type: 'team_content_restore_finish'; frameId: string; permitToken: string }
  | {
      type: 'team_content_restore_permit'
      frameId: string
      permitToken: string
      lockRevision: number
      expiresAt: number
  ...
  | {
      type: 'team_content_restore_refuse'
      frameId: string
      reason: string
      state?: TeamContentLockSnapshot
    }
  ...
  | { type: 'ping'; t: number }
  | { type: 'pong'; t: number; serverTimeMs?: number }
  | { type: 'catchup_processed'; frameId: string }
  ...
  | { type:'assignment_snapshot_request';teamId:string;moduleId:string;entityId:string;recordId:string;fieldId:string }
  | {
  ...
  | {
      /** Atomic bounded batch variant. `kick_member` remains supported. */
      type: 'kick_members'
      frameId: string
      memberIds: string[]
    }
  ...
  | {
      ...
      frameId: string
      memberId: string
      /** True only when roster/session removal succeeded but chat cleanup threw. */
      chatCleanupFailed?: boolean
      /** True when canonical explicit Compose ACL cleanup could not persist. */
      contentAclCleanupFailed?: boolean
    }
  | {
      ...
      members: Array<{
        memberId: string
        /** Device identifiers only; session tokens/hashes are never exposed. */
        kickedDeviceIds: string[]
        kickedDeviceIdsTotal: number
        deviceIdsTruncated: boolean
      }>
      /** Roster/session removal succeeded; these chat cleanups need operator attention. */
      chatCleanupFailedMemberIds?: string[]
      /** Roster/session removal succeeded; canonical Compose ACL cleanup needs attention. */
      contentAclCleanupFailedMemberIds?: string[]
    }
  | {
      type: 'leave_team'
      frameId: string
  ...
  | {
      ...
      updateB64: string
      /** Safety-lock revision captured when this update was authored. */
      lockRevision?: number
    }
  | {
      type: 'yjs_ok'
      ...
      /** Durable accepted module state, included on an authorized join. */
      acceptedStateB64?: string
    }
  | {
      type: 'yjs_refuse'
      frameId: string
  ...
  | { type: 'presence_edit'; frameId: string; target: import('./module-edit-presence.js').ModuleEditTarget; active: boolean }
  ...
  | {
      ...
      peers: Array<{
        ...
        deviceId: string
        lastSeen: number
        editing?: import('./module-edit-presence.js').ModuleEditLease[]
        avatarRef?: string
      }>
