...
import {GuestSubmissionReceipts,SubmissionReceiptError,type SubmissionReplay} from './guest-submission-receipts.js'
/**
...
import { GuestTokenRetirementStore } from './guest-token-retirement.js'
import { readGuestFieldAclPayload, stampGuestFieldAclPayload, type GuestFieldAclGuard } from './guest-field-acl.js'
...
export type PublicSharePayload = {
  ...
  viewConfig?: {
    ...
    calendarRange?: 'month' | 'week' | 'day' | 'agenda'
    startZoom?: number
    columns?: 2 | 3 | 4
  ...
  rows: Array<{ id: string; data: Record<string, unknown>; display?: Record<string, string>; parent_id?: string | null; sort_order?: number }>
  events?: Array<{ id: string; recordId: string; kind: string; actorName: string; summary: string; createdAt: string; payload: Record<string, unknown> }>
...
export class PublicShareBridgeStore {
  private fieldAclGuard: GuestFieldAclGuard = () => true
  setFieldAclReadGuard(guard: GuestFieldAclGuard): void { this.fieldAclGuard = guard }
  private readonly registryPath: string
  private readonly payloadDir: string
  private readonly submissionsDir: string
  private readonly submissionReceipts: GuestSubmissionReceipts
  private readonly atRest: AtRestKey | null
  private readonly retiredTokens: GuestTokenRetirementStore
  ...
  constructor(dataDir: string, atRest: AtRestKey | null) {
    this.atRest = atRest
    this.retiredTokens = new GuestTokenRetirementStore(join(dataDir, 'public-share-retired-tokens'), atRest)
    ...
    this.submissionReceipts = new GuestSubmissionReceipts(join(dataDir, 'share-submission-receipts'), atRest)
  ...
  private saveRegistry(shares: PublicShareRow[]): void {
    retainIndependentCheckpoint(dirname(this.registryPath),'public-shares.json')
    const now = Date.now()
    ...
    for (const row of dead) {
      this.retiredTokens.retire(row.tokenHash)
    ...
    for (const row of dropped) this.retiredTokens.retire(row.tokenHash)
  ...
  upsertShare(
    ...
    if (prev && isShareRevoked(prev)) this.retiredTokens.retire(prev.tokenHash)
    if (this.retiredTokens.isRetired(tokenHash)) {
      return { ok: false, error: 'Public share token was revoked; create a new link' }
    }
    ...
    if (args.payload !== undefined) {
      const wrote = this.writePayloadFile(tokenHash, stampGuestFieldAclPayload(args.payload, args.fieldAclBaseHash))
    ...
    if (prev && prev.tokenHash !== tokenHash) this.retiredTokens.retire(prev.tokenHash)
  ...
  revokeShare(args: {
    ...
    const idx = found.idx
    const row = idx >= 0 ? shares[idx] : null
    const validHash = /^[0-9a-f]{64}$/.test(hash)
    const alreadyRetired = validHash && this.retiredTokens.isRetired(hash)
    if (validHash) this.retiredTokens.retire(hash)
    if (row && validHash && row.tokenHash !== hash && alreadyRetired) return { ok: true }
    if (idx < 0) {
      // Mint and revoke can arrive in reverse order. Persist the token's
      // revocation before acknowledging even when register has not arrived.
      if (!localId || !/^[0-9a-f]{64}$/.test(hash)) {
        return { ok: false, error: 'Share not found' }
      }
      return { ok: true }
    }
    const now = Date.now()
    this.retiredTokens.retire(shares[idx].tokenHash)
  ...
  setPayload(
    ...
    const tokenHash =
      typeof args.tokenHash === 'string' ? args.tokenHash.toLowerCase().trim() : ''
    if (/^[0-9a-f]{64}$/.test(tokenHash) && this.retiredTokens.isRetired(tokenHash)) {
      return { ok: false, error: 'Share is not active' }
    }
    const localId = capGuestStoreScopeId(args.localShareId, GUEST_STORE_LOCAL_SHARE_ID_MAX)
    const callerTeam = resolveGuestStoreStamp(undefined, args.teamId ?? args.team_id) ?? ''
    const loaded = this.loadRegistryResult()
    ...
    const wrote = this.writePayloadFile(row.tokenHash, stampGuestFieldAclPayload(args.payload, args.fieldAclBaseHash))
    if (!wrote.ok) return wrote
    const idx = found.idx
    if (idx >= 0) {
      shares[idx] = {
  ...
  readPayload(tokenHash: string): PublicSharePayload | null {
    ...
    const data = readGuestFieldAclPayload(readJson<unknown>(path, null, this.atRest), this.fieldAclGuard)
    if (!isPayload(data)) return null
    // Also protect payloads published by older desktops, including cached links
    // whose owner is offline. Internal record metadata is never a public field.
    for (const row of data.rows) {
      for (const bag of [row?.data, row?.display]) {
        if (!bag || typeof bag !== 'object') continue
        for (const key of Object.keys(bag)) if (key.startsWith('_')) delete bag[key]
      }
    }
    return data
  }
  ...
  resolveGuest(tokenPlain: string): {
    ...
    try {
      if (this.retiredTokens.isRetired(hash)) return { row, active: false, reason: 'revoked' }
    } catch {
      return { row: null, active: false, reason: 'unreadable' }
    }
  ...
  enqueueSubmission(args: {
    ...
    let replay: SubmissionReplay | null
    try {
      replay = this.submissionReceipts.probe(args.row.tokenHash, args.submissionKey, args.rawData)
      if (replay?.complete || (replay?.exists && existsSync(join(this.submissionsDir, replay.id + '.json')))) {
        return { ok: true, id: replay.id }
      }
    } catch (error) {
      const failure = error instanceof SubmissionReceiptError ? error : new SubmissionReceiptError(503, 'retry_unavailable', 'Submission history is unavailable. Try again shortly.')
      return { ok: false, status: failure.status, error_code: "share." + failure.code, error: failure.message }
    }
    ...
    const id = replay?.id ?? randomBytes(16).toString('hex')
    const now = Date.now()
    ...
    // Reserve the stable identity before queueing. A crash in between can
    // retry the same identity; completion is recorded before removing the queue.
    if (replay) this.submissionReceipts.reserve(replay)
  ...
  listPendingSubmissions(
    ...
    for (const name of names) {
      ...
      if (this.submissionReceipts.isComplete(sub.id)) continue
  ...
  ackSubmission(
    ...
    try {
      this.submissionReceipts.complete(id)
      unlinkSync(path)
    } catch {
