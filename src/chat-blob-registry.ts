...
/** Stable refusal used when upload authority disappears before durable commit. */
export const CHAT_BLOB_UPLOAD_AUTH_REVOKED = 'Session is no longer authorized'

class ChatBlobUploadAuthorizationError extends Error {
  constructor() {
    super(CHAT_BLOB_UPLOAD_AUTH_REVOKED)
    this.name = 'ChatBlobUploadAuthorizationError'
  }
}

function chatBlobUploadAuthorized(authorize: (() => boolean) | undefined): boolean {
  if (!authorize) return true
  try { return authorize() === true } catch { return false }
}

function sameOptionalStringList(a: string[] | undefined, b: string[] | undefined): boolean {
  if (!a || !b) return a === b
...
export class ChatBlobRegistry {
  ...
  private queuePersist(
    ...
    const attempt = this.writeChain.then(() => {
      // Uploads can wait behind another registry write after their HTTP body
      // and malware scan have both completed. Re-check in the serialized
      // persistence turn so a kick, session revoke, role downgrade, or room
      // removal cannot commit bytes using authority captured earlier.
      if (!chatBlobUploadAuthorized(authorize)) {
        // Roll back in this same serialized turn, before writeChain releases
        // the next registry persist. Otherwise another caller could persist
        // this upload's uncommitted in-memory row in the gap before the
        // awaiting registerUpload continuation gets to run.
        try { onAuthorizationLost?.() } catch { /* best-effort rollback below repeats */ }
        throw new ChatBlobUploadAuthorizationError()
      }
      this.persistSync()
    })
  ...
  async registerUpload(input: {
    ...
    bytes: Uint8Array
    maxBytes?: number
    /** Exact live session + room ACL check, evaluated at durable commit time. */
    authorize?: () => boolean
    ...
    // Hash before storage quota checks; duplicate uploads still require a
    // current scan before publishing another reference to the same bytes.
    ...
    const gate = new Promise<void>((resolveGate) => {
      this.uploadChain = this.uploadChain.then(async () => {
        try {
          if (!chatBlobUploadAuthorized(input.authorize)) {
            result = { error: CHAT_BLOB_UPLOAD_AUTH_REVOKED }
            return
          }
          // A deduplicated upload is still a new exposure. Re-scan with the
          // current engine/signatures before granting another room access.
          const scan = await runChatAttachmentScan(input.bytes, name)
          if (!scan.ok) { result = { error: scan.reason }; return }
          if (!chatBlobUploadAuthorized(input.authorize)) { result = { error: CHAT_BLOB_UPLOAD_AUTH_REVOKED }; return }
          const existing = this.bySha.get(sha)
          if (existing) {
            const prior = {
              lastRefAt: existing.lastRefAt,
              roomIds: existing.roomIds ? [...existing.roomIds] : undefined,
              name: existing.name,
              mime: existing.mime,
            }
            let dirty = false
            ...
            if (dirty) {
              ...
              const rollback = () => {
                // Do not overwrite a newer legitimate touch that happened
                // while this upload waited behind another registry persist.
                if (existing.lastRefAt === applied.lastRefAt) existing.lastRefAt = prior.lastRefAt
                if (sameOptionalStringList(existing.roomIds, applied.roomIds)) {
                  if (prior.roomIds) existing.roomIds = [...prior.roomIds]
                  else delete existing.roomIds
                }
                if (existing.name === applied.name) existing.name = prior.name
                if (existing.mime === applied.mime) existing.mime = prior.mime
              }
              try {
                await this.queuePersist(input.authorize, rollback)
              } catch (err) {
                if (err instanceof ChatBlobUploadAuthorizationError) {
                  rollback()
                  result = { error: CHAT_BLOB_UPLOAD_AUTH_REVOKED }
                  return
                }
          ...
          if (raced) {
            const prior = {
              lastRefAt: raced.lastRefAt,
              roomIds: raced.roomIds ? [...raced.roomIds] : undefined,
            }
            ...
            if (!chatBlobRoomIds(raced).includes(roomId)) {
              ...
              const rollback = () => {
                if (raced.lastRefAt === applied.lastRefAt) raced.lastRefAt = prior.lastRefAt
                if (sameOptionalStringList(raced.roomIds, applied.roomIds)) {
                  if (prior.roomIds) raced.roomIds = [...prior.roomIds]
                  else delete raced.roomIds
                }
              }
              try {
                await this.queuePersist(input.authorize, rollback)
                ...
                /* Preserve the established dedup behavior on an ordinary disk failure. */
              }
            }
            result = raced
            return
          ...
          try {
            ...
            rollback()
            result = {
              error: err instanceof ChatBlobUploadAuthorizationError
