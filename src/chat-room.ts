...
/** New teams keep history until deleted; saved Admin retention policies still apply. */
export const CHAT_RETENTION_DAYS_DEFAULT = 0
...
function isMemberToken(s: string): boolean {
  // The first native Admin is bootstrapped with a device:<uuid> identity.
  return s.length <= 128 && /^(?:device:)?[a-zA-Z0-9_-]+$/.test(s)
}
...
export function scrubChatBody(raw: unknown): string | null {
  ...
  // Over the send ceiling: refuse (never silent-truncate). At or under the
  // cap: still run capStr so a trailing lone lead surrogate or a mid-string
  // orphan is repaired (TS-CHAT-042). An early `return cleaned` lets a body
  // that is already exactly CHAT_BODY_MAX units long persist into history
  // and peer fanout.
  if (!cleaned || cleaned.length > CHAT_BODY_MAX) return null
  return capStr(cleaned, CHAT_BODY_MAX)
}
