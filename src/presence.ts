 ...
 */
import { liveModuleEditLeases, type ModuleEditLease } from './module-edit-presence.js'
import { capStr } from './text-cap.js'
import type { BridgeRole } from './index.js'

/**
 * Covers every live device under the default 200-socket server ceiling with
 * room for multi-window reconnect overlap, while keeping snapshots bounded.
 */
export const PRESENCE_SNAPSHOT_MAX = 512
...
export function normalizePresencePeer(raw: {
  ...
  return {
    ...
    deviceId,
    lastSeen,
    ...(Array.isArray(raw.editing) ? { editing: liveModuleEditLeases(raw.editing) } : {}),
    ...(typeof raw.avatarRef === 'string' && /^[a-f0-9]{64}$/.test(raw.avatarRef) ? { avatarRef: raw.avatarRef } : {}),
  }
}
