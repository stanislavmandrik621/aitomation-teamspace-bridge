...
export const GUEST_VIEW_PIVOT_JS = `
  ...
  function renderPivotHtml(content){
    ...
    var sticky = 'position:sticky;left:0;z-index:1;background:var(--guest-card);white-space:nowrap;';
    var stickyHead = 'position:sticky;left:0;z-index:2;background:var(--guest-hover);white-space:nowrap;text-align:left;';
    var numStyle = 'text-align:right;white-space:nowrap;font-variant-numeric:tabular-nums;';
    var muted = 'color:var(--guest-muted);';
    ...
    var valueName = valueUnavailable || !valueField
      ? 'records'
      : fieldLabel((valueField.name || valueField.slug) || 'records');
    var caption = '<p class="hint" style="margin:8px 12px 4px">'
      + esc(pivotAggLabel(agg) + ' of ' + valueName)
      + ' · rows: ' + esc(fieldLabel(rowField.name || rowField.slug))
      + ' · columns: ' + esc(fieldLabel(colField.name || colField.slug))
      + '</p>';
    var head = '<th style="' + stickyHead + '">' + esc(fieldLabel(rowField.name || rowField.slug)) + '</th>';
    var hj;
    ...
    var footBg = 'background:var(--guest-card);';
    ...
    return '<div class="card"><div class="pivot-wrap">' + notes + caption
      + '<table class="pivot-matrix" style="width:max-content;min-width:100%;font-size:12px"><thead style="background:var(--guest-hover)"><tr>' + head + '</tr></thead><tbody>' + body + '</tbody>'
