 ...
 * Saved columns and visible subtitle/badge bindings travel in the payload.
 * Column density follows the desktop container-width breakpoints.
...
export const GUEST_VIEW_GALLERY_JS = `
  ...
  function renderGalleryHtml(content){
    ...
    var density = String(cfg.columns == null ? '' : cfg.columns).trim();
    if (density !== '2' && density !== '3' && density !== '4') density = '4';
    return '<div class="guest-card-container"><div class="gallery guest-card-grid" data-grid-columns="' + density + '">' + rows.map(function(r){
      ...
      if (extra.length) {
        ...
        for (var ei = 0; ei < extra.length; ei++) {
          ...
          thumbs += '<div style="flex:1;min-width:0;height:40px;overflow:hidden;position:relative;background:var(--guest-input)">';
