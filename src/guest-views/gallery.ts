 ...
 * http(s) gate never sees a real src. Desktop `extractMediaUrls` +
 * `collectRecordImageUrlsDetailed` unwrap those shapes, take the explicit
 * image field first, then other image/file/url/text cells, de-dupe, and cap.
 * This snippet does the same, still gated by `isSafeHttpUrl` on every src
...
export const GUEST_VIEW_GALLERY_JS = `
  ...
  function guestGalleryHttpUrlsFromCell(row, slug, requireLikely){
    ...
    if (!extracted.length) {
      ...
      if (typeof disp === 'string' && disp.trim()) extracted.push(disp.trim());
    }
    var out = [];
  ...
  function renderGalleryHtml(content){
    ...
    return '<div class="gallery">' + rows.map(function(r){
      ...
      for (var mi = 0; mi < cols.length && shown < GALLERY_MAX_META; mi++) {
        ...
        if (val === '') continue;
        kv += '<div class="kv"><div class="kv-k">' + esc(c.name || slug) + '</div><div class="kv-v">' + val + '</div></div>';
        shown++;
      }
