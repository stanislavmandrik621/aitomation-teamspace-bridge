  ...
  function renderListHtml(content, wrapClass){
    ...
    function listFieldVal(field, row){
      ...
      if (t === 'auto_id') {
        ...
        if (id) return '<code style="font-size:10px;color:var(--guest-muted)">' + esc(id.slice(0, 8)) + '</code>';
      }
    ...
    function listShareCss(){
      return '<style>' +
        ...
        '.list-thumb{flex:0 0 36px;width:36px;height:36px;border-radius:8px;overflow:hidden;border:1px solid var(--guest-border)}' +
        ...
        '.list-desc{margin-top:2px;font-size:11px;color:var(--guest-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}' +
