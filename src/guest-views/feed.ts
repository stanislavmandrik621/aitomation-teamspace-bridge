...
export const GUEST_VIEW_FEED_JS = `
  ...
  function feedInitials(name){
    ...
    if (out.length === 1) {
      var one = out[0];
      var slice = (typeof Array.from === 'function') ? Array.from(one).slice(0, 2).join('') : one.slice(0, 2);
      return (slice || '?').toUpperCase();
    }
    return ((feedFirstChar(out[0]) + feedFirstChar(out[1])) || '?').toUpperCase();
  }
  function feedHue(str){
    var s = typeof str === 'string' ? str : (str == null ? '' : String(str));
    var h = 0;
    var i;
  ...
  function feedSensitiveField(slugOrName){
    var lower = String(slugOrName == null ? '' : slugOrName).toLowerCase();
  ...
  function feedEventCardHtml(ev){
    ...
    var commentSliced = false;
    if (comment.length > FEED_COMMENT_CAP) {
      comment = comment.slice(0, FEED_COMMENT_CAP);
      commentSliced = true;
    }
