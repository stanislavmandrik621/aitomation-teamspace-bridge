 ...
 *
 * Current native publications contain a field-filtered activity snapshot.
 * Older publishers may still send rows only; those retain an explicit fallback
 * note. Empty event snapshots never fabricate activity from records.
 *
 ...
 *
 * Kind chips filter the loaded window in CSS; counts describe that window.
 * Public pages poll their refreshed snapshot; mutation controls stay in the app.
 */
...
export const GUEST_VIEW_FEED_JS = `
  ...
  function feedEmptyHtml(){
    return '<div class="stack feed"><div class="card"><div class="empty">No activity yet.</div></div></div>';
  }
  ...
  function feedDaySepHtml(ms){
    return '<div class="feed-day-sep" aria-hidden="true" style="display:flex;align-items:center;gap:10px;padding-top:6px"><span class="muted" style="font-size:10px;font-weight:600;letter-spacing:.04em;text-transform:uppercase;flex:0 0 auto">' + esc(feedDayLabel(ms)) + '</span><span style="flex:1;height:1px;background:var(--guest-border)"></span></div>';
  }
  ...
  function feedChangeLine(ch){
    ...
    return '<div style="font-size:11px;color:var(--guest-muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-top:2px"><span style="font-weight:500;color:var(--guest-secondary)">' + esc(label) + '</span>: <span style="text-decoration:line-through;opacity:.6">' + esc(fromTxt) + '</span> -> <span>' + esc(toTxt) + '</span></div>';
  }
  function feedEventCardHtml(ev){
    ...
    var html = '<div class="row-card feed-ev" data-kind="' + esc(kind) + '" style="gap:10px;align-items:flex-start">';
    ...
    if (kind === 'comment' && comment) {
      html += '<div class="doc-body" style="margin-top:6px;background:var(--guest-input);border:1px solid var(--guest-border);border-radius:8px;padding:6px 8px;white-space:pre-wrap;overflow-wrap:anywhere">' + esc(comment) + (commentTrunc ? '...' : '') + '</div>';
    }
    if(kind==='activity'||kind==='time'){
      ['subject','body','outcome','note'].forEach(function(key){var value=feedStr(payload,key,key);if(value)html+='<div style="margin-top:6px;white-space:pre-wrap;overflow-wrap:anywhere">'+esc(value)+'</div>';});
      if(kind==='time'&&typeof payload.durationSeconds==='number')html+='<div class="muted">'+esc(String(Math.round(payload.durationSeconds/60)))+' minutes</div>';
    }
    if (changes.length) {
      var shown = 0;
      var i;
  ...
  function feedChipLabel(id, label, count, tilde){
    return '<label for="gfk-' + id + '" style="margin:0;cursor:pointer"><span class="feed-chip" style="display:inline-flex;align-items:center;height:28px;padding:0 10px;border:1px solid var(--guest-border);border-radius:8px;background:var(--guest-bg);color:var(--guest-muted);font-size:11px">' + esc(label) + '<span style="margin-left:4px;font-variant-numeric:tabular-nums;opacity:.7">' + (tilde ? '~' : '') + feedFmtCount(count) + '</span></span></label>';
  }
  function feedFilterCss(){
    var css='.feed-root .feed-ev{display:flex}.feed-root>.feed-kind-radio{position:absolute;width:1px;height:1px;opacity:0;overflow:hidden;clip:rect(0,0,0,0)}.feed-filter-empty{display:none;text-align:center;color:var(--guest-muted);font-size:12px;padding:32px 0}';
    ['all','created','updated','comment','activity','time'].forEach(function(kind){
      css+='.feed-root #gfk-'+kind+':checked~.feed-kind-bar label[for=gfk-'+kind+'] .feed-chip{border-color:#3b82f6;background:rgba(59,130,246,.12);color:var(--guest-fg)}';
      css+='.feed-root #gfk-'+kind+':focus-visible~.feed-kind-bar label[for=gfk-'+kind+'] .feed-chip{outline:2px solid #3b82f6;outline-offset:2px}';
      if(kind!=='all')css+='.feed-root #gfk-'+kind+':checked~.stack.feed .feed-ev:not([data-kind='+kind+']),.feed-root #gfk-'+kind+':checked~.stack.feed .feed-day:not([data-kinds*=" '+kind+' "]){display:none}.feed-root #gfk-'+kind+':checked~.stack.feed:not(:has(.feed-ev[data-kind='+kind+'])) .feed-empty-'+kind+'{display:block}';
  ...
  function feedRenderEventsHtml(content, events, fromRows){
    ...
    var counts = { all: shown, created: 0, updated: 0, comment: 0, activity:0, time:0 };
    var i, ev, kind;
    ...
    html += feedFilterCss();
    ['all','created','updated','comment','activity','time'].forEach(function(kind){html+='<input class="feed-kind-radio" type="radio" name="guest-feed-kind" id="gfk-'+kind+'"'+(kind==='all'?' checked':'')+'>';});
    html += !fromRows && truncated && total===shown ? '<p class="hint">Showing newest '+shown+' events. History is limited to the published records and loaded activity window.</p>' : feedCountHint(shown,total,truncated,noun);
    if (fromRows) {
    ...
    html += feedChipLabel('activity', 'Activity', counts.activity, truncated);
    html += feedChipLabel('time', 'Time', counts.time, truncated);
    html += '</div>';
    if (truncated) {
    ...
    html += '<div class="feed-filter-empty feed-empty-comment">No comment events in this feed.</div><div class="feed-filter-empty feed-empty-activity">No activities in this feed.</div><div class="feed-filter-empty feed-empty-time">No time events in this feed.</div>';
    var prevDay = null;
    var dayOpen = false;
    var kindsBag = ' ';
    var ms, day;
  ...
  function renderFeedHtml(content){
    ...
    var fromRows = false;
    if (!events.length && !Array.isArray(content && content.events)) {
