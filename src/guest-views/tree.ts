...
export const GUEST_VIEW_TREE_JS = `
  ...
  function guestTreeNormId(raw){
    return typeof raw === 'string' && raw.trim() ? capStr(raw.trim(), 128) : '';
  }
  ...
  function guestTreeMetaHtml(row, metaFields){
    ...
    for (i=0;i<metaFields.length;i++){
      ...
      val = cellHtml(f, row);
      if (!val) continue;
      label = fieldLabel(f.name || f.slug);
      ...
      chips += '<span class="cell-muted" style="max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:10px"'
        ...
        + '>' + esc(label) + ': <span style="color:var(--guest-secondary)">' + val + '</span></span>';
    }
    if (!chips) return '';
  ...
  function guestTreeGuides(depth){
    ...
    var lvl;
    for (lvl=0;lvl<n;lvl++){
      html += '<span aria-hidden="true" style="position:absolute;top:0;bottom:0;width:1px;background:var(--guest-border);pointer-events:none;left:' + (12 + lvl * GUEST_TREE_INDENT_PX + 9) + 'px"></span>';
    }
    return html;
  }
  function guestTreeChevronHtml(hasKids){
    ...
    return '<span aria-hidden="true" class="gt-chev" style="flex:0 0 auto;width:20px;height:20px;display:inline-flex;align-items:center;justify-content:center;color:var(--guest-muted);font-size:11px;line-height:1">'
  ...
  function guestTreeRowInner(node, depth, titleSlug, metaFields, showBorder){
    ...
    var border = showBorder ? 'border-top:1px solid var(--guest-divider);' : 'border-top:0;';
  ...
  function guestTreeStyleHtml(){
    return '<style>'
      + '.guest-tree .gt-banner{margin:0 0 8px;padding:6px 12px;color:#fcd34d;border-bottom:1px solid var(--guest-border);background:rgba(245,158,11,0.05);font-size:11px}'
