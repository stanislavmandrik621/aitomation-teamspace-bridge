 ...
 *
 * The overlay renders a keyboard-accessible document picker and one selected
 * document, with a sidebar on wide pages and stacked navigation on mobile.
 * Bodies include a heading outline, word/character counts and the desktop
 * 200000-character limit (DOC_CONTENT_MAX_CHARS). Editing remains in the app.
 *
 ...
 * - Autosave, peer Keep mine / Use theirs,
...
export const GUEST_VIEW_DOC_JS = `
  ...
  function docFindField(cols, token){
    var s = (typeof token === 'string' ? token : '').trim();
    if (!s) return null;
    var i, f, slug, id;
  ...
  function docRawBody(row, slug){
    ...
    if (rowHasDisplay(row, slug)) {
      var disp = rowDisplayMap(row)[slug];
      if (typeof disp === 'string') return disp;
      if (typeof disp === 'number' || typeof disp === 'boolean') return String(disp);
      var fromDisp = valueText(disp, 0);
    ...
    if (raw && typeof raw === 'object') return valueText(raw, 0);
    return '';
  }
  function docDropTrailingLoneSurrogate(s){
    if (!s.length) return s;
    var last = s.charCodeAt(s.length - 1);
    return last >= 0xd800 && last <= 0xdbff ? s.slice(0, -1) : s;
  }
  function docRepairIfNeeded(s){
    var dropped = docDropTrailingLoneSurrogate(s);
  ...
  function docClampText(raw){
    var text = typeof raw === 'string' ? raw : valueText(raw, 0);
    var max = 200000;
    var cut = text.length <= max ? text : text.slice(0, max);
    return { text: docRepairIfNeeded(cut), truncated: text.length > max };
  }
  ...
  function docOutlineHtml(items, prefix){
    ...
    var html = '<div style="border-left:1px solid var(--guest-border);padding:6px 0;margin:8px 0 10px">';
    ...
    for (i=0;i<items.length;i++){
      ...
      html += '<a href="#' + prefix + '-h' + i + '" style="display:block;padding:3px 12px 3px ' + pad + 'px;font-size:.75rem;color:var(--guest-muted)">' + esc(it.text) + '</a>';
    }
    html += '</div>';
    return html;
  }
  ...
  function docInlineHtml(raw, depth){
    ...
    while (i < n){
      if (s.charAt(i) === tick) {
        ...
        if (j > i) {
          out += '<code style="background:var(--guest-input);border:1px solid var(--guest-border);border-radius:4px;padding:1px 4px;font-size:.8em">' + esc(s.slice(i + 1, j)) + '</code>';
          i = j + 1;
          continue;
        }
      }
  ...
  function docPreHtml(lines){
    ...
    return '<pre class="doc-body" style="background:var(--guest-input);border:1px solid var(--guest-border);border-radius:8px;padding:10px 12px;overflow-x:auto;white-space:pre-wrap;margin:8px 0"><code>' + body + '</code></pre>';
  }
  function docMdBodyHtml(text, prefix){
    ...
    flushQuote = function(){
      ...
      out.push('<blockquote class="doc-body" style="margin:8px 0;padding:4px 0 4px 12px;border-left:2px solid var(--guest-border)">' + docInlineHtml(quoteBuf.join('\\n')) + '</blockquote>');
      quoteBuf = [];
    };
    flushFence = function(){
    ...
    for (i=0;i<lines.length;i++){
      ...
      if (/^\\s*---+$/.test(ln) || /^\\s*\\*\\*\\*+$/.test(ln)) {
        flushPara(); flushList(); flushQuote();
        out.push('<hr style="border:0;border-top:1px solid var(--guest-border);margin:12px 0"/>');
        continue;
      }
      m = /^>\\s?(.*)$/.exec(ln);
      if (m) {
  ...
  function renderDocHtml(content){
    ...
    for (i=0;i<rows.length;i++){
      ...
      chars = body.length;
      choices += '<input class="doc-choice" type="radio" name="guest-document" id="' + aid + '-pick" aria-label="' + esc(titleText || 'Untitled') + '"' + (i === 0 ? ' checked' : '') + '>';
      selectionCss += '.doc-list>#' + aid + '-pick:checked~.doc-shell .doc-pages>#' + aid + '{display:block}'
        + '.doc-list>#' + aid + '-pick:checked~.doc-shell label[for="' + aid + '-pick"]{background:var(--guest-input);color:var(--guest-fg);font-weight:600}'
        + '.doc-list>#' + aid + '-pick:focus-visible~.doc-shell label[for="' + aid + '-pick"]{outline:2px solid #3b82f6;outline-offset:-2px}';
      nav += '<label for="' + aid + '-pick" class="doc-nav-item" title="' + esc(titleText || 'Untitled') + '">' + (titleText ? esc(titleText) : '<span class="cell-muted">Untitled</span>') + '</label>';
      if (!body.trim()) {
      ...
      footer = '<div class="muted" style="margin-top:10px;font-size:.65rem;display:flex;justify-content:space-between;gap:8px;flex-wrap:wrap;border-top:1px solid var(--guest-border);padding-top:8px">';
    ...
    return '<div class="stack doc-list">'
      + '<style>.doc-list>.doc-choice{position:absolute;width:1px;height:1px;opacity:0;overflow:hidden;clip:rect(0,0,0,0)}'
      + '.doc-shell{display:grid;grid-template-columns:220px minmax(0,1fr);gap:16px;min-width:0;align-items:start}'
      + '.doc-nav{min-width:0;padding:0;max-height:70vh;overflow:auto}'
      + '.doc-nav-item{display:block;padding:10px 12px;margin:0;cursor:pointer;font-size:.75rem;border-bottom:1px solid var(--guest-divider);color:var(--guest-muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}'
      + '.doc-pages{min-width:0}.doc-pages>.row-card{display:none;margin:0}'
      + '@media(max-width:700px){.doc-shell{grid-template-columns:minmax(0,1fr)}.doc-nav{max-height:180px}}'
      + selectionCss + '</style>' + choices
      + '<p class="hint" style="margin:0">View only. You can read these documents, not edit them.</p>'
      + '<div class="doc-shell"><div class="card doc-nav">'
      + '<div class="muted" style="padding:8px 12px;border-bottom:1px solid var(--guest-border);font-size:.7rem">Documents' + (pageNote ? (' - ' + esc(pageNote)) : '') + '</div>'
      + nav
      + '</div>'
      + '<div class="doc-pages">' + articles + '</div>'
      + '</div></div>';
  }
`
