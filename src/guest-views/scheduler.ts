 ...
 * A local week navigator selects one week at a time. No drag, no create.
 *
...
export const GUEST_VIEW_SCHEDULER_JS = `
  ...
  function renderSchedulerBodyHtml(content, selectedWeek){
    ...
    if(selectedWeek) { weeks=[selectedWeek]; weeksTruncated=false;weeksHidden=0; }
    ...
    function paintWeek(weekStart){
      ...
      out += '<div role="region" aria-label="Schedule days" tabindex="0" style="overflow-x:auto;border:1px solid var(--guest-border);border-radius:8px;min-width:0">';
      out += '<div style="min-width:720px">';
      out += '<div style="display:grid;grid-template-columns:' + SCHED_GRID_HEAD + ';position:sticky;top:0;z-index:3;background:var(--guest-bg);border-bottom:1px solid var(--guest-border)">';
      out += '<div style="position:sticky;left:0;z-index:4;background:var(--guest-bg);padding:6px 8px;font-size:10px;text-transform:uppercase;letter-spacing:.04em;color:var(--guest-muted);font-weight:500;border-right:1px solid var(--guest-border)">Resource</div>';
      ...
      for (di=0;di<days.length;di++){
        ...
        out += '<div style="padding:4px 4px;text-align:center;border-left:1px solid var(--guest-border);min-width:0;background:' + headBg + '">';
        out += '<div style="font-size:9px;text-transform:uppercase;letter-spacing:.04em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:'
          + (isToday ? '#3b82f6;font-weight:600' : 'var(--guest-muted)') + '">' + esc(parts.weekday) + '</div>';
        out += '<div style="font-size:11px;line-height:16px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:'
          + (isToday ? '#3b82f6;font-weight:600' : 'var(--guest-secondary)') + '">' + esc(parts.day) + '</div>';
        out += '</div>';
      }
      out += '</div>';
      order.forEach(function(k){
        ...
        for (di=0;di<days.length;di++){
          ...
          bands += '<div style="border-left:1px solid var(--guest-border);height:100%;background:' + headBg + '"></div>';
        }
        var bars = '';
        packing.placed.forEach(function(p){
          ...
          var fg = conflict ? '#fbbf24' : 'var(--guest-fg)';
        ...
        if (packing.overflow.length > 0) {
          ...
          if (moreN > moreItems.length) {
            moreList += '<div class="muted" style="padding-top:6px;border-top:1px solid var(--guest-border)">Showing first '
          ...
          bars += '<details style="position:absolute;right:6px;bottom:2px;z-index:2;font-size:10px;color:#f59e0b">'
            ...
            + '<div class="at-scroll" style="position:absolute;right:0;bottom:18px;width:220px;max-height:min(50vh,320px);overflow-y:auto;overflow-x:hidden;background:var(--guest-card);border:1px solid var(--guest-border);border-radius:8px;padding:8px 10px;color:var(--guest-fg)">'
            + moreList + '</div></details>';
        }
        out += '<div style="display:grid;grid-template-columns:' + SCHED_GRID_LANE + ';border-bottom:1px solid var(--guest-border)">';
        out += '<div class="tl-lab" style="position:sticky;left:0;z-index:2;background:var(--guest-bg);padding:8px;font-size:12px;font-weight:500;border-right:1px solid var(--guest-border);align-self:stretch;display:flex;align-items:flex-start">'
    ...
    html += weekHtml || '<p class="muted">No bookings for this week.</p>';
    html += '</div>';
    return html;
  }
  function schedGuestWeek(content,date){
    var key=ymdFromDate(date),week=schedWeekStartSun(key),cfg=viewCfg(content);
    return cfg.weekStartDay==='mon' ? schedAddDays(week,date.getDay()===0?-6:1) : week;
  }
  function schedGuestRenderWeek(content,week){
    return '<p class="muted" data-scheduler-week="'+week+'">'+esc(schedFmtRange(week,schedAddDays(week,6)))+'</p>'+renderSchedulerBodyHtml(content,week);
  }
  function renderSchedulerHtml(content){
    var week=schedGuestWeek(content,new Date());
    return '<section data-guest-scheduler data-scheduler-content="'+esc(JSON.stringify(content))+'"><style>.guest-sched-controls{display:flex;gap:6px;flex-wrap:wrap}.guest-sched-controls button{background:var(--guest-card);color:var(--guest-fg);border:1px solid var(--guest-border);border-radius:6px;padding:6px 10px;font:inherit;cursor:pointer}.guest-sched-controls button:focus-visible{outline:2px solid #3b82f6;outline-offset:2px}</style><div class="guest-sched-controls"><button type="button" data-scheduler-nav="-1" aria-label="Previous week">Previous</button><button type="button" data-scheduler-nav="0">Today</button><button type="button" data-scheduler-nav="1" aria-label="Next week">Next</button></div><div data-scheduler-body>'+schedGuestRenderWeek(content,week)+'</div></section>';
  }
  if(typeof document!=='undefined'&&typeof MutationObserver!=='undefined'){
    var initGuestSchedulers=function(){document.querySelectorAll('[data-guest-scheduler]').forEach(function(root){
      if(root.__schedulerReady)return;root.__schedulerReady=true;
      var content;try{content=JSON.parse(root.getAttribute('data-scheduler-content')||'{}');}catch(error){return;}root.removeAttribute('data-scheduler-content');
      var week=schedGuestWeek(content,new Date());
      root.addEventListener('click',function(event){var button=event.target.closest('button[data-scheduler-nav]');if(!button||!root.contains(button))return;var n=Number(button.getAttribute('data-scheduler-nav'));week=n?schedAddDays(week,n*7):schedGuestWeek(content,new Date());root.querySelector('[data-scheduler-body]').innerHTML=schedGuestRenderWeek(content,week);});
    });};new MutationObserver(initGuestSchedulers).observe(document.documentElement,{childList:true,subtree:true});initGuestSchedulers();
  }
