 ...
 * - WIP limits, stale badges, color_rules
 * - Settings maxExtraFields (guest uses the default 2)
 * - Comment-count badges and private attachment covers
...
export const GUEST_VIEW_KANBAN_JS = `
  ...
  function guestKanbanIsKanbanType(vt){
    ...
    return t === 'kanban' || t === 'kanban_board' || t === 'board' || t === 'cards' || t === 'pipeline';
  }
  ...
  function guestKanbanCardHtml(r, cols, titleSlug, groupSlug, imgSlug, descSlug, maxExtras, footer){
    var extra = '';
    var cover = imgSlug ? rowHttpUrl(r, imgSlug) : '';
    if (cover && isSafeHttpUrl(cover)) extra += '<img alt="" loading="lazy" style="width:100%;max-height:160px;object-fit:cover;border-radius:8px" src="'+esc(cover)+'"/>';
    var description = descSlug ? rowCellText(r, descSlug) : '';
    if (description) extra += '<p class="muted" style="white-space:pre-wrap;overflow-wrap:anywhere;max-height:6em;overflow:hidden">'+esc(description)+'</p>';
    var shown = 0;
    var i, c, val;
    for (i = 0; i < cols.length && shown < (typeof maxExtras === 'number' ? maxExtras : GUEST_KANBAN_CARD_EXTRAS); i++) {
      c = cols[i];
    ...
    return '<div class="board-card">' + rowTitleHtml(r, titleSlug) + extra + (footer || '') + '</div>';
  }
  function renderKanbanHtml(content){
    ...
    var cfg = viewCfg(content);
    var swimSlug = cfg.swimlaneFieldSlug || cfg.swimlane_field;
    var swimField = guestKanbanFieldBySlug(cols, swimSlug);
    if (swimField && ['select','status'].indexOf(swimField.field_type) >= 0 && swimSlug !== groupSlug && !content.guestSwimlaneChild) {
      var swimOptions = guestKanbanReadOptions(swimField), swimOrder = swimOptions.keys.slice(), groups = Object.create(null);
      swimOrder.forEach(function(key){groups[key] = [];});
      (content.rows || []).forEach(function(row){
        var key = rowCellText(row, swimSlug).trim() || GUEST_KANBAN_UNCATEGORIZED;
        key = swimOptions.alias[key] || key;
        if (!Object.prototype.hasOwnProperty.call(groups,key)) { groups[key]=[]; swimOrder.push(key); }
        groups[key].push(row);
      });
      if (swimOrder.indexOf(GUEST_KANBAN_UNCATEGORIZED) < 0) { swimOrder.push(GUEST_KANBAN_UNCATEGORIZED); groups[GUEST_KANBAN_UNCATEGORIZED]=[]; }
      return '<div data-guest-swimlanes="1">'+swimOrder.map(function(key){
        return '<section><h3>'+esc(swimOptions.labels[key] || key)+'</h3>'+renderKanbanHtml(Object.assign({},content,{guestSwimlaneChild:true,rows:groups[key]}))+'</section>';
      }).join('')+'</div>';
    }
    var sumSlug = cfg.sumFieldSlug || cfg.sum_field, sumField = guestKanbanFieldBySlug(cols, sumSlug);
    if (sumField && ['number','currency','percent','rating','progress','vote'].indexOf(sumField.field_type) < 0) sumField = null;
    var cardCols = Array.isArray(cfg.cardFields) ? cols.filter(function(f){return cfg.cardFields.indexOf(f.slug)>=0;}) : cols;
    ...
    for (li = 0; li < ordered.length; li++) {
      ...
      for (ci = 0; ci < list.length; ci++) {
        cards += guestKanbanCardHtml(list[ci], cardCols, titleSlug, groupSlug, imgSlug, descSlug);
      }
      var st = opt.colors[k] ? ' style="border-color:' + esc(opt.colors[k]) + '"' : '';
      var head = (opt.labels && opt.labels[k]) ? opt.labels[k] : k;
      var sumHtml = '';
      if (sumField) {
        var sum = list.reduce(function(total,row){
          var n=row.data && row.data[sumSlug];
          if (typeof n==='string' && n.trim()) { var clean=n.trim().replace(/[%$,\\s]/g,''); n=clean ? Number(clean) : null; }
          return total+(typeof n==='number'&&isFinite(n)?n:0);
        },0), sumData = {};
        sumData[sumSlug] = sum;
        sumHtml = '<span title="'+esc(sumField.name || sumSlug)+'" data-column-sum="'+esc(String(sum))+'">'+cellHtml(sumField,{data:sumData})+'</span>';
      }
      html += '<div class="lane"' + st + '><div class="lane-head"><span>' + esc(head) + '</span><span class="lane-count">' + String(list.length) + '</span>'+sumHtml+'</div>' + cards + '</div>';
    }
    return html + '</div>';
  }
