...
export const GUEST_VIEW_DATE_BUCKET_JS = `
  ...
  function guestDateBucketCardHtml(row, cols, titleSlug, dateSlug, imgSlug, descSlug){
    ...
    if (imgSlug) {
      ...
      if (src) {
        cover = '<img loading="lazy" alt="" src="' + esc(src) + '" style="display:block;width:100%;height:72px;object-fit:cover;border-radius:8px;margin-bottom:8px;background:var(--guest-input)"/>';
      }
    }
    var desc = '';
    if (descSlug) {
