...
export const GUEST_VIEW_CHART_JS = `
  ...
  function renderStackedBarSvg(categories, series, matrix, agg){
    ...
    if (max <= 0) max = 1;
    var w = guestChartCanvasWidth(), h = 320, padL = 36, padB = 56, padT = 28, padR = 16;
    ...
    for (i=0;i<categories.length;i++){
      ...
      parts.push('<text x="' + cx + '" y="' + (padT + innerH - (Math.max(labelVal, 0) / max) * innerH - 6) + '" text-anchor="middle" fill="var(--guest-fg)" font-size="11">' + esc(String(labelVal)) + '</text>');
      parts.push('<text x="' + cx + '" y="' + (h - 18) + '" text-anchor="middle" fill="var(--guest-muted)" font-size="11">' + esc(chartTruncLabel(categories[i].k)) + '</text>');
    }
  ...
  function renderMultiLineSvg(categories, series, matrix){
    ...
    for (i=0;i<categories.length;i++){
      ...
      parts.push('<text x="' + lx + '" y="' + (h - 18) + '" text-anchor="middle" fill="var(--guest-muted)" font-size="11">' + esc(chartTruncLabel(categories[i].k)) + '</text>');
    }
    return '<svg class="chart-svg" viewBox="0 0 ' + w + ' ' + h + '" width="100%" role="img">' + parts.join('') + '</svg>';
  }
  ...
  function renderChartBodyHtml(content){
  ...
  function renderChartHtml(content){
    return '<div data-guest-chart data-chart-content="'+esc(JSON.stringify(content))+'">'+renderChartBodyHtml(content)+'</div>';
  }
  function enhanceGuestChart(){
    if(typeof document==='undefined')return;
    document.querySelectorAll('[data-guest-chart]').forEach(function(root){
      if(root.__chartReady)return;root.__chartReady=true;
      var content;try{content=JSON.parse(root.getAttribute('data-chart-content')||'{}');}catch(error){return;}
      root.removeAttribute('data-chart-content');
      var last=0;
      var repaint=function(){
        var frame=root.querySelector('.chart-wrap');if(!frame)return;
        var style=getComputedStyle(frame), width=Math.max(240,Math.min(960,Math.round(frame.clientWidth-parseFloat(style.paddingLeft||0)-parseFloat(style.paddingRight||0))));
        if(width===last)return;last=width;
        var prior=GUEST_CHART_CANVAS_WIDTH;GUEST_CHART_CANVAS_WIDTH=width;
        try{root.innerHTML=renderChartBodyHtml(content);}finally{GUEST_CHART_CANVAS_WIDTH=prior;}
      };
      repaint();
      if(typeof ResizeObserver!=='undefined'){
        var observer=new ResizeObserver(function(){if(!root.isConnected){observer.disconnect();return;}repaint();});observer.observe(root);
      }
    });
  }
