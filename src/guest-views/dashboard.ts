 ...
 * - Saved 2/3/4 columns use the desktop container-width breakpoints.
...
export const GUEST_VIEW_DASHBOARD_JS = `
  ...
  var DASH_GRID_STYLE = 'display:grid;gap:12px;align-items:stretch';

  function dashCapStr(s, max){
    return capStr(valueText(s, 0), max);
  }
  ...
  function dashPercentRing(pct, title, caption, span){
    ...
    var svg = '<svg class="chart-svg" viewBox="0 0 72 72" width="100%" style="max-width:160px;margin:0 auto;display:block" role="img">'
      ...
      + '<circle cx="36" cy="36" r="' + R + '" fill="none" stroke="var(--guest-border)" stroke-width="8"/>'
      + '<circle cx="36" cy="36" r="' + R + '" fill="none" stroke="#3b82f6" stroke-width="8" stroke-linecap="round" stroke-dasharray="'
      + ring + ' ' + circ + '"/>'
      + '</g>'
      + '<text x="36" y="40" text-anchor="middle" fill="var(--guest-fg)" font-size="14" font-weight="600">' + esc(String(clamped)) + '%</text>'
      + '</svg>';
  ...
  function dashMiniPieSvg(buckets, donut){
    ...
    var center = donut
      ? '<text x="' + cx + '" y="' + (cy + 5) + '" text-anchor="middle" fill="var(--guest-fg)" font-size="16" font-weight="600">' + esc(String(Math.round(total * 100) / 100)) + '</text>'
      : '';
  ...
  function dashMiniBarSvg(buckets){
    ...
    for (i=0;i<buckets.length;i++){
      ...
      var cx = x + bw / 2;
      var label = buckets[i].k.length > 10 ? capStr(buckets[i].k, 9) + '...' : capStr(buckets[i].k, 10);
      parts.push('<rect x="' + x + '" y="' + y + '" width="' + bw + '" height="' + Math.max(bh, 1) + '" rx="4" fill="' + fills[i % fills.length] + '"/>');
      parts.push('<text x="' + cx + '" y="' + (y - 4) + '" text-anchor="middle" fill="var(--guest-fg)" font-size="9">' + esc(String(buckets[i].v)) + '</text>');
      parts.push('<text x="' + cx + '" y="' + (h - 10) + '" text-anchor="middle" fill="var(--guest-muted)" font-size="9">' + esc(label) + '</text>');
    }
  ...
  function dashBreakdownBuckets(rows, field){
    ...
    for (i=0;i<rows.length;i++){
      ...
      for (j=0;j<parts.length;j++){
        lab = (typeof parts[j] === 'string' ? parts[j] : valueText(parts[j], 0)).trim();
  ...
  function renderDashboardHtml(content){
    ...
    for (wi=0;wi<widgets.length;wi++){
      ...
      if (w.type === 'breakdown') {
        ...
        title = w.title || fieldLabel(field.name || field.slug || 'Breakdown');
      ...
      if (w.type === 'percent') {
        ...
        title = w.title || fieldLabel(field.name || field.slug || 'Completion');
      ...
      if (w.type === 'sum' || w.type === 'avg') {
        ...
        title = w.title || ((w.type === 'sum' ? 'Total ' : 'Average ') + fieldLabel(field.name || field.slug || '')).trim();
        sub = '';
    ...
    var density = String(viewCfg(content).columns == null ? '' : viewCfg(content).columns).trim();
    if (density !== '2' && density !== '3' && density !== '4') density = '3';
    return '<div class="dash guest-card-container"><div class="dash-tiles guest-card-grid" data-grid-columns="' + density + '" style="' + DASH_GRID_STYLE + '">' + cards + '</div>'
