...
describe('TCC-R1133-BRG-001 - process-wide HTTP body budget bounds concurrent upload heap', () => {
  it('readBodyBytes and readJsonBody both reserve against the shared budget before buffering', () => {
    ...
    assert.match(bytesFn, /if \(!tryReserveHttpBodyBudget\(reserve\)\)/, 'readBodyBytes reserves budget up front')
    ...
    assert.match(bytesFn, /releaseHttpBodyBudget\(reserve\)/, 'releases the reservation on a terminal path')
    assert.match(bytesFn, /resolve\(\{ bytes: new Uint8Array\(Buffer\.concat\(chunks\)\), release \}\)/, 'holds until caller release')
    ...
    assert.match(
      ...
      /if \(!tryReserveHttpBodyBudget\(reserve\)\)[\s\S]*?reject\(new HttpBodyBudgetExhaustedError\(HTTP_BODY_BUDGET_RETRY_AFTER_SEC\)\)/,
  ...
  it('the process-wide counter is bounded by MAX_INFLIGHT_HTTP_BODY_BYTES sourced from throughput.ts (env-tunable)', () => {
    assert.match(server, /import \{[\s\S]*?MAX_INFLIGHT_HTTP_BODY_BYTES,?[\s\S]*?\} from '\.\/throughput\.js'/)
    assert.match(
      server,
