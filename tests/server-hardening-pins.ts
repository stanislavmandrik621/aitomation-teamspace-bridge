...
describe('TCC-R1133-WS-001 - chat_rooms_list scans rooms through a bounded concurrency pool', () => {
  it('replaces the unbounded Promise.all fan-out with mapWithConcurrency(baseRooms, CHAT_ROOMS_LIST_SCAN_CONCURRENCY, ...)', () => {
    ...
    assert.match(window, /await chatStore\.readUnreadSummary\(r\.id, memberId, wm\)/, 'reads the full unread summary scoped to this member and read watermark')
    assert.match(window, /unreadTruncated = summary\.unreadTruncated/, 'preserves honest truncation reporting')
    assert.match(window, /unreadError = true/, 'scan failure cannot look like a successful empty summary')
  })
...
describe('TCC-R1134-BRG-002 - graceful SIGTERM/SIGINT shutdown drains live connections', () => {
  it('gracefulShutdown notifies + cleanly closes every live WS socket before stopping the servers', () => {
    ...
    const serverCloseIdx = fn.indexOf('server.close(async () => {')
  ...
  it('GRACEFUL_SHUTDOWN_TIMEOUT_MS is clamped to a sane bounded range (never 0, never unbounded)', () => {
    assert.match(
      server,
      /const GRACEFUL_SHUTDOWN_TIMEOUT_MS = Math\.max\(\s*1_000,\s*Math\.min\(60_000, Number\(process\.env\.TEAMSPACE_SHUTDOWN_TIMEOUT_MS \|\| \(mailService \? 30_000 : 5_000\)\) \|\| \(mailService \? 30_000 : 5_000\)\),\s*\)/,
    )
  })
})
