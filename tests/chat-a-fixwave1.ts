...
// of send order, isolating the ordering logic from real clock timing. ---
{
  ...
  try {
    ...
    const pageBeforeNewest = await store.readRecent(CHAT_ROOM_TEAM, 10, tiedAt, 'a_third')
    assert.deepEqual(
      pageBeforeNewest.messages.map((m) => m.id),
      ['z_first', 'm_second'],
      'same-ms beforeId page must keep earlier-seq rows, never skip via id.localeCompare',
    )
    const pageUnknownCursor = await store.readRecent(CHAT_ROOM_TEAM, 10, tiedAt, 'missing_id')
    assert.deepEqual(
      pageUnknownCursor.messages.map((m) => m.id),
      [],
      'unknown cursor seq must exclude same-ms rows rather than order them by id',
    )
