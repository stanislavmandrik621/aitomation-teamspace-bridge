...
try {
  ...
  // Same process: a later write can tear the tail after the first healthy
  // check. Repair must run again, not latch "already checked this room".
  const tornAgain = readFileSync(messagesPath, 'utf8').replace(/\n$/, '') + '{"id":"c_torn2","body":"cut'
  writeFileSync(messagesPath, tornAgain, 'utf8')
  const appendedSameProcess = await store2.append({
    room: CHAT_ROOM_TEAM,
    body: 'message after a same-process torn tail',
    memberId: 'm4',
    memberName: 'Drew',
    role: 'member',
  })
  if ('error' in appendedSameProcess) throw new Error(appendedSameProcess.error)
  const afterSame = readFileSync(messagesPath, 'utf8')
  const sameLines = afterSame.split('\n').filter((l) => l.length > 0)
  const lastLine = sameLines[sameLines.length - 1]!
  const lastParsed = JSON.parse(lastLine) as { body: string }
  assert.equal(lastParsed.body, 'message after a same-process torn tail')
  assert.ok(
    sameLines.some((l) => l.includes('c_torn2') && !l.includes('message after a same-process torn tail')),
    'torn fragment must stay isolated on its own line',
  )
