 ...
 *      groupByFieldSlug, with a "Uncategorized" lane for blanks;
...
const rendererApi = new Function(
  ...
  return {
    ...
    rowHttpUrl: rowHttpUrl,
    httpUrlFromUnknown: httpUrlFromUnknown,
...
// ---- (a) v1 payload: never "[object Object]", viewType dispatch is live ----
{
  ...
  // Board aliases use the same explicit missing-group state as Kanban.
  const titleOnly = {
    version: 1,
    mode: 'read',
    viewType: 'board',
  ...
  assert.deepEqual(titleOut, rendererApi.renderReadBodyHtml({ ...titleOnly, viewType: 'kanban' }), 'board alias preserves the canonical missing-group state')
  assert.ok(!titleOut.html.includes('class="board"'), 'no group field means no empty board chrome')
}

// ---- (b) v2 board grouping by display value, "Uncategorized" lane for blanks ----
{
  const v2 = {
    version: 2,
    mode: 'read',
  ...
  assert.ok(
    out.html.includes('<span>Uncategorized</span><span class="lane-count">1</span>'),
    'blank group value lands in the Uncategorized lane',
  )
  ...
  const dated = {
    ...
    rows: [
      { id: '1', data: { name: 'A', stage: 'won', due: new Date().toISOString().slice(0, 7) + '-13' }, display: { name: 'A', stage: 'Won' } },
      { id: '2', data: { name: 'B', stage: 'lost', due: new Date().toISOString().slice(0, 7) + '-13' }, display: { name: 'B', stage: 'Lost' } },
      { id: '4', data: { name: 'D' }, display: { name: 'D' } },
    ],
  }
  ...
  // ---- Field-name humanization: guest pages never leak raw slug/underscore
  // text (e.g. "unit_cost") the way an un-formatted `name` fallback would.
  // Every render surface must title-case field.name (or field.slug when name
  // is blank) exactly like the desktop app's formatFieldLabel().
  assert.equal(rendererApi.fieldLabel('unit_cost'), 'Unit Cost', 'snake_case slug is title-cased')
  assert.equal(rendererApi.fieldLabel('restockDate'), 'Restock Date', 'camelCase slug is split and title-cased')
  assert.equal(rendererApi.fieldLabel('product_url'), 'Product URL', 'known acronym (url) is upper-cased')
  assert.equal(rendererApi.fieldLabel('Unit Cost'), 'Unit Cost', 'already-clean name is unchanged (idempotent)')
  assert.equal(rendererApi.fieldLabel(''), '', 'empty name stays empty')
  ...
  const badName = {
    ...
    label: 'Inventory',
    entityId: 'e2',
    fields: [field('name', 'Name'), field('unit_cost', 'unit_cost', 'currency'), field('sku_code', '', 'text')],
    columns: ['name', 'unit_cost', 'sku_code'],
    viewConfig: {},
    rows: [
      { id: '1', data: { name: 'Widget', unit_cost: 4.5, sku_code: 'W-1' }, display: { name: 'Widget', unit_cost: '$4.50', sku_code: 'W-1' } },
    ],
    total: 1,
    truncated: false,
    includeCsv: true,
  ...
  const tableBadName = rendererApi.renderReadBodyHtml(badName)
  assert.ok(tableBadName.html.includes('<th>Unit Cost</th>'), 'table header humanizes a snake_case stored field name')
  assert.ok(tableBadName.html.includes('<th>SKU Code</th>'), 'table header humanizes the slug when the stored name is blank, incl. acronym')
  assert.ok(!/[>]unit_cost[<]/.test(tableBadName.html), 'raw underscored field name never reaches the table header')

  const listBadName = rendererApi.renderReadBodyHtml({ ...badName, viewType: 'list' })
  assert.ok(listBadName.html.includes('Unit Cost'), 'list key/value rows humanize the field label')
  assert.ok(!/[>]unit_cost[<]/.test(listBadName.html), 'list rows never show the raw slug')

  const boardBadName = rendererApi.renderReadBodyHtml({
    ...badName,
    viewType: 'board',
    fields: [field('name', 'Name'), field('stage', 'Stage', 'status'), field('unit_cost', 'unit_cost', 'currency')],
    columns: ['name', 'stage', 'unit_cost'],
    viewConfig: { groupByFieldSlug: 'stage' },
    rows: [
      { id: '1', data: { name: 'Widget', stage: 'open', unit_cost: 4.5 }, display: { name: 'Widget', stage: 'Open', unit_cost: '$4.50' } },
    ],
  })
  assert.ok(boardBadName.html.includes('Unit Cost'), 'board card extra fields humanize the label')
  assert.ok(!/[>]unit_cost[<]/.test(boardBadName.html), 'board card never shows the raw slug')

  const csvBadName = rendererApi.csvFromContent(badName)
  const csvHeaderLine = csvBadName.split('\r\n')[0]
  assert.ok(csvHeaderLine.includes('Unit Cost'), 'CSV header humanizes the field label')
  assert.ok(!csvHeaderLine.includes('unit_cost'), 'CSV header never emits the raw slug')

  const dashboardBadName = rendererApi.renderReadBodyHtml({
    ...badName,
    viewType: 'dashboard',
    viewConfig: { widgets: [{ type: 'sum', field: 'unit_cost' }] },
  })
  assert.ok(dashboardBadName.html.includes('Total Unit Cost'), 'dashboard sum widget title humanizes the field label, never "Total unit_cost"')
  assert.ok(!dashboardBadName.html.includes('unit_cost'), 'dashboard widget title never shows the raw slug')
  ...
  const sched = rendererApi.renderReadBodyHtml({
    ...dated,
    viewType: 'scheduler',
    rows: dated.rows.map(row => ({...row, data:{...row.data, due:[new Date().getFullYear(),String(new Date().getMonth()+1).padStart(2,'0'),String(new Date().getDate()).padStart(2,'0')].join('-')}})),
  ...
  assert.ok(scrum.html.includes('Status and sprint fields are unavailable'), 'scrum requires its two distinct visible bindings')
...
// ---- v2 display ladder: any present display[slug] wins, never String(object) ----
{
  const objRow = {
    id: 'r',
    data: {
      owner: { id: 'u1', name: 'Data Name' },
      n: 99,
      blank: 'from-data',
      tags: ['raw_id'],
      site: { url: 'https://evil.example/x' },
    },
    display: {
      owner: { name: 'Jane' },
      n: 7,
      blank: '',
      tags: [{ label: 'Hot' }],
      site: { url: 'https://example.com/page', name: 'Docs' },
    },
  }
  assert.equal(rendererApi.rowCellText(objRow, 'owner'), 'Jane', 'display object uses name, not data')
  assert.equal(rendererApi.rowCellText(objRow, 'n'), '7', 'display number wins over data')
  assert.equal(rendererApi.rowCellText(objRow, 'blank'), '', 'empty display wins over data')
  assert.ok(!rendererApi.rowCellText(objRow, 'owner').includes('[object Object]'))
  const chips = rendererApi.cellHtml(field('tags', 'Tags', 'tags'), objRow)
  assert.ok(chips.includes('Hot') && !chips.includes('raw_id'), 'display chip objects win over data ids')
  const urlCell = rendererApi.cellHtml(field('site', 'Site', 'url'), objRow)
  assert.ok(urlCell.includes('href="https://example.com/page"'), 'url href unwraps display.url')
  assert.ok(urlCell.includes('Docs'), 'url label uses display name')
  assert.ok(!urlCell.includes('evil.example'), 'data url is not used when display has a safe url')
  assert.equal(rendererApi.rowHttpUrl(objRow, 'site'), 'https://example.com/page')
  assert.equal(rendererApi.httpUrlFromUnknown({ url: 'javascript:alert(1)' }, 0), '')
  assert.equal(rendererApi.httpUrlFromUnknown({ path: 'https://example.com/x' }, 0), '', 'path is not a link key')

  const jsObj = {
    id: 'r2',
    data: { site: { url: 'https://example.com/ok' } },
    display: { site: { url: 'javascript:alert(1)' } },
  }
  const jsCell = rendererApi.cellHtml(field('site', 'Site', 'url'), jsObj)
  assert.ok(!jsCell.includes('javascript:'), 'javascript: in display.url never becomes href')
  assert.ok(jsCell.includes('href="https://example.com/ok"'), 'unsafe display url falls through to data http(s)')
  ...
  const galleryDisp = {
    ...
    label: 'Catalog',
    entityId: 'e1',
    fields: [field('name', 'Name'), field('img', 'Image', 'image')],
    columns: ['name', 'img'],
    viewConfig: { titleFieldSlug: 'name', imageFieldSlug: 'img' },
    rows: [
      { id: '1', data: { name: 'Good' }, display: { name: 'Good', img: { url: 'https://example.com/b.png' } } },
      { id: '2', data: { name: 'Js' }, display: { name: 'Js', img: { url: 'javascript:alert(1)' } } },
      { id: '3', data: { name: 'File' }, display: { name: 'File', img: { path: '/tmp/x.png' } } },
    ],
    total: 3,
    truncated: false,
    includeCsv: false,
  ...
  const galleryOut = rendererApi.renderReadBodyHtml(galleryDisp)
  const galleryImgs = galleryOut.html.split('<img class="gallery-img"').length - 1
  assert.equal(galleryImgs, 1, 'display {url} http(s) becomes an image; javascript/path do not')
  assert.ok(galleryOut.html.includes('src="https://example.com/b.png"'))
  assert.ok(!galleryOut.html.includes('javascript:'))
  assert.ok(!galleryOut.html.includes('/tmp/x.png'))

  const listDisp = rendererApi.renderReadBodyHtml({
    ...galleryDisp,
    viewType: 'list',
  })
  assert.ok(listDisp.html.includes('src="https://example.com/b.png"'), 'list cover unwraps display {url}')
  assert.ok(!listDisp.html.includes('javascript:'))

  const docDisp = rendererApi.renderReadBodyHtml({
    version: 2,
    mode: 'read',
    viewType: 'doc',
    label: 'Notes',
    entityId: 'e1',
    fields: [field('title', 'Title'), field('body', 'Body', 'rich_text')],
    columns: ['title', 'body'],
    viewConfig: { titleFieldSlug: 'title', contentFieldSlug: 'body' },
    rows: [
      {
        id: '1',
        data: { title: 'T', body: { markdown: 'DATA BODY SHOULD NOT WIN' } },
        display: { title: 'T', body: 'Hello from display' },
      },
    ],
    total: 1,
    truncated: false,
  ...
  assert.ok(docDisp.html.includes('Hello from display'), 'doc body uses display string first')
  assert.ok(!docDisp.html.includes('DATA BODY SHOULD NOT WIN'), 'doc does not stringify data over display')
  assert.ok(!docDisp.html.includes('[object Object]'))
}
...
// ---- Theme: shared baseline document + styled 404/410 page ----
{
  ...
  assert.ok(doc.includes('var(--guest-bg)'), 'baseline page background present')
  assert.ok(GUEST_PAGE_CSS.includes('var(--guest-card)') && GUEST_PAGE_CSS.includes('var(--guest-border)') && GUEST_PAGE_CSS.includes('prefers-color-scheme:dark'), 'card colors in baseline')
  ...
  assert.ok(gone.includes('var(--guest-bg)'), 'error page carries the same baseline')
}
...
// ---- server.ts shells route through the shared modules (structural pins) ----
{
  ...
  // G15: list / date-board previews must cap through capStr (TS-CHAT-032/034).
  const guestViewsDir = join(dirname(fileURLToPath(import.meta.url)), '..', 'src', 'guest-views')
  const listSrc = readFileSync(join(guestViewsDir, 'list.ts'), 'utf8')
  const dateBucketSrc = readFileSync(join(guestViewsDir, 'date_bucket.ts'), 'utf8')
  const listPreview = listSrc.slice(
    listSrc.indexOf('function preview(text, max)'),
    listSrc.indexOf('function listChipsHtml'),
  )
  const datePreview = dateBucketSrc.slice(
    dateBucketSrc.indexOf('function guestDateBucketPreview(text)'),
    dateBucketSrc.indexOf('function guestDateBucketEmptyHtml'),
  )
  assert.ok(listPreview.includes('capStr('), 'list preview uses capStr')
  assert.equal(listPreview.includes('.slice(0, max)'), false, 'list preview must not raw-slice')
  assert.ok(datePreview.includes('capStr('), 'date-board preview uses capStr')
  assert.equal(
    datePreview.includes('.slice(0, GUEST_DATE_BUCKET_DESC_MAX)'),
    false,
    'date-board preview must not raw-slice',
  )
}
