...
try {
  ...
  try {
    ...
    writeFileSync(join(capDir, 'chat', '_meta.json'), '{}', 'utf8')
    ...
    assert.ok(
      capped.files.includes('chat/_meta.json'),
      '_meta.json must survive a saturated cap',
    )
  } finally {
  ...
  const authority=join(dir,'independent');mkdirSync(join(authority,'chat'),{recursive:true})
  writeFileSync(join(authority,'chat','rooms.json'),'CURRENT_PRIVATE_MEMBERSHIP')
  const current=store.pickChatExportEntries(MAX_BACKUP_ZIP_CHAT_ENTRIES,authority)
  const roomEntries=current.entries.filter(entry=>entry.name==='chat/rooms.json')
  assert.equal(roomEntries.length,1)
  assert.equal(readFileSync(roomEntries[0]!.absolutePath,'utf8'),'CURRENT_PRIVATE_MEMBERSHIP')
