 ...
 * (`if (staleDeviceAckKeys.has(ackKey)) continue`). Then
...
import { BridgeStore, hashSessionToken, memberSessionDeviceIds, memberDeviceAckKey } from '../src/store.js'
...
function writeFixture(
  ...
  writeFileSync(join(dir, 'acks.json'), JSON.stringify(Object.fromEntries(Object.entries(args.acks).map(([deviceId, bag]) => [memberDeviceAckKey('mem_a', deviceId), bag]))), 'utf8')
  writeFileSync(
    join(dir, 'ops.jsonl'),
...
function writeTwoAdminFixture(
  ...
  writeFileSync(join(dir, 'acks.json'), JSON.stringify(Object.fromEntries(Object.entries(args.acks ?? {}).map(([deviceId, bag]) => [memberDeviceAckKey(Object.hasOwn(args.liveSessions, deviceId) ? 'mem_live' : 'mem_ghost', deviceId), bag]))), 'utf8')
}
...
try {
  await t('stale-skip lives inside the prune quorum loop (comment it and this EXIT 1)', () => {
    ...
    const forIdx = prune.indexOf('for (const { deviceId: d, memberId, ackKey } of devices)')
    assert.ok(forIdx >= 0, 'pruneOps must loop registered devices')
    const window = prune.slice(forIdx, forIdx + 900)
    assert.match(
      window,
      /staleDeviceAckKeys\.has\(ackKey\)/,
  ...
  await t('hello refuses over-cap identity instead of truncating it into an alias', () => {
    ...
    assert.equal(store.listMembers().length, 0, 'refused identity must not bootstrap a truncated Admin')

    const hello = store.helloOrBootstrap({
      memberId: 'm'.repeat(127),
      deviceId: 'd'.repeat(127),
      memberEmail: 'cjk@example.com',
      displayName: `${'名'.repeat(199)}${emoji}`,
    })
  ...
  await t('source-scan: stored stamps cap text while presented identities stay exact', () => {
    ...
    assert.match(hello, /exactIdentityId\(\s*args\.memberId/)
    assert.match(hello, /exactIdentityId\(\s*args\.deviceId/)
    assert.doesNotMatch(hello, /args\.memberId\.trim\(\)\.slice/)
    const redeem = sourceWithoutComments(sliceClassMethod(storeSrc, 'redeemInviteInner'))
    assert.match(redeem, /exactIdentityId\(\s*args\.deviceId/)
  ...
  await t('source-scan: forget-device twins drop acks (comment dropDeviceAcks and this EXIT 1)', () => {
    for (const name of ['revokeSession', 'kickMembers', 'leaveTeam', 'recoverAdminWithKey', 'redeemInviteInner'] as const) {
    ...
    const kickCompat = sourceWithoutComments(sliceClassMethod(storeSrc, 'kickMember'))
    assert.match(kickCompat, /this\.kickMembers\s*\(/, 'single kick delegates to the batch-safe implementation')
    const drop = sourceWithoutComments(sliceClassMethod(storeSrc, 'dropDeviceAcks'))
  ...
  await t('source-scan: revokeSession last-Admin is live sessions not role count', () => {
    ...
    assert.match(mark, /registeredMembersForDevice\(d\)/)
    assert.match(mark, /if \(!registered\.length\) return/)
  ...
  await t('additional device preserves acks; role-changing redeem forgets revoked devices', async () => {
    ...
    assert.ok(store.deviceAckCount('dev_b1') > 0, 'additional project retains its authorized device ack bag')
    let row = store.listMembers().find((m) => m.email === 'b@example.com')
    assert.ok(row?.sessions.dev_b1)
    assert.ok(row?.sessions.dev_b2)
    const downgrade = store.createInvite('mem_admin', 'b@example.com', 'viewer')
    assert.ok(downgrade.ok)
    const changed = await store.redeemInvite({ token: downgrade.invite.token, deviceId: 'dev_b3', memberEmail: 'b@example.com' })
    assert.ok(changed.ok)
    assert.equal(store.deviceAckCount('dev_b1'), 0, 'role change drops revoked device ack bags')
    row = store.listMembers().find((m) => m.email === 'b@example.com')
    assert.equal(row?.sessions.dev_b1, undefined)
    assert.equal(row?.sessions.dev_b2, undefined)
    assert.ok(row?.sessions.dev_b3)
  })
  ...
  await t('BRG-104: revoking one member on a shared device must not drop the OTHER member\'s ack bag', () => {
    ...
    writeFileSync(
      join(dir, 'acks.json'),
      JSON.stringify({ [memberDeviceAckKey('mem_alice', 'shared-kiosk')]: { op_old: Date.now() - 100 * DAY }, [memberDeviceAckKey('mem_bob', 'shared-kiosk')]: { op_old: Date.now() - 100 * DAY } }),
      'utf8',
    )
    writeFileSync(join(dir, 'ops.jsonl'), `${JSON.stringify(op('op_old'))}\n`, 'utf8')
    const store = openStore(dir)
    assert.ok(store.deviceAckCount('shared-kiosk', 'mem_bob') > 0, 'precondition: Bob has a scoped receipt on the shared device')
    ...
    assert.ok(
      store.deviceAckCount('shared-kiosk', 'mem_bob') > 0,
      'Bob still holds a live session for shared-kiosk - the ack bag must survive Alice\'s revoke',
    )
    // Alice’s receipt is gone independently; Bob’s receipt survives until his own revoke.
    assert.equal(store.deviceAckCount('shared-kiosk', 'mem_alice'), 0)
