 ...
 * (BRG-067). BRG-069 data plane: no workers, no cluster. The optional
 * encrypted mail store has one bounded storage worker, not a bridge listener
 * or fanout worker. Only its exact launch sites are excepted below.
 *
...
function collectOffenders(files: string[]): string[] {
  ...
  for (const abs of files) {
    ...
    hits.push(...liveHitsInFile(rel, withoutMailStorageLaunches(rel, readFileSync(abs, 'utf8'))))
  }
  return hits
}

function withoutMailStorageLaunches(rel: string, text: string): string {
  if (rel === 'src/mail-store.ts') return text
    .replace("import { Worker } from 'node:worker_threads'", '')
    .replace('new Worker(compiled, { workerData, execArgv })', 'approvedMailStorageWorker')
    .replace('new Worker(bootstrap, { eval: true, workerData })', 'approvedMailStorageWorker')
  if (rel === 'src/mail-store-worker.ts') return text.replace("import { parentPort, workerData } from 'node:worker_threads'", '')
  return text
}
...
await t('only the bounded encrypted mail-store launch sites are excepted', () => {
  const facade = readFileSync(join(srcRoot, 'mail-store.ts'), 'utf8')
  const storage = readFileSync(join(srcRoot, 'mail-store-worker.ts'), 'utf8')
  assert.match(facade, /new URL\('\.\/mail-store-worker\.js', import\.meta\.url\)/)
  assert.match(facade, /new URL\('\.\/mail-store-worker\.ts', import\.meta\.url\)/)
  assert.match(facade, /this\.pending\.size >= \(operation === 'close' \? 1024 : 1023\)/)
  assert.match(facade, /this\.worker\.terminate\(\)/)
  assert.doesNotMatch(storage, /createServer|WebSocketServer/)
  for (const rel of ['src/mail-store.ts', 'src/mail-store-worker.ts']) {
    const original = rel.endsWith('-worker.ts') ? storage : facade
    assert.equal(isForbiddenLive(sourceWithoutComments(withoutMailStorageLaunches(rel, original))), false)
    for (const injected of ["\nnew Worker(other)", "\nimport cluster from 'node:cluster'", "\nimport { Worker } from 'node:worker_threads'"]) {
      assert.equal(isForbiddenLive(sourceWithoutComments(withoutMailStorageLaunches(rel, original + injected))), true)
    }
  }
})

await t('bridge data plane has no live workers or cluster', () => {
...
if (passed !== 16) {
    console.error(`g16-no-workers: expected 16, got ${passed}`)
    process.exit(1)
  }
  console.log(`g16-no-workers: ${passed}/16 ok`)
} catch (err) {
  console.error(err)
  process.exit(1)
}
