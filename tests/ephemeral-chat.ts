...
// ── Group room id: mint (bridge-only) + parse + never confusable with 1:1 ──
{
  ...
  const any11 = parseAnyEphemeralRoomId('eph:bob.alice')
...
// ── TS-CHAT-145: leaving the team ends that member's temporary chats now ──
{
  ...
  const finalizeAt = serverSrc.indexOf('function finalizeKickedMembers(')
  must(finalizeAt > 0, 'batch offboarding helper must exist')
  const finalizeBody = live(serverSrc.slice(finalizeAt, serverSrc.indexOf('\n}', finalizeAt)))
  must(finalizeBody.includes('handleMemberLeaveChat(memberId)'), 'each kicked member must run chat departure teardown')
  // One/many kicks delegate to the same helper; voluntary leave calls the
  // member teardown directly. Comments cannot satisfy either contract.
  for (const frame of ['kick_member', 'kick_members', 'leave_team']) {
    const at = serverSrc.indexOf(`case '${frame}': {`)
    must(at > 0, `${frame} handler must exist`)
    const nextCase = serverSrc.indexOf('\n      case ', at + 1)
    must(
      live(serverSrc.slice(at, nextCase)).includes(frame === 'leave_team' ? 'handleMemberLeaveChat(' : 'finalizeKickedMembers('),
...
// Real owner identity uses device:, while group and arbitrary colon prefixes remain forbidden.
{
  const room = ephemeralRoomId('device:owner-123', 'mem_bob')
  must(room === ephemeralRoomId('mem_bob', 'device:owner-123'), 'device identity canonical order')
  must(parseAnyEphemeralRoomId(room).ok, 'device identity room parses')
  for (const bad of ['g:owner', 'device:', 'device:a.b', 'device:a:b', 'device:' + 'x'.repeat(122)]) {
    must(ephemeralRoomId(bad, 'mem_bob') === null, 'malformed device identity refused')
  }
}
