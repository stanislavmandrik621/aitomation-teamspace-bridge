...
/** Bound the handler by the next sibling label, independent of case ordering. */
function sliceSwitchCase(src: string, name: string): string {
  const label = new RegExp(`^([ \t]*)case '${name}':`, 'm').exec(src)
  assert.ok(label, `missing switch case ${name}`)
  const afterLabel = label.index + label[0].length
  const next = src.slice(afterLabel).search(new RegExp(`^${label[1]}(?:case\\b|default\\s*:)`, 'm'))
  assert.ok(next >= 0, `missing next sibling case after ${name}`)
  return src.slice(label.index, afterLabel + next)
}
...
describe('BRG-066 source-scan server.ts', () => {
  ...
  it('all ops use the current-ACL bucketed serialize-once fanout', () => {
    const body = sliceNamedFunction(server, 'handleOps')
    assert.match(body, /sendColleagueScopedOps\(ws, frameId, applied\)/)
    assert.doesNotMatch(body, /const teamWide = applied\.filter/, 'missing sender audience cannot bypass current ACL')
  ...
  it('set_team_name / set_role / ephemeral shared-frame loops serialize once', () => {
    const nameCase = sliceSwitchCase(server, 'set_team_name')
    const nameFrame = nameCase.indexOf('const peerFrame: BridgeFrame')
    assert.ok(nameFrame >= 0, 'set_team_name shared frame exists')
    assertSerializeOnceLoop(nameCase.slice(nameFrame), 'set_team_name team_name_peer')

    const roleCase = sliceSwitchCase(server, 'set_role')
    const roleFrame = roleCase.indexOf('const rolePeer: BridgeFrame')
    assert.ok(roleFrame >= 0, 'set_role shared frame exists')
    assertSerializeOnceLoop(roleCase.slice(roleFrame), 'set_role role_peer')
    assert.match(roleCase, /forceCloseBackpressured\(peerWs, 'role update backpressure'\)/,
      'a target device that misses an authority downgrade must reconnect')
