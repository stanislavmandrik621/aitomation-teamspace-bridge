...
// TS-CHAT-111 (Wave 14 Box 12, I6): `chat_typing` used to gate the token
// bucket behind `typingOn && !takeChatTypingToken(...)` - a "stop" frame (or
// any `typing` value that is not strictly boolean `true`) short-circuited
// past the bucket entirely and reached `fanoutChatTypingPeer` with ZERO rate
// limiting, unlike every other mutating case in this switch. Pin structural
// facts that fail on the old short-circuit and pass on the fixed gate:
// (1) the token check's own guard condition must not reference `typingOn`
// (proving it is NOT conditioned on the start/stop direction), and (2) the
// vulnerable `typingOn && ...takeChatTypingToken(` bypass pattern is absent
// from the whole file.
{
  const window = caseWindow('chat_typing', 'chat_edit')
  const guardMatch = window.match(/if\s*\(([^)]*takeChatTypingToken\([^)]*\)[^)]*)\)\s*\{/)
  assert.ok(guardMatch, 'chat_typing gates on a takeChatTypingToken(...) if-condition')
  assert.ok(
    !guardMatch![1].includes('typingOn'),
    'chat_typing takeChatTypingToken gate must not be conditioned on typingOn (else stops/garbage bypass the bucket)',
  )
  const tokenIdx = window.indexOf('takeChatTypingToken(')
  const fanoutIdx = window.indexOf('fanoutChatTypingPeer(')
  assert.ok(tokenIdx > 0 && fanoutIdx > tokenIdx, 'chat_typing gates the token before fanoutChatTypingPeer')
}
assert.ok(
  !server.includes('typingOn && !takeChatTypingToken('),
  'TS-CHAT-111: the typingOn-gated token bypass must never return - every chat_typing frame (start or stop) must consume the same bucket',
)
