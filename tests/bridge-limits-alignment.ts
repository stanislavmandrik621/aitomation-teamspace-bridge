...
describe('TCC-R1133-WS-003 - presence join fanout is coalesced (no O(N^2) presence_peer storm)', () => {
  it('schedulePresenceFanoutBroadcast exists and is a debounced setTimeout-based single dispatcher', () => {
    ...
    assert.match(body, /for \(const \[sock, recipient\] of live\)/, 'visits every live socket to resolve its private editing audience')
    assert.match(body, /presenceSnapshotNow\(recipient, snapshot\)/, 'editing metadata must use recipient authority over one shared base snapshot')
    assert.match(body, /for \(const sock of bucket.sockets\)/, 'identical authorized snapshots are serialized once per audience')
    assert.match(body, /forceCloseBackpressured\(sock, 'presence fanout backpressure'\)/,
      'a peer that misses the authoritative full snapshot must reconnect')
  })
...
describe('TCC-R1133-SEC-001 - chat attachment/avatar HTTP GET has a member-scoped rate limit', () => {
  it('GET /v1/chat/avatars/:sha checks takeChatAvatarGetToken before serving bytes', () => {
    ...
    assert.match(window, /sendRateLimited\(res,/, 'refuses via the shared 429 + Retry-After helper when the budget is exhausted')
