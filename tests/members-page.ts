...
assert.equal(p0.unreadable, false)
...
assert.equal(over.unreadable, false)

assert.equal(pageMembersList([]).unreadable, false)
assert.equal(pageMembersList([]).total, 0)

const unreadObj = pageMembersList({ oops: true } as never, 100, 0)
assert.equal(unreadObj.unreadable, true)
assert.equal(unreadObj.members.length, 0)
assert.equal(unreadObj.total, 0)
assert.equal(unreadObj.has_more, false)

const unreadNull = pageMembersList(null as never)
assert.equal(unreadNull.unreadable, true)

const unreadStr = pageMembersList('mem_a,mem_b' as never)
assert.equal(unreadStr.unreadable, true)
assert.equal(unreadStr.total, 0, 'a string must not page as an array-like roster')
