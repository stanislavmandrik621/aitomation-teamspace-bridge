...
async function main(): Promise<void> {
  ...
  const revokeAt = storeSrc.indexOf('revokeSession(args:')
  assert(revokeAt >= 0, 'revokeSession present')
  const revokeSlice = storeSrc.slice(revokeAt, revokeAt + 900)
  assert(
    revokeSlice.includes('return this.withMembersMutation(() => {'),
    'revokeSession must serialize under withMembersMutation',
  )
