...
t('source-scan: handleOps ACKs and fans out from append outcomes, not input rows', () => {
  const serverSrc = readFileSync(join(root, 'src/server.ts'), 'utf8')
  const body = sliceFunction(serverSrc, 'handleOps')
  const live = body
    .replace(/\/\*[\s\S]*?\*\//g, '')
    .replace(/(^|[^:'"])\/\/[^\n]*/g, '$1')
  assert.match(live, /const\s+persisted\s*=\s*store\.appendOps\(toPersist\)/)
  assert.match(live, /for\s*\(const outcome of persisted\.outcomes\)/)
  assert.match(live, /applied\.push\(outcome\.op\)/)
  assert.doesNotMatch(live, /applied\.push\(stamped\)/)
})
...
t('source-scan: appendFileSync is outside the per-op try (disk throw propagates)', () => {
  const body = sliceClassMethod(storeSrc, 'appendOps')
  const source = ts.createSourceFile('append.ts', `class Fixture { ${body} }`, ts.ScriptTarget.Latest, true)
  const writes: ts.CallExpression[] = []
  const visit = (node: ts.Node): void => {
    if (ts.isCallExpression(node) && ts.isIdentifier(node.expression) && node.expression.text === 'appendFileSync') writes.push(node)
    ts.forEachChild(node, visit)
  }
  visit(source)
  assert.equal(writes.length, 1, 'one durable batch append')
  for (let node: ts.Node | undefined = writes[0]!.parent; node && !ts.isMethodDeclaration(node); node = node.parent) {
    assert.equal(ts.isTryStatement(node), false, 'append failure must not be swallowed by a try/catch')
    assert.equal(ts.isForStatement(node) || ts.isForOfStatement(node) || ts.isWhileStatement(node), false, 'batch append must not become a per-item write')
  }
  // The separate checkpoint catch deliberately freezes content and rethrows;
  // counting every try block would incorrectly reject that safety boundary.
  assert.match(body, /catch \(error\) \{ this\.contentAccess\.failClosed\(\); throw error \}/)
})
...
t('appendOps reports accepted, exact-retry, and conflicting-replay outcomes', () => {
  withStore((store, opsPath) => {
    const first = op('op_outcome', { targetId: `${'t'.repeat(199)}\u{1F600}` })
    const written = store.appendOps([first])
    assert.equal(written.outcomes[0]?.status, 'accepted')
    assert.equal(written.accepted.length, 1)
    assert.equal(written.accepted[0]?.targetId, 't'.repeat(199))

    const exact = store.appendOps([first])
    assert.deepEqual(exact, {
      accepted: [],
      outcomes: [{ status: 'idempotent', opId: 'op_outcome' }],
    })
    const conflict = store.appendOps([op('op_outcome', { targetId: 'altered' })])
    assert.deepEqual(conflict, {
      accepted: [],
      outcomes: [{ status: 'conflict', opId: 'op_outcome' }],
    })
    assert.equal(jsonlRecords(opsPath).length, 1)
  })
})

t('restart rebuilds durable op identity and refuses an altered replay', () => {
  withStore((store, opsPath) => {
    const original = op('op_restart_identity', { patch: { title: 'canonical' } })
    store.appendOps([original])
    const reopened = new BridgeStore(dirname(opsPath), 21, null)
    assert.deepEqual(reopened.appendOps([original]), {
      accepted: [],
      outcomes: [{ status: 'idempotent', opId: 'op_restart_identity' }],
    })
    assert.deepEqual(
      reopened.appendOps([op('op_restart_identity', { patch: { title: 'altered' } })]),
      {
        accepted: [],
        outcomes: [{ status: 'conflict', opId: 'op_restart_identity' }],
      },
    )
    assert.equal(jsonlRecords(opsPath).length, 1)
  })
...
t('batch admission is linear and a rejected hierarchy writes nothing', () => {
  const dir = mkdtempSync(join(tmpdir(), 'brg-067-admission-'))
  try {
    const store = new BridgeStore(dir, 21, null)
    let treeCalls = 0, teamworkItems = 0
    const treeValidate = store.recordTree.validateCommit.bind(store.recordTree)
    const teamworkValidate = store.recordTeamwork.validateCommit.bind(store.recordTeamwork)
    store.recordTree.validateCommit = ops => { treeCalls++; return treeValidate(ops) }
    store.recordTeamwork.validateCommit = (ops, keys, accepted) => {
      teamworkItems += ops.length
      return teamworkValidate(ops, keys, accepted)
    }
    const rows = Array.from({length: 1000}, (_, i) => op('linear-' + i))
    store.appendOps(rows)
    assert.equal(treeCalls, 1)
    assert.equal(teamworkItems, rows.length, 'each new operation is admitted once')
    const before = readFileSync(join(dir, 'ops.jsonl'), 'utf8')
    const create = (id: string, parent: string) => op(id, {
      kind: 'record.create', targetKind: 'record', entityId: 'tree',
      patch: {data: {name: id}, parent_id: parent},
    })
    assert.throws(() => store.appendOps([create('cycle-a', 'cycle-b'), create('cycle-b', 'cycle-a')]), /loop/)
    assert.equal(readFileSync(join(dir, 'ops.jsonl'), 'utf8'), before)
    assert.equal(store.hasSeenOpId('cycle-a'), false)
    assert.equal(store.recordTree.node('cycle-a'), undefined)
    assert.equal(store.appendOps(rows).accepted.length, 0, 'exact retry stays idempotent')
    assert.equal(readFileSync(join(dir, 'ops.jsonl'), 'utf8'), before)
  } finally { rmSync(dir, {recursive: true, force: true}) }
})

console.log(`brg-067-append-ops-batch: ${passed} ok`)
