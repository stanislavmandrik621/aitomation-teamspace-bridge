 ...
 * must close revoked WS peers without disconnecting authorized additional devices.
 *
...
describe('WS peer close on invite redeem / leave_team (TCC-R1125-BRG-001 / TCC-R1126-BRG-001)', () => {
  it('HTTP /v1/invite/redeem closes only revoked live WS peers for the redeemed member', () => {
    ...
    assert.match(window, /!store\.findBySession\(peer\.sessionToken\)/, 'preserves other authorized project sessions')
  ...
  it('leave_team closes every live WS socket for the leaving member (incl. actor + sibling devices)', () => {
    ...
    // Revoke every live authorization slot before fallible chat/Compose
    // cleanup; then reply before close so the actor receives the ack.
    const dropIdx = window.indexOf('dropLiveSession(peerWs)')
    const composeCleanupIdx = window.indexOf('removeMembersFromComposeAuthority(')
    const chatCleanupIdx = window.indexOf('handleMemberLeaveChat(')
    const replyIdx = window.indexOf('leave_team_ok')
    const closeIdx = window.indexOf("peerWs.close(4005")
    assert.ok(dropIdx > 0 && composeCleanupIdx > dropIdx, 'drops every live session before Compose cleanup')
    assert.ok(chatCleanupIdx > dropIdx, 'drops every live session before fallible chat cleanup')
    assert.ok(replyIdx > 0 && closeIdx > replyIdx, 'replies leave_team_ok before closing sockets')
    assert.match(window, /chatCleanupFailed/)
    assert.match(window, /contentAclCleanupFailed/)
  })
