...
async function main(): Promise<void> {
  ...
  try {
    ...
    const overlongRedeem = await store.redeemInvite({
      token,
      deviceId: `${maxDeviceId}x`,
      displayName: 'Joiner',
    })
    assert(overlongRedeem.ok === false, 'over-cap device selector is refused without consuming invite')
