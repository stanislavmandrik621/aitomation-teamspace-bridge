...
import {
  ...
  ChatBlobRegistry,
} from '../src/chat-blob-registry.js'
...
describe('TCC-R1132-BLOB-001 - chat blob registry ACKs only after durable persist', () => {
  ...
  it('rechecks upload authority in the serialized durable-commit turn', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'ts-chat-blob-revoked-commit-'))
    try {
      const reg = new ChatBlobRegistry(dir, null)
      let releasePersist!: () => void
      const heldPersist = new Promise<void>((resolve) => { releasePersist = resolve })
      ;(reg as unknown as { writeChain: Promise<void> }).writeChain = heldPersist

      const bytes = new TextEncoder().encode('must disappear when upload authority is revoked')
      const sha = (await import('node:crypto'))
        .createHash('sha256')
        .update(bytes)
        .digest('hex')
      let authorized = true
      const pending = reg.registerUpload({
        roomId: 'chat:team',
        uploadedBy: 'member1',
      ...
      // The body is written synchronously just before registerUpload queues
      // its registry persist. Seeing it proves the upload passed scanning and
      // is now held at the final durable-commit boundary.
      for (let i = 0; i < 1_000 && !existsSync(reg.blobPath(sha)); i++) {
        await new Promise<void>((resolve) => setTimeout(resolve, 2))
      }
      assert.equal(existsSync(reg.blobPath(sha)), true, 'upload reaches the held commit boundary')
      authorized = false
      // Queue a second registry persistence behind the rejected upload. The
      // revoked row must be rolled back inside the first serialized turn, or
      // this later write would accidentally make it durable.
      assert.equal(reg.touchReference(sha, Date.now() + 2 * 60 * 60 * 1000), true)
      releasePersist()

      assert.deepEqual(await pending, { error: CHAT_BLOB_UPLOAD_AUTH_REVOKED })
      await (reg as unknown as { writeChain: Promise<void> }).writeChain
      assert.equal(existsSync(reg.blobPath(sha)), false, 'revoked pre-commit upload rolls back its bytes')
      const persisted = JSON.parse(
        readFileSync(join(dir, 'chat', 'blob-registry.json'), 'utf8'),
      ) as { blobs: Array<{ sha256: string }> }
      assert.equal(
        persisted.blobs.some((row) => row.sha256 === sha),
        false,
        'a later queued persist cannot capture the revoked in-memory row',
      )
    } finally {
      rmSync(dir, { recursive: true, force: true })
    }
